# Verlinde Origin Gravity 2010

**Source:** `13_Verlinde_Origin_Gravity_2010.pdf`

---

arXiv:1001.0785v1 [hep-th] 6 Jan 2010             On the Origin of Gravity
                                                                          ...
                                                                          University of Amsterdam
                                                                             ...
                                                                             1018 XE, Amsterdam
                                                                                The Netherlands
                                                                                         ...
                                                                                         Abstract
                                               Starting from first principles and general assumptions Newton's law of grav-
...
Of all forces of Nature gravity is clearly the most universal. Gravity influences and
is influenced by everything that carries an energy, and is intimately connected with
...
The quest for unification of gravity with these other forces of Nature, at a microscopic
...
gravity allows an action at a distance even when there in no mediating force field.
...
equation is expressed in terms of entropy differences, and is independent of the details
of the microscopic dynamics. In particular, there is no fundamental field associated
...
effects. Osmosis is another phenomenon driven by an entropic force.

    Perhaps the best known example is the elasticity of a polymer molecule. A single
polymer molecule can be modeled by joining together many monomers of fixed length,
where each monomer can freely rotate around the points of attachment and direct itself
in any spatial direction. Each of these configurations has the same energy. When the
polymer molecule is immersed into a heat bath, it likes to put itself into a randomly
coiled configuration since these are entropically favored. There are many more such
...
out of its equilibrium configuration by an external force F , as shown in figure 1. For
definiteness, we keep one end fixed, say at the origin, and move the other endpoint
along the x-axis. The entropy equals

                      S(E, x) = kB log (E, x)     (2.1)

where kB is Boltzman's constant and (E, x) denotes the volume of the configuration
space for the entire system as a function of the total energy E of the heat bath and
the position x of the second endpoint. The x dependence is entirely a configurational
effect: there is no microscopic contribution to the energy E that depends on x.
...
In the canonical ensemble the force F is introduced in the partition function3
             ...
             Z(T, F ) = dEdx (E, x) e-(E+F x)/kBT .                             (2.2)

as an external variable dual to the length x of the polymer. The force F required to
keep the polymer at a fixed length x for a given temperature E can be deduced from
...
reduced. For an infinite heat bath this would be by the same amount as the increase in
...
forces. Specifically, one considers the micro canonical ensemble given by (E + F x, x),
...
One easily verifies that this leads to the same equations (2.3). However, it illustrates

that microcanonically the temperature is in general position dependent, and the force

also energy dependent. The term F x can be viewed as the energy that was put in to
...
return to its equilibrium position, but that the entropy in first approximation stays

the same. Our aim in the following sections is to argue that gravity is also an entropic

force, and that the same kind of reasonings apply to it with only slight modifications.
...
formation is finite for each part of space, it is impossible to localize a particle with
infinite precision at a point of a continuum space. In fact, points and coordinates
...
to be given by a local field theory, or anything familiar. The microscopic details are
irrelevant for us.
...
that the microscopic theory has a well defined notion of time, and its dynamics is time
translation invariant. This allows one to define energy, and by employing techniques
...
mass m attached to a fictitious "string" that is lowered towards a black hole. Just
before the horizon the particle is dropped in. Due to the infinite redshift the mass
...
it increases the mass and horizon area by a small amount, which he identified with one
bit of information. This lead him to his area law for the black hole entropy.

    We want to mimic this reasoning not near a black hole horizon, but in flat non-
...
but before it does so, it already influences the amount of information that is stored on
the screen. The situation is depicted in figure 2.
...
and the membrane carries a temperature, it will experience an effective force equal to
...
as is easily verified by combining (3.8) together with (3.6) and (3.7).
...
specifically, let us assume it is a sphere. For the following it is best to forget about
...
area may be defined: each fundamental bit occupies by definition one unit cell.
...
tified with Newton's constant, of course. But since we have not assumed anything
yet about the existence a gravitational force, one can simply regard this equation as
the definition of G. So, the only assumption made here is that the number of bits is
...
pdaisrttriobfustpedacoevseurrtrhoeunodcceudpbieydtbhietss,carenedn.is equivalent to the mass M that would emerge in the

is then determined by the equipartition rule                                       (3.11)
                                                      1
...
we have reversed these arguments. But the logic is clearly different, and sheds new
...
soning can be generalized to arbitrary dimensions4 with the same conclusion. But how
...
the entropic force. In the non-relativistic setting this heat bath is infinite, but in
principle one has take into account that the heat bath loses or gains energy when the
particle changes its location under the influence of an external force. This will lead to
...
we needed to introduce Planck's constant in the first place, since the only aim was
...
for dimensional reasons. It can therefore be chosen at will, and defined so that (3.5)
...
(R0/R)2, while the temperature is divided by the same factor. Effectively, this means
...
proposal for precisely how it is distributed, even for general matter configurations.

4In  d  dimensions  (3.10)  includes  a  factor  1  d-2  to  get  the  right  identification  with  Newton's  constant.
                                                 2  d-3
...
to connect space at different times. In fact, it may appear somewhat counter-intuitive

that the temperature T is related to the vector quantity a, while in our identifications
...
Since  each  bit  carries  an  energy  1  kB  T  ,  the  number    of  bits  n  follows  from
                                       2
...
succeeded in making the identifications more natural in terms of their scalar versus
...
a finite entropy associated with each matter configuration. This entropy measures the
...
of these dualities is by many still believed to be independently defined. But in our
...
Gravity and closed strings are not much different, but we just have not yet got used
to the idea.
...
space that corresponds to a "coarse graining" variable, something like the cut-off scale
of the system on the screens.

    A priori there is no preferred holographic direction in flat space. However, this is
...
direction is given by the gradient  of the Newton potential. In other words, the
holographic screens correspond to equipotential surfaces. This leads to a well defined
...
enclose different regions of space. This is depicted in figure 4.
...
belong to the same macroscopic state. The entropy for each of these state is defined
as the number of microscopic states that flow to the same macroscopic state.
...
matter at fixed locations. Hence, it is described by a static matter density (r). Our
...
fixed Newton potential 0. We assume that the entire mass distribution given by (x) is
...
multiplied by the temperature. The difference with the spherically symmetric case is
that the temperature on the screen is not necessarily constant. Indeed, the situation

                                                      13
is in general not in equilibrium. Nevertheless, one can locally define temperature and
...
by our earlier discussion we define temperature analogous to (3.8), namely by
...
eration, but we don't know yet whether it satisfies an equation that relates it to the
mass distribution.
...
mass is added to the region enclosed by the screen S, for example, by first putting it
close to the screen and then pushing it across, the mass M should change accordingly.
This condition can only hold in general if the potential  satisfies the Poisson equation

2(r) = 4G (r).      (4.21)

We conclude that by making natural identifications for the temperature and the in-
...
Figure 5: A general mass     distribution inside  mthaessneostmyei tareemleorcgaetdedpaarttaorfbistpraacrey  enclosed   by
                             test particles with                                                             points ri  in
the screen. A collection of

the already emerged space outside the screen. The forces Fi due to gravity are determined

by the virtual work done after infinitesimal displacement ri of the particles.
...
arbitrary points outside the screen. For this we need a generalization of the first pos-
tulate (3.6) to this situation. What is the entropy change due to arbitrary infinitesimal
displacements ri of the particles? There is only one natural choice here. We want to
find the change s in the entropy density locally on the screen S. We noted in (3.16)
that the Newton potential  keeps track of the changes of information per unit bit.
Hence, the right identification for the change of entropy density is

                                                                                                             (4.22)
                                 s = kB 2c2 dN

where  is the response of the Newton potential due to the shifts ri of the positions
of the particles. To be specific,  is determined by solving the variation of the Poisson

equation

          2(r) = 4G mi ri �i (r - ri)                                                                        (4.23)

                                     i

One can verify that with this identification one indeed reproduces the entropy shift
...
by all of the forces on the test particles is determined by the first law of thermody-
...
nor a reason why curvatures would not be present. Specifically, we would like to see
how Einstein's general relativity emerges from similar reasonings as in the previous
section. We will indeed show that this is possible. But first we study the origin of
...
Killing vector field with the temperature and the entropy gradients. In particular, we
...
that at a reference point with  = 0, which we will take to be at infinity.

    Just like in the non relativistic case, we like to use  to define a foliation of space,
...
force is less clearly defined, since it can be transformed away by a general coordinate
transformation. But by using the time-like Killing vector one can given an invariant
meaning to the concept of force [11].

    The four velocity ua of the particle and its acceleration ab  uaaub can be expressed
...
and the definition of . One finds that the acceleration can again be simply expressed
...
outward pointing vector N b normal to the screen S and to b.
...
respect to the reference point at infinity.
...
position near the screen, as measured from the reference point at infinity. It is the
...
One easily verifies that this leads to the same equation (5.30). This fixes the equilibrium
...
(x) is entirely fixed by the other matter in the system.

    We have arrived at equation (5.31) by making use of the identifications of the
temperature and entropy variations in space time. But actually we should have gone
the other way. We should have started from the microscopics and defined the space
...
to a gravitational field or due to the fact that one considers an accelerated frame.
...
. We assume that it is enclosing a certain static mass configuration with total mass
...
a  mass  unit  equal  to  1  T  .  Hence
                          2
...
hand side is precisely Komar's definition of the mass contained inside an arbitrary
...
identifications for the temperature and the number of bits on the screen. But we don't

                                               19
know yet whether it satisfies any field equations. The key question at this point is
whether the equation (5.34) is sufficient to derive the full Einstein equations.
...
the Killing vector a as
                ...
                1       dxa dxb abcdcd             (5.35)
       M=
                      S
              8G

The left hand side can be expressed as an integral over the enclosed volume of certain
components of stress energy tensor Tab. For the right hand side one first uses Stokes
...
hand side can presumably be fixed by comparing properties on both sides, such as for
...
screens. This is sufficient to obtain the full Einstein equations.
...
The Komar definition of the mass will again be useful for this purpose. The definition
...
this will effect the definition of the Komar mass of the matter configuration inside the
screen. In fact, the temperature on the screen will be directly affected by a change in
the redshift factor.
...
however. In fact, also the spatial metric may be affected by this displacement. We are
...
in the amount of information. These are de difference that will lead to the entropic
...
dependences should cancel. So we impose
              ...
              S M (xi +xi), xi +xi = S M (xi), xi  (5.38)
...
The point is that the first term represents the force that acts on the i-th particle due
...
the energy in the heat bath due to entropic effects.

    Since the Komar integral is defined on the holographic screen, it is clear that like
...
the similarity, but finally do away with gravity as a fundamental force.
...
the gravitational field, we uncovered what is the origin of force and inertia in a context
in which space is emerging. We identified a cause, a mechanism, for gravity. It is driven
by differences in entropy, in whatever way defined, and a consequence of the statistical
averaged random dynamics at the microscopic level. The reason why gravity has to
keep track of energies as well as entropy differences is now clear. It has to, because
...
not exist to begin with. The profound nature of these questions in our view justifies
the heuristic level of reasoning. The assumptions we made have been natural: they fit
...
Wilsonian effective field theory, namely that integrating out short distance degrees
of freedom only generate local terms in the effective action, most of which become
...
effective description. It arises not in the same space as the microscopic theory, but in
...
Figure 6: The microscopic theory in a) is effectively described by a string theory consisting

of open and closed strings as shown in b). Both types of strings are cut off in the UV.
...
language, the holographic screens can be identified with D-branes, and the microscopic
degrees of freedom on these screens represented as open strings defined with a cut off
in the UV. The emerged part of space is occupied by closed strings, which are also
defined with a UV cut off, as shown in figure 6. The open and closed string cut offs
are related by the UV/IR correspondence: pushing the open string cut off to the UV
forces the closed string cut off towards the IR, and vice versa. The value of the cut offs
...
microscopically: they are emergent as an effective description.
...
is a well defined fundamental theory. But in our view gravity and closed strings are
...
resistant in accepting the obvious. Gravity and closed strings are not much different,
...
to the very last. The first cracks in the fundamental nature of gravity appeared when
...
the mass m on the other, must be such that F = T S translates in to the gravitational
...
the correspondence rules must be such that F = T S translates in to the inertial force
...
valid with identified with the actual Planck constant. However, we have no direct
confirmation or proof that the same is true when we go away from the horizon, or
eespecially when there is no horizon present at all. In fact, there are reasons to believe
that the equations work slightly different there. The first is that one is not exactly
at thermal equilibrium. Horizons have well defined temperatures, and clearly are in
...
making use of the freedom to rescale the value of . This would not effect the final
outcome for the force, nor the fact that it is entropic. In fact, one can even multiply
by a function f (0) of the Newton potential on the screen. This rescaling would affect
...
other modifications possible, like a description that uses weighted average over many
screens with different temperatures. Even then the essence of our conclusions would
...
References
  ...
  [6] T. Damour, "Surface effects in black hole physics, in Proceedings of the Sec-
       ond Marcel Grossmann Meeting on General Relativity", Ed. R. Ruffini, North
       Holland, p. 587, 1982
