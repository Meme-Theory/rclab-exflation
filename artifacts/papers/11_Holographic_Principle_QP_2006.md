# Holographic Principle QP 2006

**Source:** `11_Holographic_Principle_QP_2006.pdf`

---

arXiv:physics/0611104v1 [physics.comp-ph] 10 Nov 2006                 Holographic Principle and Quantum Physics

                                                                                      Zolt�an Batiz  and Bhag C. Chauhan 1
                                                                                1 Centro de F�isica Te�orica das Part�iculas (CFTP)
...
We create a two-dimensional hologram (II A). Next, we describe the reconstruction of the

                                                        3
hologram in the usual three-dimensional space (II B). It must be noted that the holograms
...
angle r between the beam itself and the x4 axis. We didn't take it in the (x4, x5) plane
because in such case the reference wave that hits the screen remains always normal to it.
We assume that the phase of the reference beam is ri at the point (y, x4) = (0, 0). The
wavelength of the radiation is  = 2/k and k is the wave number. Therefore the phase of
the reference beam at an arbitrary site is r = ri + kx4 cos(r) - ky sin(r). On the screen
x4 = 0, so r = ri - ky sin(r).

   This wave has an amplitude Er0, so its value is

Er = Er0 exp[i(ri - ky sin(r))].  (1)

4
reference beam                  screen
                                     (x,y) plane
...
FIG. 2: A schematic diagram showing the reference beam propagation in (y, x4) plane into the
...
d is the dimensionality of our space. However, ro is the initial phase of the wave which
...
interference picture that is the hologram.
   ...
   How do we determine Eo0? The energy that hits the object is SdEo20/2, where Sd is the
"cross section" of the object in d dimensions. Thus, in two dimensions it is 2ro (where ro is
the radius of the "spherical" object) in three dimensions it is ro2, and by generalization,
for d dimensions it is Vd-1, where Vd is the volume of the d dimensional sphere. The energy
that comes out (Io) is R2 times the one that goes in (Ii). The energy is spread out on the
whole "solid angle" d. d is determined from the following identities:

                                5

                       ddx exp (-ax2) = d exp (-ar2)rd-1dr
                                               0            (3)
...
and it is found to be
   ...
   where  is the Euler function. The d dimensional volume is drd/d. The energies going
in and coming out are given as Io = drd-1Eo2/2 and Ii = SdEi2/2, respectively. As discussed
...
it, one must know the reflected fields at any given point. The square of those fields is
                                                        ...
                                                        6
proportional to the intensity of the reflected radiation, and knowing that, we can have an
...
(r) = d3rG(r, r)(r) + d2rG(r, r)n(r) - d2r(r)nG(r, r)) ,  (10)
V     S                                                S
...
we must require that the normal derivative of the Green's function vanishes on the surface,
                                             ...
                                             7
we are said to make use of a Neumann Green's function. Note that any of these conditions
...
(in terms of our orthonormal basis e1, e2, e3), r1,2 = (r1,2 � r1,2). In these terms, the
...
G(r, r; t)  =  1      (t   - r1/c)  -  (t  - r2/c)   ,                                  (13)
               4            r1              r2

where c is the phase velocity of our wave. Since we consider only one frequency (), we only
need the Fourier transform of this Green's function, which is:

G(r, r; )   =  1     exp   (-ikr1)  -  exp  (-ikr2)     ,                               (14)
               4            r1               r2

with k = /c as the wave number. We substitute this result into Eq. (11). How can we
...
(r, t) = (r, t = 0) exp [-i(t - l/c)],                                                  (16)

where l is the distance between the source and observer. Substituting this into Eq. (15) and
dividing the resulting equation by exp (-i(t - l/c)) we obtain Eq. (6). So if we work with

                           8
the time Fourier transforms of the wave fields and Green's functions and assume only one
...
nG(r, r) =  1  (-2ik)  (z  -     z)  exp  (-ikr1)  -  (z    +  z)  exp  (-ikr2)  |z=0  (18)
            4                         r1                            r2
...
greater than three), Eq. (19) would be modified in the following way:
                 ...
                 nG(r,     r)    =   kz   exp(-ikr    )  ,                             (19)
                                             rd-2

   where d is the dimensionality of the space and  is an irrelevant constant we do not
...
reference beam in eq. 1).
               ...
               Erd = x^Er0 exp (-iky sin(r) + iri) ,                                   (21)

                                     9
   where Er0 is its amplitude, ri is its phase at the point (y, x4) = (0, 0) and r is its
angle of incidence. By construction, the reading beam is the original reference wave (2) that
propagates in the (y, x4) plane. There is a phase shift of  after the reflection.
...
This is the case, when, for example D = 5 units, d = 15 units and  = 0.5 units which we

                                                        10
                                                  D 5 7.5 10 12
                                                   0.83 0.64 0.40 0.30
                TABLE I: Empirical study (particle at rest) of the parameters:  and D

                                                D 50 62.5 75 87.5 100
                                                  1.0 1.3 1.6 1.9 2.2
                TABLE II: Empirical study (particle at rest) of the parameters:  and D
...
FIG. 3: The image for D = 5 units, d = 15 units and  = 0.5 units. We found that  = 0.83 units.

    is the width of the Gaussian curve. From table I it appears that   1/D. Gaussian
curves are obtained if   d,   D and d > D.

   Another feature we obtained from our procedure is a wave like pattern. This is the case,
if D = 100 units, d = 0.4 units and  = 5 � 10-3 units. Here  is the wavelength of the

                      11
pattern obtained. The specimen figure is shown in Fig. 4. The table II shows that   D.
...
FIG. 4: Holographic projection of a point particle at rest. Here D = 75 units, d = 0.4 units and
 = 0.005 units.
...
square of the momentum, prms. Therefore one can say that   D  1/ p2, so   1/prms.
...
the former calculations is that on the RHS of Eq. (2) the exponent r0 + kr, which is the
eikonal function of the wave field is replaced with the full eikonal. We do this because the
moving point is also a dynamical system. Therefore this exponent becomes r0 + kr + cP .R,
...
c = 1. Here we used a non-relativistic approximation, P = mv, where m is the mass of
...
                                             P � 10-4 2 3 4 5
               ...
               TABLE III: Empirical study (moving particle) of the parameters:  and P

This way we proved that the wavelength depends on the momentum, but not on the mass
or velocity. We found that the wavelength  of this wave depends on the momentum P . We
considered D = 40 units, d = 1.5 units and  = 5 � 10-3 units. We divided the screen into
702 equal squares in order to do the numerical integration involved in reading the hologram.
  ...
  1/P is satisfied. The only thing remaining to be clarified is the origin of the spinorial
...
solved.
   ...
   For a point like particle, in the non-relativistic context, the variational principle reads:
                     ...
                     dt                m   v2  -  V   = minimum .                               (22)
                                           2

Here m is the particle mass, v is its velocity, and V is its potential energy. This variational

principle  leads to  the conservation  of  H   =  1  mv  2  +  V  ,  which  happens  to be the  energy.  In
                                                  2

relativistic machanics, if we neglect interactions, there are two variational principles for a

                                                  14
point-like particle. The first one is not manifestly covariant

        - mc2  1                           -  v2  dt  =  minimum  .  (23)
                                              c2

The function H = mc2 is the corresponding conserved quantity that is the energy as it
    v2
1-  c2
...
The manifestly covariant principle reads:
   ...
   But here u stands for the four-velocity and  for the proper time. However, the conserved
...
do communicate instantaneously. It doesn't matter whether they are 1 meter or 1 billion
                                                        ...
                                                        15
kilometers apart. This result is hard to digest, however a British physicist David Bohm [12]
...
relation is similar to the famous de Broglie-type relation:   1/prms.
...
length and momentum is similar to that of the de Broglie relation, it could be the the
                                                        ...
                                                        16
wave-pattern which we associate with all the particles in quantum mechanics. If this is so,
...
of the particle.
   ...
   As said before the relation   1/P and the wave pattern we found as a holographic
...
conditions e.g.   d,   D,   1/N, d  D and  > d/N. Even in the real world
...
VII. ACKNOWLEDGMENTS
   ...
   The work of BCC was supported by Fundac�~ao para a Ci^encia e a Tecnologia through the
grant SFRH/BPD/5719/2001.

 [1] Collier, R., Burckhardt, C., Lin, L., "Optical Holography," 1971, Academic Press, Inc., New
      York.

 [2] http://en.wikipedia.org/wiki/Holography.
 [3] Bohm, D., "Quantum Theory as an Indication of a New Order in Physics�Implicate and

      Explicate Order in Physical Law," PHYSICS (GB), 3.2 (June 1973), pp. 139-168.
 [4] Talbot, M., "The Holographic Universe" 1991, HarperCollins Publishers, Inc., New York.
 [5] Pribram, K., "The neurophysiology of remembering", Scientific American, 220:75, 1969.
 [6] Susskind, L.and Lindesay, J., "Black Holes, Information and the String Theory Revolution.

      The Holographic Universe", World Scientific, December 2004.
 [7] Jacob D. Bekenstein, "Information in the Holographic Universe," August 2003; Scientific

      American Magazine.
 [8] Luis de-Broglie, Phil. Mag. 46 (1924) 446.
 [9] Max Born, Emil Wolf, Principles of Optics, 1975, Pergamon Press.
[10] Leonard Eyges, "The Classical Electromagnetic Field," 1980, Dover Publications, Inc. New

      York.
[11] Aspect, A., Dalibard, Roger, G, "Experimental test of Bell's inequalities using time-varying

      analyzers", Physical Review Letters 49, 1804 (20 Dec 1982).
[12] Bohm, D., "Hidden variables and the implicate order." In Quantum Implications, eds. B.

      Hiley and F. Peat. London: Routledge and Kegan Paul (1987).
[13] Susskind, L., Sci. Am. 276 (1997) 52; J. Maths. Phys. 36 (1995).
[14] Taubes, G., Science 285, (1999) 512.
[15] t' Hooft G., "Dimenssional reduction in quantum gravity" Slamfest 1993, p 284;

      gr-qc/9310026.
[16] Zoltan Batiz and Bhag C. Chauhan, in preparation.
