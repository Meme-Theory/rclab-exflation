...
| File | Description |
|:-----|:-----------|
| `computations/s51_polariton.py` | Computation script |
| `computations/s51_polariton.npz` | All numerical results |
...
**Script**: `computations/s51_local_resonance.py` | **Data**: `computations/s51_local_resonance.npz`
...
| File | Description |
|:-----|:-----------|
| `computations/s51_gauge_u1k7.py` | Computation script (3 levels) |
| `computations/s51_gauge_u1k7.npz` | All numerical results |
...
- Script: `computations/s51_cutoff_conv.py`
- Data: `computations/s51_cutoff_conv.npz`
- Plot: `computations/s51_cutoff_conv.png`
...
**Files**: `computations/s51_critical_scaling.py`, `computations/s51_critical_scaling.npz`, `computations/s51_critical_scaling.png`
...
| File | Description |
|:-----|:-----------|
| `computations/s51_crossover_sound.py` | Computation script |
| `computations/s51_crossover_sound.npz` | All numerical results |
...
| File | Description |
|:-----|:-----------|
| `computations/s51_sa_goldstone_mixing.py` | Computation script |
| `computations/s51_sa_goldstone_mixing.npz` | All numerical results |
| `computations/s51_sa_goldstone_mixing.png` | 6-panel diagnostic plot |
...
| File | Description |
|:-----|:-----------|
| `computations/s51_strutinsky.py` | Computation script (Strutinsky + TF + chi_SA) |
| `computations/s51_strutinsky.npz` | All numerical results (S_exact, S_smooth, delta_S, n_s, chi_SA, plateau data) |
| `computations/s51_strutinsky.png` | 9-panel diagnostic plot (S vs Lambda, plateau condition, level density, n_s vs Lambda) |
...
- **(A) Exact computation**: Dirac operator D_π assembled and diagonalized for all irreps (p,q) with p+q ≤ N, at the fold τ = 0.19. Infrastructure from `dirac_spectrum.py` (Session 12). SU(2)/S³ benchmark PASS at machine epsilon (8.88e-16).
...
- Script: `computations/s51_high_pw.py`
- Data: `computations/s51_high_pw.npz` (6.0 MB — exact eigenvalues at N=6 and N=8, scaling parameters, extended spectrum at N=20)
- Plot: `computations/s51_high_pw.png`
...
| File | Gate |
|:-----|:-----|
| `computations/s51_polariton.py` | POLARITON-51 |
| `computations/s51_polariton.npz` | POLARITON-51 |
...
|:-----|:---------------|:------------|:----------|:-------|
| 2026-03-20 | GAUGE-U1K7-51 (Anderson-Higgs) | OPEN | **PROMOTED** | Anderson-Higgs permanently closed: [iK_7, D_K] = 0 prevents U(1)_7 gauging at ALL orders. K_7 is a Kosmann derivative (diffeomorphism), not an inner automorphism (gauge). Three independent proofs. The sole surviving Goldstone theorem loophole is sealed. |
| 2026-03-20 | POLARITON-51 (polariton coupling) | OPEN | **CLOSED** | Polariton coupling too weak: Mass asymmetry 39x makes avoided crossing invisibly narrow. Max |alpha_eff - 2| = 0.0038 (need 0.1). 26x short. |
| 2026-03-20 | LOCAL-RESONANCE-51 (local resonance Re(Sigma)) | OPEN | **PROMOTED** | Local resonance closed: Zero-mode protection extends to Re(Sigma) via full Born series. Ward identity forces Sigma(0,0) = 0. Best surviving mechanism gives m_eff = 2.45 M_KK (5x short). |
| 2026-03-20 | CRITICAL-SCALING-51 (critical scaling hypothesis) | OPEN | **CLOSED** | Critical scaling closed: 170x is structural, not critical. Fold is anti-critical (m_L maximal there). eta = 0.036 contributes only 1.8% of tilt. |
| 2026-03-20 | STRUTINSKY-51 (Strutinsky smooth-part n_s) | OPEN | **CLOSED** | Strutinsky FAIL: Smooth spectral action doesn't independently select n_s. Shell correction 49% of susceptibility. |
| 2026-03-20 | HIGH-PW-51 (KK tower n_s > 1 structural theorem) | OPEN | **PROMOTED** | n_s > 1 structural for KK tower: Spectral geometer proved the bare Dirac heat kernel on any compact manifold gives n_s >= 1. Red tilt REQUIRES 4D dynamics. |
| 2026-03-20 | Convex combination theorem (additive correlator mixing) | OPEN | **PROMOTED** | Convex combination theorem for additive correlator mixing (structural). |
| 2026-03-20 | SA correlator alpha_eff = 0.86 (cutoff robustness) | OPEN | **PROMOTED** | SA correlator alpha_eff = 0.86 qualitatively robust across cutoffs (structural). |
| 2026-03-20 | Eigenvalue scaling max|lambda| = 0.633*sqrt(C_2) + 0.555 | OPEN | **PROMOTED** | Eigenvalue scaling: max|lambda| = 0.633*sqrt(C_2) + 0.555 on Jensen SU(3) (numerical). |
| 2026-03-20 | K* = m_G/sqrt(J) = 0.087 M_KK (SA-Goldstone mixing threshold) | OPEN | **PROMOTED** | K* = m_G/sqrt(J) = 0.087 M_KK as the SA-Goldstone mixing threshold (derived). |
