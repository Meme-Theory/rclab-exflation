...
**Output**: `computations/s48_*.py` / `.npz` / `.png`
**Constants**: Import from `computations/canonical_constants.py`
...
**Data files**:
- Script: `computations/s48_goldstone_mass.py`
- Data: `computations/s48_goldstone_mass.npz`
- Plot: `computations/s48_goldstone_mass.png`
...
- Script: `computations/s48_npair_full.py`
- Data: `computations/s48_npair_full.npz`
...
| File | Content |
|:-----|:--------|
| `computations/s48_aniso_gap.py` | Computation script |
| `computations/s48_aniso_gap.npz` | All numerical results |
| `computations/s48_aniso_gap.png` | 4-panel diagnostic figure |
...
| File | Contents |
|:-----|:---------|
| `computations/s48_wilson_loop.py` | Computation script |
| `computations/s48_wilson_loop.npz` | All phases, multiplet info, per-sector data |
| `computations/s48_wilson_loop.png` | 9-panel diagnostic plot |
...
- All 28 sectional curvatures K(e_a, e_b) via Milnor formula (dirac_spectrum infrastructure, validated in r20a S20)
...
#### Data Files
- Script: `computations/s48_curv_gap_corr.py`
- Data: `computations/s48_curv_gap_corr.npz`
- Figure: `computations/s48_curv_gap_corr.png`
...
| File | Content |
|:-----|:--------|
| `computations/s48_aniso_oz.py` | Computation script |
| `computations/s48_aniso_oz.npz` | All numerical results (m scan, P(K), eigenvalues) |
| `computations/s48_aniso_oz.png` | 6-panel diagnostic figure |
...
Two-level structure: bulk cluster (branches 1-6, 27 modes in [0.322, 0.347], 7.8% spread) inherited from bi-invariant 1/3 branch; outlier pair (branches 7-8, 4 modes at 0.627 and 0.939) inherited from bi-invariant 3/4 branch.
...
| File | Content |
|:-----|:--------|
| `computations/s48_tt_lichnerowicz.py` | Computation script (6 modules) |
| `computations/s48_tt_lichnerowicz.npz` | All eigenvalues, sectors, per-tau data |
| `computations/s48_tt_lichnerowicz.png` | 4-panel figure |
...
| File | Content |
|:-----|:--------|
| `computations/s48_qtheory_goldstone.py` | Computation script (9 routes) |
| `computations/s48_qtheory_goldstone.npz` | All masses, couplings, gate verdict |
...
| File | Content |
|:-----|:--------|
| `computations/s48_chi_q_phase.py` | Computation script |
| `computations/s48_chi_q_phase.npz` | All susceptibilities, ratios, couplings |
...
- Script: `computations/s48_leggett_mode.py`
- Data: `computations/s48_leggett_mode.npz`
- Figure: `computations/s48_leggett_mode.png`
...
**Files**: `computations/s48_dissolution_berry.{py,npz,png}`
...
**Files**: `computations/s48_sakharov_gn.{py,npz,png}`
...
**Script**: `computations/s48_paasch_backlog.py`
**Data**: `computations/s48_paasch_backlog.npz`, `s48_paasch_backlog.png`
...
**Data files**: `computations/s48_berry_complete.{py,npz,png}`
...
- Script: `computations/s48_dmde_refine.py`
- Data: `computations/s48_dmde_refine.npz`
- Plot: `computations/s48_dmde_refine.png`
...
**Data Files**: `computations/s48_curv_extend.{py,npz,png}`
...
**Data files**: `computations/s48_volovik_string.{py,npz,png}`
...
**Data files**: `computations/s48_qa_tachyon.{py,npz,png}`, `computations/s48_eigenvectors.{py,npz,png}`
...
| File | Content | Gate |
|:-----|:--------|:-----|
| `computations/s48_goldstone_mass.py` / `.npz` / `.png` | Goldstone mass from spectral action | GOLDSTONE-MASS-48 |
| `computations/s48_npair_full.py` / `.npz` | Physical pair number from 16-mode ED | N-PAIR-FULL-48 |
| `computations/s48_aniso_gap.py` / `.npz` / `.png` | k-dependent gap from rho_s anisotropy | ANISO-GAP-48 |
| `computations/s48_wilson_loop.py` / `.npz` / `.png` | Non-Abelian Wilson loop Berry phase | WILSON-LOOP-48 |
| `computations/s48_curv_gap_corr.py` / `.npz` / `.png` | Curvature-gap correlation across tau | CURV-GAP-CORR-48 |
| `computations/s48_aniso_oz.py` / `.npz` / `.png` | Anisotropic O-Z power spectrum | ANISO-OZ-48 |
| `computations/s48_tt_lichnerowicz.py` / `.npz` / `.png` | TT Lichnerowicz operator spectrum | TT-LICH-48 |
| `computations/s48_qtheory_goldstone.py` / `.npz` | Q-theory Goldstone mass | Q-THEORY-GOLD-48 |
| `computations/s48_chi_q_phase.py` / `.npz` | Q-theory phase susceptibility | CHI-Q-PHASE-48 |
| `computations/s48_leggett_mode.py` / `.npz` | Leggett mode frequency | LEGGETT-MODE-48 |
| `computations/s48_dissolution_berry.py` / `.npz` / `.png` | Dissolution topology test | DISSOLUTION-48 |
| `computations/s48_sakharov_gn.py` / `.npz` / `.png` | Sakharov G_N computation | SAKHAROV-GN-48 |
| `computations/s48_qa_tachyon.py` / `.npz` / `.png` | QA/Tachyon batch (7 sub-items) | QA-TACHYON-48 |
| `computations/s48_eigenvectors.py` / `.npz` / `.png` | Dirac eigenvector analysis at fold | EIGENVECTOR-48 |
| `computations/s48_paasch_backlog.py` / `.npz` / `.png` | Paasch 7-item backlog | PAASCH-BACKLOG-48 |
| `computations/s48_berry_complete.py` / `.npz` / `.png` | Berry 5-item completion | BERRY-COMPLETE-48 |
| `computations/s48_dmde_refine.py` / `.npz` / `.png` | DM/DE 3-item refinement | DMDE-REFINE-48 |
| `computations/s48_curv_extend.py` / `.npz` / `.png` | Curvature 6-item extensions | CURV-EXTEND-48 |
| `computations/s48_hfb_selfconsist.py` / `.npz` | HFB self-consistent gaps | HFB-SELFCONSIST-48 |
| `computations/s48_nilsson.py` / `.npz` / `.png` | Curvature Nilsson diagram | NILSSON-48 |
| `computations/s48_pbcs_0d.py` / `.npz` | PBCS vs BCS in 0D limit | PBCS-0D-48 |
| `computations/s48_pair_rotation.py` / `.npz` | Pair coupling along geodesics | PAIR-ROTATION-48 |
| `computations/s48_protected_chain.py` / `.npz` | q_7^2=1/16 PW survival | PROTECTED-CHAIN-48 |
| `computations/s48_volovik_string.py` / `.npz` / `.png` | Volovik+String 8-item batch | VOLOVIK-STRING-48 |
...
|:-----|:---------------|:------------|:----------|:-------|
| S48 | Goldstone mass from spectral action / GOLDSTONE-MASS-48 | OPEN | **CLOSED** | Trace theorem: S[UDU†]=S[D] for any U,D,f |
| S48 | Singlet CC crossing (N≥2) / N-PAIR-FULL-48 | OPEN | **CLOSED** | N=1.000 exact, 8 modes is complete singlet |
| S48 | k-dependent gap from rho_s anisotropy / ANISO-GAP-48 | OPEN | **CLOSED** | n_s=-2.930, 927σ. 12th single-cell route |
| S48 | Q-theory equilibrium Goldstone mass / Q-THEORY-GOLD-48 | OPEN | **CLOSED** | Self-tuning runaway, no finite fixed point |
| S48 | Goldstone mass (equilibrium, any) | OPEN | **CLOSED** | Both routes + Naz GMOR review. 56 OOM hierarchy impossible from microscopic BCS |
| S48 | Zak phase topological protection / DISSOLUTION-48 | OPEN | **CLOSED** | Artifact of index permutation. 0/10 survive at eps=1e-4 |
| S48 | Sakharov G_N from curvature anatomy / SAKHAROV-GN-48 | OPEN | **CLOSED** | 6 milli-OOM, 60x short. a_0 species-counting dominates |
| S48 | KZ defect correlation n_s / DEFECT-CORR-48 | OPEN | **CLOSED** | n_s=0.917 at d_eff=8. Need d_eff=19. Permanently closed |
| S48 | Golden ratio in (2,2)/(0,0) / PHI-GOLDEN-22 | OPEN | **CLOSED** | Ratio=1.680, 3.8% from 1.618 |
| S48 | fN centroid in pair-transfer / FN-CENTROID-47 | OPEN | **CLOSED** | Closest=1.194, 3.4% from 1.236 |
| S48 | Gibbs-Duhem Euler relation for GGE / GIBBS-DUHEM-GGE-48 | OPEN | **CLOSED** | Gives negative P. Multi-T Euler FAILS |
| S48 | n_s from single-cell pair creation | OPEN (11 routes closed in S47) | **CLOSED** | ANISO-GAP-48 closes 12th route |
| S48 | Non-Abelian Berry L2 / WILSON-LOOP-48 | UNCOMPUTED | **REGISTERED** | Uniform phases, KS p=0.52; 10 strict Abelian pi-phases |
| S48 | Leggett collective mode / LEGGETT-MODE-48 | UNCOMPUTED | **REGISTERED** | omega_L=0.070, sharp, below pair-breaking |
| S48 | TT Lichnerowicz stability / TT-LICH-48 | S20b PASS (different method) | **REGISTERED** | Independent Peter-Weyl method, 31 modes, all positive |
| S48 | HFB self-consistency / HFB-SELFCONSIST-48 | UNCOMPUTED (Naz Priority 1) | **REGISTERED** | All 12 configs converge, max 32 iter |
| S48 | Swampland de Sitter conjecture / SWAMPLAND-48 | UNTESTED | **REGISTERED** | c=52.8>>O(1), structural permanent result |
| S48 | Analog horizons (internal) / AKAMA-DIAKONOV-48 | UNCOMPUTED | **REGISTERED** | Mach 54.3, genuine horizons confining quasiparticles |
| S48 | Geometric phase transition | UNKNOWN | **PINNED** | tau=0.537, negative curvature onset |
| S48 | Z-K DM/DE discrepancy / GIBBS-DUHEM-GGE-48 | 39.4% (S46) | **PINNED** | Definitional, not convergible |
| S48 | w_0 vs DESI DR2 / DESI-UPDATED-48 | S45: -0.709 | **PINNED** | [-0.465, -0.589] at corrected alpha. 2.8σ from DR2 |
| S48 | Spectral action trace theorem | OPEN | **REGISTERED** | S[U D U†] = S[D] for ANY Hermitian D, ANY cutoff f, ANY unitary U. Permanent structural result |
| S48 | Singlet sector completeness | OPEN | **REGISTERED** | (0,0) singlet has exactly 16 eigenvalues = 8 Kramers pairs. S36 "8-mode truncation" was already the full singlet sector |
| S48 | Q-theory self-tuning fixed point | OPEN | **REGISTERED** | No finite fixed point. The mass problem IS the CC problem |
| S48 | TT Lichnerowicz transversality theorem / TT-LICH-48 | OPEN | **REGISTERED** | 31 TT modes (not 36); dim jumps 35→31 at tau=0+; lambda_min local maximum at fold |
| S48 | Curvature-gap anti-correlation / CURV-GAP-CORR-48 | OPEN | **PINNED** | r_mean = -0.904 across 26 tau values (all > 0.89); structural but marginally below theorem-grade threshold |
| S48 | N3 = dim(3,0) = T_4 = 10 / N3-DIM-48 | OPEN | **REGISTERED** | NCG cutoff at p+q ≤ N selects exactly dim(N,0) = T_{N+1} sectors. Exact algebraic identity. Sole surviving Paasch-NCG bridge |
| S48 | KZ-anisotropy cross-check / KZ-ANISO-48 | OPEN | **PINNED** | Curvature-weighted KZ defect density n_geom = 63.7, matching S38 BdG quench n_pairs = 59.8 to 6.5% |
| S48 | B2 eigenvector weights / EIGENVECTOR-48 | OPEN | **REGISTERED** | Exactly {3/8, 1/8, 4/8}; equal to dimension ratios of {su(2), u(1), C^2} decomposition |
| S48 | Z_3 domain wall tension / Z3-WALL-48 | OPEN | **PINNED** | sigma = 4.50 M_KK, K(U1-C^2) = 0.0625 CONSTANT at all tau. Z_3 is a rigid topological feature |
| S48 | Charge quantization Q_7 / PROTECTED-CHAIN-48 | OPEN | **REGISTERED** | Operator trace identity Tr(K_7^2)/16 = 1/8 exact; <Q_7>=0 exact in ED. Topologically protected |
| S48 | MASS-SOURCE-48 master gate | OPEN | **CLOSED** | Both spectral action (trace theorem) and q-theory (runaway) give m=0. No equilibrium mass |
| S48 | Berry topology completion / BERRY-COMPLETE-48 | OPEN | **PINNED** | max\|gamma_closed\|=1e-14, R(p,q)=1.0, conj=gauge, 5 pi dynamical, 12/13 VH no enhancement |
