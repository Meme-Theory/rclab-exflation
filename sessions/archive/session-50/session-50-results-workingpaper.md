...
**Files:**
- Script: `computations/s50_leggett_propagator.py`
- Data: `computations/s50_leggett_propagator.npz`
- Plot: `computations/s50_leggett_propagator.png`
...
**Data files**:
- Script: `computations/s50_jpair_calibrate.py`
- Data: `computations/s50_jpair_calibrate.npz`
...
**Files**: `computations/s50_leggett_damping.py`, `s50_leggett_damping.npz`, `s50_leggett_damping.png`
...
**Files**:
- Script: `computations/s50_leggett_phi_confirm.py`
- Data: `computations/s50_leggett_phi_confirm.npz`
- Plot: `computations/s50_leggett_phi_confirm.png`
...
**Script**: `computations/s50_running_mass.py`
**Data**: `computations/s50_running_mass.npz`
**Plot**: `computations/s50_running_mass.png`
...
**Script**: `computations/s50_lorentzian_cmpp.py`
**Data**: `computations/s50_lorentzian_cmpp.npz`
**Plot**: `computations/s50_lorentzian_cmpp.png`
...
**Data files**: `computations/s50_eikonal_damping.py`, `computations/s50_eikonal_damping.npz`, `computations/s50_eikonal_damping.png`
...
**Files:**
- Script: `computations/s50_desi_dr3_joint.py`
- Data: `computations/s50_desi_dr3_joint.npz`
- Plot: `computations/s50_desi_dr3_joint.png`
...
**Files**: `computations/s50_wa_source.py`, `s50_wa_source.npz`, `s50_wa_source.png`
...
**Data files**: `computations/s50_kz_spatial.py`, `computations/s50_kz_spatial.npz`, `computations/s50_kz_spatial.png`
...
**Files:**
- Script: `computations/s50_sigma8_oz.py`
- Data: `computations/s50_sigma8_oz.npz`
- Plot: `computations/s50_sigma8_oz.png`
...
**Data files**: `computations/s50_fabric_rpa.py`, `computations/s50_fabric_rpa.npz`, `computations/s50_fabric_rpa.png`
...
**Data files**: `computations/s50_anomalous_dispersion.py`, `computations/s50_anomalous_dispersion.npz`, `computations/s50_anomalous_dispersion.png`
...
**Method**: Research corpus cross-correlation + direct computation on existing computation data. Not delegated to subagents — leverages cross-domain connections that individual specialists miss.
...
- **Input data**: Existing computation Dirac computation scripts (s12_*, s44_*), canonical_constants.py
...
- **Input data**: Dirac eigenvalues and eigenfunctions from existing computation data, tessellation geometry from S47
...
- **Input data**: Dirac eigenvalues from existing computation data (already loaded in s50_crossdomain_routes.py)
...
|:-----|:---------------|:------------|:----------|:-------|
| S50 | 3-pole Leggett propagator (LEGGETT-PROPAGATOR-50) | OPEN | **CLOSED** | Poles 99.95% degenerate (mass hierarchy σ/m² = 5e-4); identity α_s = n_s² - 1 survives at 8.4σ from Planck. |
| S50 | Bogoliubov imprint of Leggett mass (BOGOLIUBOV-IMPRINT-50) | OPEN | **CLOSED** | Trans-Planckian erasure (ω_L/ω_transit = 10⁻⁵); Bogoliubov imprint channel closed, Leggett mass cannot be frozen into post-transit power spectrum. |
| S50 | Running mass in single-pole (RUNNING-MASS-50) | OPEN | **CLOSED** | Structural theorem γ < 1-n_s = 0.035; gate threshold γ > 1.7 exceeds the algebraic bound by a factor of 49. |
| S50 | Eikonal texture damping (EIKONAL-DAMPING-50) | OPEN | **CLOSED** | Zero-mode protection: Goldstone is KK n=0, ⟨V⟩ = 0 on T²; Γ(K_pivot) / m_G² = 0 exact. |
| S50 | Anomalous dispersion from Z₃ (ANOMALOUS-DISPERSION-50) | OPEN | **CLOSED** | Goldstone theorem (K² structural for broken U(1)); Z₃ softens coupling (m² renorm) but dispersion stays K². |
| S50 | Fabric RPA vertex correction (FABRIC-RPA-50) | OPEN | **CLOSED** | χ₀(K) 0.3% variation; mass hierarchy Π·D₀ = 4e-4; nuclear analogy breaks because m² >> J K² in denominator. |
| S50 | Spatial KZ pair creation (KZ-SPATIAL-50) | OPEN | **CLOSED** | Sudden-quench universality (δn/n = 10⁻⁴); 13th n_s route CLOSED, all cells identical to 0.059%. |
| S50 | w_a from inter-cell coupling (W_A-SOURCE-50, mechanism a) | OPEN | **CLOSED** | No condensate post-transit (J_ij = 0); P_exc = 1.0 destroys phase coherence. |
| S50 | w_a from GGE thermalization (W_A-SOURCE-50, mechanism b) | OPEN | **CLOSED** | Wrong direction or no thermalization; FGR gives instant relaxation to w_0 = -0.323 AWAY from DESI, or PAGE-40 averaging gives constant Π. |
| S50 | w_a from modulus evolution (W_A-SOURCE-50, mechanism c) | OPEN | **CLOSED** | Frozen modulus dichotomy: either Δτ = 0 (w_a = 0) or τ runs away (framework excluded). |
| S50 | w_a from viscous pressure (W_A-SOURCE-50, mechanism d) | OPEN | **CLOSED** | Π = constant (integrability); no cosmological-timescale relaxation. |
| S50 | R-G integral spatial variation (Cross-domain Route 2) | OPEN | **CLOSED** | Factorization: δI = (dI/dτ)·δτ, K² from τ; conserved integrals carry no independent spatial structure. |
| S50 | Non-equilibrium FDT breaking (Cross-domain Route 3) | OPEN | **CLOSED** | High-T limit: T/ω = 2-8, < 0.001% variation across K. |
| S50 | Spectral dimension anomaly (Cross-domain Route 4) | OPEN | **CLOSED** | Classical lattice: d_s ≥ 0, no CDT-type flow; would need quantum geometry. |
| S50 | O-Z identity α_s = n_s²-1 | OPEN (S49 wayforward) | **PROMOTED** to STRUCTURAL THEOREM (5 proofs) | Five independent proofs (3-pole, running mass, eikonal, RPA, anomalous dispersion); cannot be broken within the phase sector. |
| S50 | Leggett damping | OPEN (S49 marginal concern) | **CLOSED** (Q = 670,000) | W1-D: E_qp >> ω_L; all pair-breaking channels energetically forbidden, 2·E_min = 1.800 M_KK is 25.9× above ω_L. |
| S50 | CMPP classification (LORENTZIAN-CMPP-50) | Type II (S49, Riemannian) | **PROMOTED** to Type D (Lorentzian, exact) | W1-G: signature artifact; static product M^{3,1} × (SU(3), g_Jensen) is EXACT CMPP Type D at all τ. |
| S50 | w_0 = -0.509 vs DESI (DESI-DR3-JOINT-50) | B = 20.9 (S49, preferred) | **CLOSED** (χ²/N = 23.2, excluded by raw BAO) | W2-D: S49 compared derived parameters; Δχ² = +241 vs LCDM, framework excluded by BAO distances at overwhelming confidence. |
| S50 | w_a = 0 | OPEN | **PINNED** as TRIPLE-LOCKED | W2-E: 4 mechanisms all give 0; trapping + integrability + frozen modulus, breaking any one excluded. |
| S50 | SA correlator | UNCOMPUTED | **REGISTERED** (Identity broken, dev 0.08, α_eff = 1.21) | Cross-domain Route 1; pole spread 110% (C₂ from 1.33 to 9.33), not protected by Goldstone's theorem. |
| S50 | Pair-transfer form factor | UNCOMPUTED | **REGISTERED** (Identity broken, sinc², n_s = 0.70) | Cross-domain Route 5; nuclear pair-transfer response G_pair(K) has sinc²(K·l_cell) form factor from cell shape. |
| S50 | Mass problem (170×) | Implicit in all S49 work | **PINNED** as binding constraint | Cross-domain synthesis; m_required/m_Leggett = 170, the 12 M_KK effective mass corresponds to the spectral edge of D_K at high Peter-Weyl truncation. |
| S50 | Acoustic Fano resonance for mass enhancement | OPEN (QA Section 3.3) | **CLOSED** | V_direct = 0 by zero-mode protection implies q = 0; symmetric Lorentzian dip, not asymmetric Fano lineshape. |
| S50 | Nuclear effective mass (m*/m) from Landau Fermi liquid | OPEN (Naz Section 3a) | **CLOSED** | Nuclear m*/m = 0.6-0.8 (O(1) multiplicative factor); the 170x enhancement is qualitatively different — a scale hierarchy, not a correction factor. |
| S50 | Particle-vibration coupling mass renormalization | OPEN (Naz Section 3c) | **CLOSED** | δ_m ~ g_GPV²/ω_GPV = 0.031 M_KK; shifts Goldstone mass from 0.070 to ~0.10 M_KK, still 120x below required 11.85. |
| S50 | Disordered superfluid Bose glass loophole | OPEN (Landau Section 1, loophole 3) | **CLOSED** | Z_3 disorder is periodic, not random; even true random disorder on 32 sites would not produce Anderson localization of the Goldstone. |
| S50 | Frustrated magnet Type B_2 anomalous Goldstone | OPEN (Landau Section 1, loophole 4) | **CLOSED** | U(1)_7 breaking has one broken generator, one Goldstone with α = 2; Chadha-Nielsen counting saturated, no frustration mechanism. |
| S50 | Plasmon/long-range Coulomb loophole | OPEN (Landau Section 1, loophole 1) | **CLOSED** | U(1)_7 is a global symmetry with short-range Josephson coupling; no gauge field, no divergent V(q). Closed unless U(1)_7 is gauged. |
| S50 | Volovik dilution model (w_a from quasiparticle dilution) | OPEN (Volovik Section 3) | **CLOSED** | Quasiparticles are fiber-localized, not 4D spatial; their density is a fiber quantity, triple-lock on w_a = 0 confirmed by frozen-modulus dichotomy. |
