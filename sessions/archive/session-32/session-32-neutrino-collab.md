...
*Review completed by the Neutrino Detection Specialist, 2026-03-03. Experimental values sourced from the 12-paper reference corpus in `/researchers/Neutrino-Detection/` (Papers 01-12, Pauli 1930 through KATRIN 2024). Global fit parameters from NuFIT 5.3. PMNS fine-grid data from `computations/s32c_pmns_fine_grid.py` (run during this review). All constraint references indexed in `.claude/agent-memory/constraint-map.md`. Gate registry at `.claude/agent-memory/neutrino-detection-specialist/gate-registry.md`.*
