...
However, the mechanism chain I-1 -> RPA -> Turing -> WALL -> BCS now provides a concrete scenario for HOW tau gets frozen, which feeds into the CC computation (Hawking Option (c), deferred). If the BCS condensation at walls eventually determines Lambda through the sector sum (Level 3 prediction, L-8), then Session 32 is an essential step toward that extragalactic observable.
...
But as I noted in Session 28: w = -1 is a Level 4 null prediction. LCDM also predicts w = -1. No discriminating power.
