...
**Specific computation**: Re-run the BdG gap equation from `computations/s23a_gap_equation.py` replacing the bulk DOS with the wall-localized DOS from `s32b_wall_dos.npz`. The input V matrix should be re-projected into the rotated eigenvector basis at the wall (the eigenvector overlap matrix from W-32b provides the rotation). Gate: M_max > 1.0.
