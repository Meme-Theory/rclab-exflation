...
**kk1_bosonic_spectrum.npz multiplicity convention**: This pre-computed file stores Peter-Weyl multiplicities as dim(p,q)^2, while the standard convention used in all computational scripts (collect_scalar_spectrum, dirac_spectrum) is dim(p,q).
...
| File | Description |
|:-----|:-----------|
| `computations/d19d_casimir_gate.py` | D-1 gate computation: boson/fermion E_proxy separation |
| `computations/d19d_casimir_gate.png` | 6-panel plot: E_proxy components, R(tau), dR/dtau, E_total, gradient comparison, scalar/vector breakdown |
| `computations/d19d_casimir_gate.npz` | Numerical results: tau_values, boson/fermion E_proxy, R_ratio, gradients, verdict |
