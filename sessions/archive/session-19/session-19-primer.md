...
This is ALREADY implemented in spectral_action.py (function
...
The following pieces from the computations/ codebase are directly usable:

| Component | File | Function | What it provides |
|:----------|:-----|:---------|:----------------|
| Dirac spectrum | dirac_spectrum.py | collect_spectrum() | Eigenvalues at any tau, sector-labeled |
| Jensen metric | dirac_spectrum.py | jensen_metric() | g_K(tau) construction (eq 3.68-3.71) |
| Spectral action | spectral_action.py | spectral_action_smooth_cutoff() | S = Tr(f(D^2/Lambda^2)) |
| V_tree | spectral_action.py | baptista_V_tree() | Classical potential eq 3.80 |
| Gauge boson mass | spectral_action.py | baptista_m2() | m^2(tau) from eq 3.84 |
| s-sweep | dirac_spectrum.py | sweep_s() | Eigenvalues at 21 tau-values |
...
| Irrep construction | dirac_spectrum.py | get_irrep_matrices() | Peter-Weyl decomposition |
...
Computational infrastructure: dirac_spectrum.py (collect_spectrum,
jensen_metric, sweep_s), spectral_action.py (spectral_action_smooth_cutoff,
