...
The Riemann tensor R_{abcd}(s) of the Jensen-deformed SU(3) is computable from the structure constants and the metric. The connection coefficients Gamma^b_{ac}(s) are already computed in `dirac_spectrum.py` (function `connection_coefficients`). The Riemann tensor follows from:
