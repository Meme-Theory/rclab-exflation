...
4. Items (a) and (b) are already computed in kk1_bosonic_tower.py and dirac_spectrum.py. Item (c) is new: the Riemann tensor on a Lie group with left-invariant metric. The formula is known (Milnor 1976):
...
**Input**: su3_generators, compute_structure_constants, jensen_metric, orthonormal_frame, connection_coefficients -- all already in dirac_spectrum.py.
