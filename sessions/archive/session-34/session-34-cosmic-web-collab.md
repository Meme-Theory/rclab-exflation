...
**Sole surviving extragalactic observable**: Lambda from sector cancellation (Level 3). Status: NOT COMPUTED. Requires 12D back-reaction and wall volume fraction (TURING-1). Unchanged.
...
The mechanism chain operates in internal geometry. Its extragalactic observable is Lambda (Level 3, uncomputed). The DESI w_0 = -1.016 +/- 0.035 (Paper 17, D17-E1) is consistent with the framework's w = -1 null prediction, but w = -1 has zero discriminating power against LCDM. The S8 tension (sigma_8 = 0.777 from DESI vs 0.811 from Planck) remains unexplained by both the framework and standard LCDM.
