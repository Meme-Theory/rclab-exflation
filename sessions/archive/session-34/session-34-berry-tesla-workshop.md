...
At N_max = 6 (the computational truncation used in all computation work), this gives N_B2 ~ 72 modes. The physical system is a compact manifold with no truncation -- N_max -> infinity. The infinite sum of independent B2 channels, each with N_eff = 4, gives a total N_eff that is either:
...
This is computable from existing data. The Kosmann matrices K_a in the spinor basis are already constructed in the computation code. Restricting to the B3 subspace and computing the Casimir gives the answer.
