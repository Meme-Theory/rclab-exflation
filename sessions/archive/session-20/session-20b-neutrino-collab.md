...
In my 19d review, I suggested computing the ratio (lambda_3^2 - lambda_2^2) / (lambda_2^2 - lambda_1^2) as a function of s from the existing Level 1 Dirac spectrum data. This computation is still valid and now MORE important, not less. Here is why:
...
The computation:
1. Load `computations/l20_TT_spectrum.npz` or the Level 1 Dirac eigenvalues at 21 tau values
...
2. Extract lightest D_K(s_0) eigenvalues -- Ready (Level 1 data exists)
3. Fix M_scale from a known fermion mass -- Ready (once s_0 known)
4. Compute PMNS angles from eigenspinor overlaps -- Level 2 (not yet started)
...
This question is answerable from existing Level 1 data. It requires tracking the three lightest eigenvalues individually as functions of tau, not just the overall spectral gap.
