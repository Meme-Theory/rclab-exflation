...
can now be evaluated for the TT eigenvalues. The derivative d(Delta_L)/d(tau) comes from the tau-derivative of the Riemann tensor (available from `computations/r20a_riemann_tensor.npz` at 21 tau-values -- finite differences suffice). The eigenstates are needed, requiring re-running the Lichnerowicz diagonalization with eigenvector output.
...
| File | Description |
|:-----|:-----------|
| `computations/l20_TT_spectrum.npz` | TT Lichnerowicz eigenvalues per sector, 21 tau-values |
| `computations/l20_vtotal_minimum.npz` | Full V_total data at 21 tau-values |
| `computations/r20a_riemann_tensor.npz` | R_{abcd}(tau) at 21 tau-values, shape (21,8,8,8,8) |
