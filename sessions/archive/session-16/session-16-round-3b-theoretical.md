...
**Converged test counts**: 11 Level 1 tests (executable from current infrastructure), 9 deferred to Level 2 (8 require spinor transport or inter-generation identification; 1 requires higher irreps for convergence).
...
- **LEFT Z_3 = (p-q) mod 3**: Commutes with D_K. Labels conserved quantum numbers. Does NOT create mass splitting. Visible in Level 1 eigenvalues.
- **RIGHT Z_3**: Does NOT commute with D_K at s>0. THIS is the generation mechanism. Creates mass splitting within each dim(p,q)-fold degeneracy. Invisible in Level 1 D_{(p,q)} eigenvalues. Requires spinor transport computation (~1 week, ~145 lines new code).
- Paper 18 lines 2560-2567: Inter-generation splitting occurs at "perturbation scale" (electroweak), NOT Jensen/Planck scale.

Consequence: Table B (inter-generation tests B3-B6) is **entirely deferred to Level 2**. Only structural tests B1-B2 and a new coarse B3 (Jensen-level splitting bound) remain at Level 1.
...
- Type: Test
- Specification:
  **Step 1 (TRIVIAL)**: For each (p,q) sector in the Level 1 eigenvalue catalog, assign generation = (p-q) mod 3. Three sectors: Gen 0 [(p-q)=0 mod 3: (0,0), (1,1), (3,0), (0,3), (2,2), ...], Gen 1 [(p-q)=1 mod 3: (1,0), (0,2), (2,1), ...], Gen 2 [(p-q)=2 mod 3: (0,1), (2,0), (1,2), ...].

  **IMPORTANT**: This assigns LEFT Z_3, which labels conserved quantum numbers but does NOT produce inter-generation mass splitting. The physical generation mechanism requires RIGHT Z_3 (Level 2, spinor transport).

  **Step 2 (MEDIUM)**: Modify `collect_spectrum()` in `dirac_spectrum.py` to use `np.linalg.eigh(1j * D_pi)` instead of `np.linalg.eigvals(D_pi)`, storing eigenvectors. Physical masses = |evals_h|. Net speedup expected (eigh ~3x faster than eigvals for Hermitian).
...
- Pass/Fail:
  ...
  **Level 1 Tests** (from LEFT Z_3 + U(2) quantum numbers):
  ...
  **Level 2 Tests** (deferred, require RIGHT Z_3 spinor transport):

  | Criterion | Condition | Level 2 requirement |
  |-----------|-----------|-------------------|
  ...
  **Consensus priors** (Round 2c, revised): B1-B2: ~80%. B3: ~75% (LEFT splitting SHOULD be O(1)). B4 (Level 2): ~45%. B6 (Level 2): ~12%. B7 (Level 2): ~12%.

- Feasibility: GREEN (Level 1); YELLOW (Level 2, ~1 week additional for spinor transport)
- Timeline: ~2 days Level 1 (Phase 1: eigenvectors 4h, Phase 2: U(2) generators 4h, Phase 3: Z_3+analysis 4h, Phase 4: multi-s evaluation 2h). ~1 additional week for Level 2 (~145 lines new code for spinor transport).
- Owner: sim-specialist (code) + baptista-analyst (D_K validation) + qa-theorist (phonon interpretation)
- Decisiveness: 8/10 Level 1 (parameter-free, representation-theoretic, independent of V_eff). 9/10 Level 2 (generation hierarchy = SM litmus test).
- Dependencies:
  - `dirac_spectrum.py` (existing, all infrastructure)
  ...
  - Level 2: Baptista Paper 18 spinor transport formalism
- Code:
  - MODIFY: `computations/dirac_spectrum.py` (~20 lines: eigh instead of eigvals)
  - NEW: `computations/level1_u2_projection.py` (~315 lines: U(2) generators, quantum number assignment, Z_3 labeling, generation analysis, phi test)
  - REUSE: `computations/branching_computation.py` (U(2) spinor embedding extraction)
  - NEW (Level 2): `computations/level2_spinor_transport.py` (~145 lines: RIGHT Z_3 eigenvectors, inter-generation splitting)
...
- Code:
  - NEW: `computations/g_decomposition.py` (~200 lines: SM constant formulas, evaluation at s_0, comparison table)
  - REUSE: `dirac_spectrum.py` (eigenvalues at any s), `spectral_action.py` (V_eff)
...
- Specification:
  ...
  **Table A: Inter-Species Mass Ratios at s_0** (Level 1, 5 tests)
  ...
  **Priority 1** (from LEFT Z_3, Items B1-B3 in Item 1 above):
  ...
  **Priority 2** (5 tests, deferred):

  | # | Test | Reason deferred | Level 2 requirement |
  |---|------|-----------------|--------------------|
  ...
  **NOTE on C6**: m_tau/m_p = 1.8938 (0.089% from phi^{3/2} = 1.8954), which is 3x closer than p/K (0.271%). This is remarkably suggestive but is an INTER-generation test (tau = gen 2, proton = gen 0), so it must wait for RIGHT Z_3 identification at Level 2.
  ...
  **Combined Decision Matrix** (Round 2c, endorsed by Giants, UPDATED):
  ...
  |---------|-------------|-----------------|---------------|
  | FULL SUCCESS | A(4+), B-Level2(B6+B7), C(C1 or C2 or C5) | +15-20% -> 65-80% | "Kepler finds Newton" |
  | PARTIAL SUCCESS | A(2-3), B-Level1(B1-B3), C(none) | +5-10% -> 55-70% | Z_3 works, mass scaling approximate |
  | MIXED | A(0-1), B-Level1(B1-B2), C(C5 only) | +/- 0 -> 50-62% | Phi real but mass numbers wrong |
  | NULL | A(0), B-Level1(B1-B2), C(none) | -10% -> 40-52% | Paasch orphaned |
  ...
  **Converged Test Count Summary**:
  - **Priority 1**: A(5) + B(3: B1-B3) + C(3: C1,C2,C5) = **11 tests**
  - **Priority 2**: B(4: B4-B7) + C(5: C3,C4,C6,C7,C8) = **9 deferred** (but 8 require new infrastructure; C3 requires only higher irreps)
  - **Paasch-internal**: D(4) = **4 tests** (separate section, independent of D_K)

- Pass/Fail: See individual table criteria above.
- Feasibility: GREEN (all Level 1 tests run from Z_3 + U(2) output)
- Timeline: Hours (after Item 1 Z_3 test complete) for Level 1. ~1 week additional for Level 2.
- Owner: paasch-analyst (table design, N-value analysis) + sim-specialist (numerical evaluation) + qa-theorist (phonon interpretation)
- Decisiveness: Table C5: 8/10 (pre-registered, untested, phi^{3/2} target). Table A: 6/10 (can refute). Table B Level 1: 5/10 (structural). Table B Level 2: 8/10 (generation hierarchy). Table D: 4/10 (internal).
- Dependencies:
  ...
  - Level 1 eigenvalues at target s: already available from `dirac_spectrum.py`
  - Level 2: RIGHT Z_3 spinor transport (Item 1, Level 2 code)
- Code:
  - NEW: `computations/paasch_binding_tests.py` (~250 lines: N-value computation from eigenvalues, f_N chain test, phi^{3/2} test, mass ratio comparison, decision matrix evaluation)
  - REUSE: `level1_u2_projection.py` (Item 1, generation-grouped eigenvalue catalog)
...
- Code:
  - NEW: `computations/order_one_physical_dk.py` (~400 lines: D_F construction from eigenvectors, A_F basis rotation, order-one test for all 9 factor pairs, chirality verification)
  - REUSE: `branching_computation_32dim.py` (J, gamma_F), `branching_computation.py` (A_F generators), `level1_u2_projection.py` (eigenvectors + quantum numbers)
...
- Specification:
  ...
  **Complete Dictionary (17 entries)**
  ...
  | 15 | Z_3 generation mechanism | Zone-boundary mode tripling | 1 | C+ | -> 2/B: If Z_3 gives 3 generations with hierarchy (Item 1 B1-B3 Level 1 + B4-B7 Level 2). -> 3/A: If lepton/quark OOM match (Item 1 B6-B7). -> 0/N/A: If B1 fails. |
...
```
[EXISTING] dirac_spectrum.py + branching_computation*.py
    |
    v
[Item 1] Z_3 Test Procedure
    |  Level 1: 2 days        Level 2: +1 week (spinor transport)
    |         |         |
    v         v         v
...
**Critical path**: Item 1 Level 1 -> Item 5 -> Item 2 Phase D (Pfaffian). Everything else runs in parallel.
...
| Days 1-2 | Track A: V_eff (Round 3a) | Track B: Z_3 + U(2) (Item 1 Level 1) |
|----------|--------------------------|--------------------------------------|
| Day 3 | Gauge coupling test (Item 3) | Paasch tests Level 1 (Item 4) |
...
| Week 2 | Level 2 spinor transport (Item 1) | Level 2 generation tests (Item 4 Table B) |
...
No change from Round 2d: **38-60%** (all-Giant range). Items 1-6 above are the specifications that will MOVE this number. The coming week's computations are the most consequential since the Level 1 Dirac spectrum.
...
*Round 3b theoretical specifications FINAL. Six items, all with executable pass/fail criteria, explicit dependencies, and code specifications. 11 Level 1 tests + 8 Level 2 deferred + 4 Paasch-internal. The critical path runs through Z_3 Level 1 (Item 1) -> Order-one (Item 5) -> Pfaffian (Item 2 Phase D). The phonon-NCG dictionary tracks 17 entries at average grade B with zero contradictions, 4 fixable breaks, and 3 potentially fundamental breaks all jointly discriminated by the Bell computation. Key correction: phi^{3/2} = 1.8954 (verified).*
