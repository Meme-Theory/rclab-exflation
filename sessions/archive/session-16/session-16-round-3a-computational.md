...
The `collect_spectrum()` function in `dirac_spectrum.py` gets a new kwarg `store_eigenvectors=False`:
...
where:
  ...
  - `m_C2^2(s)` from `gauge_boson_masses_baptista(0, s)` [existing code, line 660 of spectral_action.py]
  - `lam_j^{(p,q)}(s)` from `collect_spectrum(s, ..., max_pq_sum=6)` [existing code, line 1245 of dirac_spectrum.py]
  - `dim(p,q)` = Peter-Weyl multiplicity [existing code, `dim_su3_irrep()` in spectral_action.py]
...
- **Code**:
  ...
  |------|----------|-------|---------|--------|
  | `computations/veff_full.py` (NEW) | `collect_spectrum_cached()` | ~40 | 8.7s first, 50ms cached | I/O wrapper |
  | `computations/veff_full.py` | `compute_veff_cw()` | ~70 | 10ms/point | Core formula |
  | `computations/veff_full.py` | `scalar_laplacian_on_irrep()` | ~15 | <1ms/sector | Bosonic tower (Version C) |
  | `computations/veff_full.py` | `veff_sweep()` | ~50 | 30 min first, 2 min cached | Production |
  | `computations/veff_full.py` | `convergence_test()` | ~35 | ~43 min (all pq levels) | Quality check |
  | `computations/veff_full.py` | `veff_diagnostics()` | ~60 | <1 min | Hawking thermo |
  | `computations/veff_full.py` | `high_T_veff()` | ~30 | <1 min | Cross-check |
  | `computations/run_veff.py` (NEW) | `main()` | ~100 | Orchestrates above | Driver |
...
- **Specification**: For each eigenvector of D_{(p,q)} on (SU(3), g_s), assign:
  ...
  **CRITICAL CODE CHANGE** -- modify `collect_spectrum()` (line 1245 of `dirac_spectrum.py`):
...
- **Code**:
  ...
  |------|----------|-------|--------|
  | `computations/dirac_spectrum.py` | Modified `collect_spectrum()` (eigh + evecs) | ~20 modified | CRITICAL |
  | `computations/level1_u2_projection.py` (NEW) | `u2_generators_in_irrep()` | ~50 | New |
  | `computations/level1_u2_projection.py` | `assign_quantum_numbers()` | ~40 | New |
  | `computations/level1_u2_projection.py` | `z3_generation_label()` | ~5 | New |
  | `computations/level1_u2_projection.py` | `analyze_generations()` | ~60 | New |
  | `computations/level1_u2_projection.py` | `test_phi_in_generations()` | ~30 | New |
  | `computations/level1_u2_projection.py` | `particle_type_map()` | ~20 | New |
  | `computations/level1_u2_projection.py` | `u2_spinor_embedding()` | ~40 | New |
  | `computations/level1_u2_projection.py` | `resolve_degenerate_subspaces()` | ~40 | New |
  | `computations/level1_u2_projection.py` | `main() / driver` | ~50 | New |
  | `computations/run_z3_u2.py` (NEW) | Driver script | ~80 | New |
...
- **Risk Factors**:
  ...
  | U(2) quantum numbers not cleanly quantized | MEDIUM | Round to nearest physical value; cross-validate vs computation branching |
...
- **Code**: ~50 lines added to `level1_u2_projection.py` analysis section.
...
- **Specification**: Assess the feasibility and level-of-effort for computing the MISSING bosonic KK tower eigenvalues, to address the DOF inversion (45:16 boson:fermion asymptotic ratio, Round 2d-i Section III).
  ...
  **Scalar Laplacian**: Peter-Weyl decomposition gives `Delta_0^{(p,q)}` as a `dim(p,q) x dim(p,q)` matrix (no spinor factor). The construction uses the same representation matrices `rho_{(p,q)}(e_a)` and metric infrastructure. Formula:
  ...
  ```
  where g^{ab}(s) is the inverse Jensen metric: g^{u(1)} = e^{-2s}/g_0, g^{su(2)} = e^{2s}/g_0, g^{C^2} = e^{-s}/g_0. The metric is block-diagonal (dirac_spectrum.py line 111-151), so g^{ab} is trivially computed. This is the quadratic Casimir for the deformed metric. ~30 lines new code. Eigenvalues are REAL and non-negative (Laplacian is self-adjoint, non-negative).
  ...
  **THIS IS A SCOPING STUDY ONLY.** The full computation is Level 2+.
...
- **Code**: ~150-200 lines new (D_F construction + Pfaffian computation)
  ...
  | File | Function | Lines | Status |
  |------|----------|-------|--------|
  | `computations/pfaffian_test.py` (NEW) | `construct_D_F_truncated()` | ~60 | New |
  | `computations/pfaffian_test.py` | `compute_pfaffian_sign()` | ~30 | New |
  | `computations/pfaffian_test.py` | `spectral_gap_check()` | ~20 | New |
  | `computations/pfaffian_test.py` | `pfaffian_sweep()` | ~40 | New |
  | `computations/pfaffian_test.py` | `main()` | ~50 | New |
  | **Total** | | **~200** | |
...
|-----|-----------------|--------------------|----|
| **Day 1** | Code veff_full.py. Test at pq=3. | Modify collect_spectrum() for eigh+evecs. Code U(2) generators. | -- |
...
- `veff_full.py`: V_eff Track A (matches existing `dirac_spectrum.py`, `spectral_action.py`)
- `level1_u2_projection.py`: Z_3+U(2) Track B
- `pfaffian_test.py`: Pfaffian Item #7
- `run_veff.py`, `run_z3_u2.py`: Driver scripts

### 3. Scalar Laplacian Placement
In `veff_full.py` (bosonic operator, conceptually part of V_eff, no eigenvector dependency).
