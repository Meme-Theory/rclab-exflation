...
| **9** | Z_3 Level 2 (spinor transport) | 3b #1 T2 | 9/10 | YELLOW | +1 week | ~145 lines | +5-10% / -5% |
...
| **5** | Z_3 generation hierarchy (Level 2) | 2 | Known SM values | Requires spinor transport |
...
```
   ...
   veff_full.py           level1_u2_projection.py
...
|:---:|:----------------|:---------------------|:------------|
| **1** | Code `veff_full.py`. Test at pq=3 (~4h). | Modify `collect_spectrum()` for eigh+evecs. Code U(2) generators (~4h). | -- |
...
**IMPORTANT**: LEFT Z_3 does NOT create inter-generation mass splitting (the physical O(200) hierarchy). That requires RIGHT Z_3 (Level 2, spinor transport, ~1 additional week).
...
| 6 | **Jensen metric + volume preservation** | u(1)->e^{2s}, su(2)->e^{-2s}, C^2->e^s. det(g_s)/det(g_0) = 1.0000000000. | `dirac_spectrum.py` |
| 7 | **Scalar curvature R(s)** | R(0) = +2.000000 (exact for bi-invariant SU(3)). R(s)/R(0) matches Baptista eq 3.70 at 5e-15. | `spectral_action.py` |
| 8 | **Gauge boson mass pattern** | C^2 bosons massive, u(2) bosons massless. Matches eq 3.84 exactly. | `spectral_action.py` |
| 9 | **Dirac spectrum pipeline (8 checks)** | Jacobi identity, metric compatibility, Omega anti-Hermiticity, Clifford algebra, SU(2) benchmark, torsion-free, bi-invariant quantization, volume preservation. ALL pass at < 10^{-10}. | `dirac_spectrum.py` |
...
- **LEFT Z_3 = (p-q) mod 3**: Commutes with D_K. Labels conserved quantum numbers (generation label). Does NOT create mass splitting. Visible in Level 1 eigenvalues.
- **RIGHT Z_3**: Does NOT commute with D_K at s > 0. THIS is the generation mechanism. Creates mass splitting within each dim(p,q)-fold degeneracy. Invisible in Level 1 D_{(p,q)} eigenvalues.

**Consequence**: Inter-species mass ratios (proton/kaon, etc.) are captured by Level 1 computation. Inter-generation hierarchy (m_tau/m_e ~ 3477) requires Level 2 spinor transport (~1 week, ~145 lines new code). Paper 18 lines 2560-2567: inter-generation splitting occurs at electroweak perturbation scale, NOT Jensen/Planck scale. This two-scale architecture is a structural prediction.
...
| **7** | Level 2 spinor transport for RIGHT Z_3 | Sim-specialist | 1 week | 3b #1 T2 |
...
**If Session 16 computations are NEGATIVE** (V_eff monotonic, gauge fail at backup s_0): Session 17 should focus on Seeds 5 and 7-8 (Pfaffian becomes ESSENTIAL, spinor transport for Level 2 generation tests, order-one as structural test). The perturbative route is closed; non-perturbative methods take priority.
...
## Existing Scripts (Sessions 7-14, in `computations/`)
...
| `dirac_spectrum.py` | ~1580 | Level 1: Dirac spectrum p+q <= 6 |
| `spectral_action.py` | ~900 | Spectral action + heat kernel |
| `phi_analysis.py` | ~500 | Phi structure analysis |
...
|:-----|:-----:|:-----:|:------:|
| `veff_full.py` (NEW) | ~400 | A (V_eff) | Specified |
| `run_veff.py` (NEW) | ~100 | A (V_eff) | Specified |
| `level1_u2_projection.py` (NEW) | ~375 | B (Z_3+U(2)) | Specified |
| `run_z3_u2.py` (NEW) | ~80 | B (Z_3+U(2)) | Specified |
| `pfaffian_test.py` (NEW) | ~200 | Integration | Specified |
| `dirac_spectrum.py` (MODIFIED) | ~25 mod | Both | Specified |
