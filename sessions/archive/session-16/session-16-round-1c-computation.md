...
|---|------------|-------------|----------|--------------|-------|-------|
| 1 | **Full spectral 1-loop V_eff** (CW + classical) | **GREEN** | ~1 day (code) + ~10 hr (runtime at pq<=6) | Level 1 eigenvalues (DONE) | **DECISIVE** | See V_eff spec below. NOT zero-parameter -- one free parameter (kappa or Lambda). |
| 2 | **U(2)-singlet projection** | **GREEN** | ~1 day | computation branching (DONE) + Level 1 eigenspinors | HIGH | Resolves "wrong test" criticism. Requires storing eigenvectors (currently discarded). |
...
- Script: `spectral_action.py` (`baptista_Veff()` already implemented)
- Sweep kappa in [0.1, 100], record s_0(kappa)
- At each s_0, compute mass ratios and gauge couplings from Level 1 eigenvalues
