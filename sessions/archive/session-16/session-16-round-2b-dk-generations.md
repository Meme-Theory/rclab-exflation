...
1. **D_K on SU(3) IS the correct mass operator** (Corollary 3.4). The Level 1 eigenvalues are physical. No operator change needed.
...
**This is EXACTLY what `dirac_spectrum.py` computes.** The Level 1 eigenvalues ARE D_K eigenvalues on SU(3). Confirmed by Paper 17 Corollary 3.4 and validated in Sessions 11-12.
...
**Why Lambda is NOT needed for the generation test**: The generation ASSIGNMENT is determined by the Peter-Weyl decomposition -- the Z_3 center acts on V_{(p,q)} by the scalar omega^{(p-q) mod 3}. This is a purely algebraic statement about the Z_3 character of the representation and does not require computing Phi. The generation MASSES are the D_K eigenvalues per (p,q) sector, already computed in Level 1.
...
These are Level 2 computations, not needed for the generation-level test.
...
### File: `computations/dirac_spectrum.py`
...
### New file: `computations/level1_u2_projection.py`

Contains all U(2) labeling and Z_3 analysis functions. Imports from `dirac_spectrum.py`.

### Summary of modifications

| Component | File | New/Modified | Lines | Difficulty |
|-----------|------|-------------|-------|------------|
| Store eigenvectors | dirac_spectrum.py | Modified | ~20 | EASY |
| U(2) generators in irrep | level1_u2_projection.py | New | ~50 | MEDIUM |
| Quantum number assignment | level1_u2_projection.py | New | ~40 | MEDIUM |
| Z_3 labeling | level1_u2_projection.py | New | ~5 | TRIVIAL |
| Generation analysis | level1_u2_projection.py | New | ~60 | MEDIUM |
| phi_paasch test in generations | level1_u2_projection.py | New | ~30 | EASY |
| U(2) spinor embedding | level1_u2_projection.py | New | ~40 | MEDIUM |
| Particle type mapping | level1_u2_projection.py | New | ~20 | EASY |
| Driver / main | level1_u2_projection.py | New | ~50 | EASY |
| **TOTAL** | | | **~315** | |
...
| Phase 2 | Quantum number assignment + validation vs computation branching | 4 hours |
...
```
[EXISTING] dirac_spectrum.py (eigenvalues + metric infrastructure)
    |
    v
...
| Spinor U(2) embedding wrong normalization | MEDIUM | Systematic shift in Y values | Cross-validate against computation branching results |
...
The Level 1 code uses the LEFT-regular representation rho_L for the Peter-Weyl decomposition. The U(2) quantum numbers that label 4D particles correspond to the gauge symmetry, which acts via the LEFT action (L_{e_a}).
...
**Resolution**: For the FIRST computation, use LEFT U(2) quantum numbers only (sufficient for particle identification). RIGHT U(2) projection is a Level 2 refinement.
...
4. **Code plan**: ~315 new lines in `level1_u2_projection.py` plus ~20 modified lines in `dirac_spectrum.py`.
...
2. **Implement Phase 2**: U(2) quantum number assignment, cross-validate with computation
