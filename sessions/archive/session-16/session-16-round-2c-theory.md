...
The finite-T correction is computable from the existing Level 1 eigenvalue catalog. No new code is needed beyond what's specified in Round 2a. The thermal sum converges rapidly because the heat kernel f(x) = exp(-x) provides exponential suppression of high-lying modes. At p+q <= 6, the truncation error in V_thermal is comparable to the truncation error in V_CW.
