...
| 6 | **Jensen metric + volume preservation** | u(1)->e^{2s}, su(2)->e^{-2s}, C^2->e^s. det = 1.000000 | `dirac_spectrum.py` |
| 7 | **Scalar curvature R(s)** | R(0) = +2.000, R(s)/R(0) matches eq 3.70 at 5e-15 | `spectral_action.py` |
| 8 | **Gauge boson mass pattern** | C^2 massive, u(2) massless. Matches eq 3.84 | `spectral_action.py` |
| 9 | **Pipeline validation** | 8 checks (Jacobi, metric, Omega, Clifford, SU(2), torsion-free, quantization, volume) all < 1e-10 | `dirac_spectrum.py` |
...
Every ingredient is already computed in `dirac_spectrum.py`. The computation replaces kappa with arithmetic.
