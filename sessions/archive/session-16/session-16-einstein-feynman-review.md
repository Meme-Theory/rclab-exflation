...
The Level S/A/B/C/D/F classification I introduced in Round 2d-i remains the most honest summary:
- **Level S** (computed, verified): KO-dim, quantum numbers, Jensen metric, 11 equations
- **Level A** (computable in days): V_eff, Z_3 labeling, U(2) projection
- **Level B** (computable in weeks): gauge couplings, Paasch test, Pfaffian
- **Level C** (clear algorithm, prerequisites missing): full Pfaffian, mass integrals
- **Level D** (no algorithm exists): Bell CHSH
- **Level F** (no computation behind it): "phase boundary," "no-boundary 12D"

Everything below Level B is, at present, a PROGRAM -- not a theory.
