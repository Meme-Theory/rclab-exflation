...
`computations/dirac_spectrum.py` extended from p+q ≤ 3 (10 irreps) to p+q ≤ 6 (28 irreps).
...
`computations/spectral_action.py` — full implementation.
...
`computations/mc_phi_significance.py` + `computations/extended_phi_analysis.py`
...
|------|--------|-------------|
| `dirac_spectrum.py` | kk-theorist | MODIFIED: extended to p+q ≤ 6, caching, general irrep construction |
| `spectral_action.py` | sim-specialist | NEW: spectral action, heat kernel, Seeley-DeWitt, Baptista comparison |
| `phi_analysis.py` | sim-specialist | NEW: comprehensive phi structure analysis framework |
...
**computation:** DONE (all phases, KO-dim=6, chirality resolved, A_F identified).

**Level 1 Dirac D(SU(3)):** DONE through p+q ≤ 6. phi_paasch^1 = 3.65σ (real). Paasch spiral absent.

**Level 1 Spectral Action:** DONE (classical). 1-loop V_eff minimum computable.

**Level 1.5 (NEXT PRIORITIES):**
