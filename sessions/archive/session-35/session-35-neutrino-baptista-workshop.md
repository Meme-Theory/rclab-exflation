...
2. **The pass criteria gain a new level.** Given that $\tilde{\Phi}$ is now the sole mechanism, a "soft pass" (R in [10, 100] with wrong mixing angles) no longer represents a mechanism that "needs refinement" -- it represents a mechanism that either works or does not. The $\tilde{\Phi}$ computation has zero free parameters. If it produces the right $R$ but wrong angles, the framework's PMNS mechanism is closed (no knobs to turn).
...
**Level 1 pass** (structural viability):
...
**Level 2 pass** (quantitative prediction, Evidence Level 4):
...
- **Computation pre-registered**: INTER-SECTOR-PMNS-36 (FINAL) has well-defined pass/fail criteria at two levels.
...
| 12 | Proximity of $R = 33$ to fold ($\Delta\tau = 0.004$) | E2 (R2 baptista), SD1, Re:SD1 | **Partial** | Fold *enables* $R \gg 1$ (structural). $R = 33$ specifically requires $\tau_0$ selection (conditional). Corridor $\Delta\tau = 0.005$ for Level 2. |
...
| 22 | INTER-SECTOR-PMNS-36 gate definition | N5, E6, SE5 | **Converged** | FINAL gate: Level 1 ($R \in [17, 66]$, angles in generous bands) and Level 2 ($R \in [29, 37]$, angles within $3\sigma$ NuFIT). Kill: near-diagonal $U$ or chaotic mixing. |
...
5. **Compute the PMNS matrix from $\tilde{\Phi}$**: Execute Steps B-C of the E6 program. Output: $R$, $\sin^2(\theta_{12})$, $\sin^2(\theta_{13})$, $\sin^2(\theta_{23})$, $\delta_{CP}$. Evaluate against INTER-SECTOR-PMNS-36 (FINAL) Level 1 and Level 2 criteria.
