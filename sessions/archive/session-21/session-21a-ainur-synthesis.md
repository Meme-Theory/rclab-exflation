...
**Computation files**: `computations/s21a_low_mode_TT_casimir.py`, `s21a_low_mode_TT.npz`, `s21a_low_mode_TT.png`
...
Script: `computations/s21a_low_mode_TT_casimir.py` (378s runtime)
Data: `computations/s21a_low_mode_TT.npz`
Plot: `computations/s21a_low_mode_TT.png`
...
### Level 1 — Zero Cost, Do Immediately
...
**Data files:**
- `computations/s19a_sweep_data.npz` — 11,424 Dirac eigenvalues at 21 tau values (tau=0.0 to 2.0, step 0.1). Keys: `tau_values`, `eigenvalues_{i}`, `sector_p_{i}`, `sector_q_{i}`, `multiplicities_{i}`, `fermionic_mult_{i}` for i=0..20.
- `computations/kk1_bosonic_spectrum.npz` — 2,170 bosonic modes at 4 tau values (0.0, 0.15, 0.30, 0.50).
- `computations/l20_TT_spectrum.npz` — Aggregate E_TT(tau) only, no per-mode eigenvalues.
...
**Data source:** `computations/l20_vtotal_minimum.npz` (21-point Session 20b data)
...
**Section VIII update:** V''_total spinodal scan (was Level 2 item #9) is DONE — answer is no spinodal. Demote to COMPLETED. V_IR for p+q<=2 confirmed as the decisive remaining test.
