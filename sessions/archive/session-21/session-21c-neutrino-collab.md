...
2. Extract lightest D_K(tau_0) eigenvalues -- Ready (Level 1 data exists)
...
This is an important self-consistency test: the framework must simultaneously predict neutrino masses from D_K(tau_0) and an expansion history from the modulus dynamics, and both must be consistent with CMB + LSS data. This cross-check is a future Level 3 deliverable but should be flagged now.
