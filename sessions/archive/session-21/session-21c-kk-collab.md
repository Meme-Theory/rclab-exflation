...
I wrote preliminary code for this during Phase B (file: `computations/s21c_kk_verify.py`), which the synthesis notes was not run to completion. The computation is straightforward:

delta_T(tau) = -(1 / (64 pi^2 e^{4 tau})) * Sum_{(p,q)} Delta_b(p,q) * Sum_n ln(lambda_n(tau)^2)

All ingredients exist: Delta_b = -(5/9) b_2 from branching rules (computed), lambda_n(tau) from dirac_spectrum.py sweep data (21 tau-values, stored). The computation is a pure post-processing step.
...
This is diagonal in the Peter-Weyl basis and computable from existing representation matrices rho(e_a) stored by dirac_spectrum.py. The eigenvalues can be computed at all 21 tau-values in minutes, giving a dense bosonic V_IR curve to compare with the fermionic one.
