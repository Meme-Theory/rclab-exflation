...
#### computation: Zero-Cost (minutes, existing data)
...
#### Level 1: Hours (new computation needed)
...
#### Level 2: Days (major computation)
...
|:--|:---------|:-----------|:-----|:---------------|:---------|
| 1 | Jahn-Teller mechanism at M0 selects Jensen deformation | QA | Moderate | Elevates Jensen from parametrization to geometric necessity | No (Level 2) |
| 2 | Acoustic impedance mismatch as dynamical trapping | QA, Tesla | Zero | New stabilization mechanism bypassing V_eff | Yes (computation) |
| 3 | EIH derivation of modulus equation from 12D Bianchi identity | Einstein | Zero (theory) | Establishes purely geometric dynamics | Yes (computation) |
| 4 | Higgs-sigma portal lambda_{H sigma}(tau) | Connes | 1 day | Independent escape from spectral-sum traps | Yes (Level 1) |
| 5 | BCS-BEC crossover line vs tau | Tesla, QA | Low | Identifies condensate character at FR minimum | Yes (Level 1) |
...
| 7 | Singlet/fundamental BCS gap ratio as mass hierarchy | Neutrino, Baptista | Phase 2 | Natural m_nu/m_e mechanism (numbers need work) | No (Level 2) |
| 8 | Order-one condition as algebraic tau_max | Connes, Baptista | 1 day | Stabilization without dynamics | Yes (Level 1) |
...
| 10 | T''(0) proper-time regularization sweep | Feynman | Zero | Robust vs pathological UV dominance | Yes (computation) |
| 11 | Page curve with tau-dependent N_species | Hawking | Phase 2 | Modified black hole evaporation prediction | No (Level 2) |
| 12 | Cavity mode analysis of [0.10, 1.58] | Tesla | Zero | Explains feature clustering as cavity eigenmodes | Yes (computation) |
| 13 | Stokes phenomenon at monopole transitions | Berry | Phase 1 | Instanton action may flip at monopoles | Yes (Level 1) |
| 14 | Poplawski torsion from fermion condensate | Tesla | Phase 2 | Non-perturbative repulsive potential | No (Level 2) |
| 15 | Rolling quintessence from V_total + Hubble friction | Einstein, SP | Zero | w(tau) trajectory for DESI comparison | Yes (computation) |
| 16 | T''(0) for non-Jensen deformation (null hypothesis) | Sagan | Level 2 | P(T''(0) > 0 | ~framework); false positive rate | No (Level 2) |
| 17 | Golden ratio in eigenvalue flow derivatives | Paasch | Zero | Paasch scaling in Berry curvature sector | Yes (computation) |
| 18 | Equivalence principle / Eotvos parameter from modulus | Einstein | Low | Testable WEP constraint on KK scale | Yes (computation) |
| 19 | Fano resonance profile at monopoles | QA | Zero | Independent coupling strength measurement | Yes (computation) |
| 20 | Generalized second law constraint on Branch A/B | Hawking | Phase 1 | Thermodynamic discrimination without instanton | Yes (Level 1) |
| 21 | Phonon Green's function / Fano parameter at M1, M2 | QA | Minutes | Coupling strength from line shape | Yes (computation) |
| 22 | Koide formula from Z3-dependent BCS gap | Paasch | Phase 2 | Q = 2/3 from spectral geometry | No (Level 2) |
| 23 | DNP stability bound lambda_L >= 3m^2 on Jensen SU(3) | KK | Zero | Instability window for NP transition | Yes (computation) |
| 24 | L_tilde vs L_X closure correction | Baptista | 1 day | Systematic coupling matrix correction | Yes (Level 1) |
| 25 | PMNS delta_CP from monopole-structure eigenspinors | Neutrino | Phase 2 | Near-maximal CP violation qualitative prediction | No (Level 2) |
...
The REFUTED label caused all 15 collaborative reviewers to skip CP-1's algebraic identity entirely. Zero of the eleven "New Physics" findings (Section III) address the flux-spectral identity. The only trace in this document is computation item #17 ("Gauge coupling running g₁(τ) — KK — Correct observable for CP-1 e^{-4τ} identity"). KK saw it. Nobody else followed up.
...
4. kk's computation #17 (g₁(τ) gauge coupling running) elevated from priority table line item to named investigation
...
|:--|:----------|:-------|:-----|
| 1 | g₁(τ) gauge coupling running | kk (computation #17) | Does g₁(τ) inherit the e^{-4τ} profile from the Cartan 3-form? |
...
| 3 | δ_T(τ) sector decomposition | baptista (computation #6) | Do sector-dependent zero-crossings reflect the e^{-4τ} identity? |
...
All three candidate observables computed from existing sweep data (21 tau values, 28 sectors). Script: `computations/s21c_cp1_identity_investigation.py`. Output: `computations/s21c_cp1_output.txt`. Plot: `computations/s21c_cp1_investigation.png`.
