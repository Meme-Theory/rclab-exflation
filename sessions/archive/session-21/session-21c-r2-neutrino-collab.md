...
### 3.1 Coupled R(tau) (Level 1 #2): Most Urgent Neutrino-Specific Computation
...
I note that this computation must be done WITH proper J-symmetry enforcement (Dirac's computation #5 suggestion). If the coupling breaks J-symmetry numerically, the CPT guarantee ([J, D_K(s)] = 0 from Session 17a D-1) is violated at the computational level, and the neutrino/antineutrino eigenvalue pairing -- confirmed by KamLAND (Paper 09) at the experimental level -- would be spuriously broken. The J-odd component of delta_T must vanish exactly, as Dirac noted.
...
I acknowledged in Round 1 that the simplest BCS formula gives the wrong ratio (~0.89 vs the required ~10^{-7} for m_nu/m_e). This remains true. The mechanism identifies the right ingredient (sector-dependent DOS) but not the right numbers. The quantitative calculation requires the full BCS coupling matrix (Level 2 #1), not just the gap-edge approximation.
...
4. Compute PMNS angles from eigenspinor overlaps -- Level 2
...
- **PENDING**: PMNS mixing angles and delta_CP (requires Level 2 spinor transport)
