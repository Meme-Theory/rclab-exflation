...
KK wrote preliminary code in `computations/s21c_kk_verify.py`. This code should be validated and executed as the very first action of Phase 1.
...
Baptista's pre-registered prediction (Section IV, CP-2): low-mode level statistics for the lowest 20-50 eigenvalues should show Brody parameter q ~ 0.3-0.5 (intermediate between Poisson q=0 and GOE q=1). This is testable with existing data from `computations/dirac_spectrum.py`.
...
| File | Description |
|:-----|:-----------|
| `computations/dirac_spectrum.py` | Dirac spectrum, 28 irreps, mps=6 |
| `computations/s21c_V_IR.npz` | V_IR data at multiple N cutoffs |
| `computations/s21c_neutrino_fine_grid.npz` | Neutrino R(tau) fine grid |
| `computations/s21c_T_double_prime_result.txt` | T''(0) = +7,969 |
| `computations/s21c_S_signed.npz` | S_signed monotonically decreasing |
| `computations/s21c_kk_verify.py` | Preliminary delta_T(tau) code (P1-0) |
