...
**Output data**: `computations/s25_baptista_results.npz`, `computations/s25_baptista_results.png`
**Computation script**: `computations/s25_baptista_results.py`
...
|------|---------------------|---------------------|--------|
| Goal 1 (Graded spectral sum) | OPEN (Level 1) | OPEN -- endorsed, not computed here | No change |
| Goal 2 (Finite cutoff V_full) | OPEN (Level 1) | OPEN -- Berry found smooth V_full monotone. V_Baptista has minimum but is different functional. | V_Baptista computed as Baptista-side analog |
| Goal 3 (Berry phase) | OPEN (Level 1) | **CLOSED** (Berry erratum, independently confirmed) | Omega = 0 identically |
| Goal 4 (Spectral flow) | OPEN (Level 2) | **CLOSED** (Lichnerowicz bound) | R_K > 0 prevents zero crossings |
| Goal 5 (Gap-edge holonomy) | OPEN (Level 2) | **CLOSED** (Berry erratum, trivial holonomy) | A = 0 identically |
| Goal 7 (Self-consistent mu) | OPEN (Level 3) | OPEN -- V_Baptista minimum depends on mu^2. Self-consistent mu_eff would fix both kappa and mu. | Enhanced by V_Baptista computation |
| Goal 8 (Higher a_k) | OPEN (Level 3) | OPEN -- a4/R_K = 985 confirmed. a6 not computed. | a4/a2 ratio cross-checked |
...
1. `computations/s25_baptista_results.py` -- Computation script (11 sections, 659 lines)
2. `computations/s25_baptista_results.npz` -- All numerical results (tau grids, R_K, m^2, M, kappa sweeps, tau_min sweeps, V_min sweeps, Lichnerowicz bound, Berry erratum flag)
3. `computations/s25_baptista_results.png` -- 6-panel figure (R_K, m^2, V_Baptista for multiple kappa, tau_0(kappa), classical vs quantum, V_Baptista near minimum)
