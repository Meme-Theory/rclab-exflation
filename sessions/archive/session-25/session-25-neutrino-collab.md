...
The graded multi-sector spectral sum S_eff(tau) = sum_{(p,q)} d_{(p,q)} * [graded contribution] is the most promising of the Level 1 goals. I evaluate it from the neutrino standpoint.
...
Neutrinoless double-beta decay experiments (LEGEND, nEXO, targeting m_{beta beta} ~ 0.01 eV) will test the Majorana hypothesis. If the framework stabilizes and produces predictions, the Dirac/Majorana question is a discriminant: a Majorana mass would require the spectral action to generate the Weinberg operator at the stabilized tau_0, while a purely Dirac mass would be the bare D_K eigenvalue. The current framework does not compute this -- it is a Level 3 target at best.
...
| All Level 1 monotone, no minimum anywhere | 0.2 | Neutrino program effectively closed |
