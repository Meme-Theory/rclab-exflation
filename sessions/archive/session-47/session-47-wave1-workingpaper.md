...
- Plot: `computations/s47_pi_sector.png`
...
- Plot: `computations/s47_k7_filter.png`
...
- Main plot: `computations/s47_condensate_torus.png`
- Analysis plot: `computations/s47_condensate_torus_analysis.png`
- Haar-weighted plot: `computations/s47_condensate_torus_haar.png`
- Character atlas: `computations/s47_condensate_torus_characters.png`
...
- Data: `computations/s47_spectral_landscape.npz`
- Figure: `computations/s47_spectral_landscape.png` (300 dpi)
- Figure: `computations/s47_spectral_landscape.pdf`
...
Computed all 28 = C(8,2) sectional curvatures K(e_a, e_b; tau) for the Jensen-deformed left-invariant metric on SU(3), using the Riemann tensor infrastructure from `dirac_spectrum.py` (validated at 147/147 checks in r20a). The Jensen deformation decomposes su(3) = u(1) + su(2) + C^2 with scale factors L_1 = e^{2tau} (u(1), 1D), L_2 = e^{-2tau} (su(2), 3D), L_3 = e^{tau} (C^2, 4D).
...
- Figure: `computations/s47_curvature_anatomy.png`
...
- Plot: `computations/s47_coherence_response.png` (6-panel diagnostic)
- Plot: `computations/s47_coherence_response_contrast.png`
...
- Plot: `computations/s47_rhos_tensor.png` (3-panel: stiffness vs tau, anisotropy at fold, scatter vs curvature)
...
- Plot: `computations/s47_aniso_dissip.png` (6-panel: anisotropy, block-diag, Clifford, enhancement, shortfall, current norms)
...
- Plot: `computations/s47_spectral_flow_ns.png` (6-panel: BdG energies, flow rates, geom-vs-BCS decomposition, alpha(tau), occupation spectra, summary)
...
- `computations/s47_pi_sector.py` / `.npz` / `.png`
- `computations/s47_k7_filter.py` / `.npz` / `.png`
- `computations/s47_ratio_correct.py` / `.npz`
- `computations/s47_spectral_landscape.py` / `.npz` / `.png` / `.pdf`
- `computations/s47_curvature_anatomy.py` / `.npz` / `.png`
- `computations/s47_condensate_torus.py` / `.npz` / `.png` (+ `_analysis.png`, `_haar.png`, `_characters.png`)
- `computations/s47_paasch_spiral.py` / `.npz` / `.png`
- `computations/s47_coherence_response.py` / `.npz` / `.png` / `_contrast.png`
- `computations/s47_rhos_tensor.py` / `.npz` / `.png`
- `computations/s47_aniso_dissip.py` / `.npz` / `.png`
- `computations/s47_spectral_flow_ns.py` / `.npz` / `.png`
...
- Script: `computations/s47_texture_corr.py`
- Data: `computations/s47_texture_corr.npz`
- Plot: `computations/s47_texture_corr.png`
...
- `computations/s47_acoustic_horizon.py` / `.npz`
...
|:-----|:---------------|:------------|:----------|:-------|
| S47 | S46 ratio 2.19x (131 pi-phases / 59.8 pairs) | OPEN | **CLOSED** | RETRACTED: The number 2.19x (131 pi-phases / 59.8 pairs). This was computed from mismatched bases. |
| S47 | SPECTRAL-FLOW-NS-47 (adiabatic spectral flow n_s route) | OPEN | **CLOSED** | alpha=2.63, n_s=-2.51 (827 sigma). k-dependent but wrong slope. 11th n_s route closed. Same eigenvalue-span obstruction. |
| S47 | ACOUSTIC-HORIZON-48 (acoustic horizon route to 151 Mpc) | OPEN | **CLOSED** | The acoustic horizon route to 151 Mpc is CLOSED by 60 orders of magnitude (physical) or 31 orders (comoving). |
