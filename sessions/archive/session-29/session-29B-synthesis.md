...
| File | Description |
|:-----|:-----------|
| `computations/s29b_3sector_fbcs.{py,npz,png}` | 3-sector F_BCS + gradient balance |
| `computations/s29b_pmns_extraction.{py,npz,png,txt}` | PMNS extraction (Methods B, C) |

### 29Bb
| File | Description |
|:-----|:-----------|
| `computations/s29b_jensen_transverse.{py,npz,png,txt}` | Jensen 5D transverse Hessian |
| `computations/s29b_bogoliubov_bcs.{py,npz,png,txt}` | Bogoliubov BCS gap equation |
| `computations/s29b_josephson_coupling.{py,npz,png,txt}` | Full 1-loop Josephson coupling |

### Gate Verdicts
`computations/s29b_gate_verdicts.txt` (all 29B gates, appended to 29A verdicts)
