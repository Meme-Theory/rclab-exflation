...
Tesla proposes (lines 87-93) computing V_spec(tau) from existing data as a "20-line Python script and 30 seconds of runtime." I concur on the feasibility -- the data exists in `computations/r20a_riemann_tensor.npz` and `computations/s23c_fiber_integrals.npz`. R_K(tau) is analytic (eq 3.70). |Ric(tau)|^2 and K(tau) are computed at 21 tau values in the Riemann tensor data.
...
I endorse this computation as valuable. It costs essentially nothing -- the V_{nm} matrix is already in `computations/s23a_kosmann_singlet.npz`.
