...
This is a genuine research direction, but it requires more infrastructure than currently exists. I flag it as a Level 2 computation.
...
|Ric(tau)|^2 and K(tau) are stored in `computations/r20a_riemann_tensor.npz` at 21 tau values. R_K(tau) is analytic. The potential shape depends on the single ratio rho = c_4/c_2 = f_4/(60 * f_2 * Lambda^2).
...
3. **Petrov type classification at tau = 0.20 and tau = 0.30** (Level 2). The 8D Weyl tensor Psi_{ABCD} has more algebraic structure than the 4D version. The Petrov type may change at monopole locations (M0, M1, M2), which would signal a change in the algebraic special structure of the internal geometry.
