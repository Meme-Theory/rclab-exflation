...
**Input**: R_K(tau) from analytic formula, |Ric(tau)|^2 and K(tau) from `computations/r20a_riemann_tensor.npz` and `computations/s23c_fiber_integrals.npz`.
...
**Input**: Eigenvectors from `computations/s23a_eigenvectors_extended.npz` at 9 tau values. Specifically the two (0,0) singlet gap-edge eigenvectors.
...
**Input**: V_{nm} matrix from `computations/s23a_kosmann_singlet.npz`. Eigenvalues lambda_n from existing data. Both at each of 9 tau values.
