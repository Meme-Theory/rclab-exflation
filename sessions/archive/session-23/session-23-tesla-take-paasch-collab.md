...
The V_{nm} matrix from `computations/s23a_kosmann_singlet.npz` defines a tridiagonal coupling on the eigenvalue ladder. In solid-state physics, a 1D tight-binding chain with nearest-neighbor hopping t and on-site energies epsilon_n has the dispersion relation:
...
Concretely, from the data in `computations/s23a_eigenvectors_extended.npz` and `computations/s23a_kosmann_singlet.npz`:
