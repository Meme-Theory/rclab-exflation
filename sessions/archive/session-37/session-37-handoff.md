...
|---|------|-----|-------|--------|--------|----------|------------|
| A1 | Kibble-Zurek defect density computation | nazarewicz + landau | TAU-DYN-36 transit rate, xi_BCS=0.808, BCS critical exponents | n_defect(v_transit) scaling law and numerical estimate | computation script + session doc | S38 | None |
| A2 | Pair vibrator radiation (Bogoliubov coefficients) | feynman or hawking | F.2 GPV data (omega=0.792, B=9.942), TAU-DYN-36 trajectory | Particle creation rate, spectral content of radiation | computation script | S38 | None |
| A3 | Instanton-averaged spectral action | quantum-foam + spectral-geometer | INST-MC-37 data, s37_cutoff_sa.npz | S_foam(tau) landscape, check for non-monotonicity | computation script | S38 | None |
...
| Path | Content |
|:-----|:--------|
| `computations/s37_cutoff_sa.{py,npz,png}` | CUTOFF-SA-37 computation, data, visualization |
| `computations/s37_k7_g1.{py,npz}` | K7-G1-37 q_7 charge analysis |
| `computations/s37_instanton_action.{py,npz,png}` | F.1 instanton action |
| `computations/s37_pair_susceptibility.{py,npz,png}` | F.2/F.3 pair susceptibility + vacuum polarization |
| `computations/s37_oneloop_sa.{py,npz,png}` | F.5 one-loop spectral action correction |
| `computations/s37_cutoff_crosscheck.{py,npz}` | Feynman cross-check (7 checks) |
| `computations/s37_instanton_mc.{py,npz,png}` | F.4 instanton MC (0D + lattice + extended) |
| `computations/s37_gate_verdicts.txt` | Gate verdicts |
...
| Path | Content |
|:-----|:--------|
| `computations/s36_cc_arithmetic.{py,npz}` | CC gradient computation |
