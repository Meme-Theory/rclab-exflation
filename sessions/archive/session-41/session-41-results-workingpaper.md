...
**Data files:**
- Script: `computations/s41_offjensen_bcs.py`
- Data: `computations/s41_offjensen_bcs.npz`
- Plot: `computations/s41_offjensen_bcs.png`
...
**Data files:**
- Script: `computations/s41_fermionic_spectral_action.py`
- Data: `computations/s41_fermionic_spectral_action.npz`
- Plot: `computations/s41_fermionic_spectral_action.png`
...
**Data files:**
- Script: `computations/s41_log_signed.py`
- Data: `computations/s41_log_signed.npz`
- Plot: `computations/s41_log_signed.png`
...
**Data files:**
- Script: `computations/s41_mkk_rge.py`
- Data: `computations/s41_mkk_rge.npz`
- Plot: `computations/s41_mkk_rge.png`
...
**Data files:**
- Script: `computations/s41_constants_vs_tau.py` and `computations/s41_spectral_refinement.py`
- Data: `computations/s41_constants_vs_tau.npz` and `computations/s41_spectral_refinement.npz`
- Plots: `computations/s41_constants_vs_tau.png` and `computations/s41_spectral_refinement.png`
...
| 2 | **Z(τ) gradient stiffness** | ROOT NODE for all fabric physics | computation + spatial perturbation | MEDIUM |
...
|:-----|:---------------|:------------|:----------|:-------|
| 2026-03-12 | B2-OFFJ-41 | OPEN | **CLOSED** | BCS condensate is topologically robust; all three BCS observables within 0.2% of Jensen at eps=0.1 — two orders of magnitude inside the 20% gate threshold. |
| 2026-03-12 | SF-TRANSIT-41 | OPEN | **CLOSED** | S_F^Pfaff monotonically increasing across [0.10, 0.30]; all 19 finite differences positive, no extremum, no sign change. |
| 2026-03-12 | LOG-SIGNED-41 | OPEN | **CLOSED** | 1/9 variants has minimum; requires physical B/F assignment — conditional PASS at fiducial A = 0.099 with minimum at tau = 0.150. |
| 2026-03-12 | N-EFF-41 | OPEN | **CLOSED** | N_eff(fold)/N_eff(0) = 240/32 = 7.50 — passes the 1.5x threshold by 5x; but step function at tau=0, not gradual refinement. |
| 2026-03-12 | TAU-H-41 | OPEN | **CLOSED** | Crystal ALWAYS resolves atomic scales; tau_H = 0, always resolved (resolution surplus 10^14 to 10^18). |
| 2026-03-12 | Fermionic spectral action (Connes channel) | OPEN | **CLOSED** | S_F^Connes = 0 identically (BDI symmetric). |
| 2026-03-12 | Fermionic spectral action (Pfaffian channel) | OPEN | **CLOSED** | S_F^Pfaff monotonic (+21.7%), no extremum. |
| 2026-03-12 | Baptista single-eigenvalue normalization | OPEN | **CLOSED** | sin^2 = 0.584, never reached in SM RGE. |
| 2026-03-12 | Spectral refinement as gradual expansion | OPEN | **CLOSED** | N_eff is step function: 32→240 at tau=0, constant after. |
| 2026-03-12 | Hydrogen resolution as spectral refinement gate | OPEN | **CLOSED** | Crystal always 10^14x finer than atomic scale — tau_H=0, not a constraint. |
| 2026-03-12 | CMB as crystal ring (naive) | OPEN | **CLOSED** | Crystal gap 10^{22}x above CMB frequency; cannot ring at 2.725 K. |
| 2026-03-12 | CMB spectral shape from internal modes | OPEN | **CLOSED** | 4D projection → degeneracy factor only; spectral shape from omega^2, not crystal. |
| 2026-03-12 | Direct eigenvalue-spacing → T_CMB | OPEN | **CLOSED** | Requires M_KK ~ 22.5 neV; 20 orders of magnitude below viable KK scale. |
| 2026-03-12 | Spectral refinement drives nucleosynthesis | OPEN | **CLOSED** | N_eff step function; crystal always resolves nuclear scales. |
| 2026-03-12 | n/p from crystal symmetry alone | OPEN | **CLOSED** | SU(2) exact → n/p = 1 → Y_P = 1.0 (4x wrong). Yukawa sector essential. |
| 2026-03-12 | Topological defects from internal quench as nuclei | OPEN | **CLOSED** | 0D limit, BDI winding = 0, skyrmion scaling off by 10²⁰. |
| 2026-03-12 | tau-thresholds for nuclear species existence | OPEN | **CLOSED** | Crystal always resolves nuclear scales; surplus 10¹⁴. No resolution bottleneck. |
| 2026-03-12 | Theorem (C2*D_K Symmetry) | OPEN | **LANDED** | (C2 D_K)^T = C2 D_K for all tau. Consequence: S_F^Connes = (1/2) psi^T C2 D psi = 0 identically for any Grassmann spinor at any tau on Jensen-deformed SU(3). |
| 2026-03-12 | Theorem (Spectral Pairing) | OPEN | **LANDED** | <BCS|D_K|BCS> = Tr(D_K n_BCS) = 0 identically by {gamma_9, D_K} = 0 pairing. |
| 2026-03-12 | BDI Channel Decomposition | OPEN | **LANDED** | Fermionic bilinears split into symmetric (C2*D, vanishes) and antisymmetric (C1*D, Pfaffian) channels. Only the Pfaffian channel is non-trivial. |
| 2026-03-12 | S_F^Pfaff sector fractions at fold | OPEN | **LANDED** | B2 = 57.2%, B1 = 18.0%, B3 = 24.8%. |
| 2026-03-12 | BCS topological robustness | OPEN | **LANDED** | B2 gap, pairing eigenvalue, and Thouless parameter all within 0.2% of Jensen at eps=0.1 along softest transverse direction g_73. QRPA stable at all epsilon. Condensate never closes even at eps=0.5. |
| 2026-03-12 | Signed sum monotonicity extension | OPEN | **LANDED** | All tau-independent B/F assignments produce monotonic or zero signed logarithmic sums. Only eigenvalue-dependent assignments (gap-edge weighted) can break monotonicity. |
| 2026-03-12 | Convention B excluded | OPEN | **LANDED** | The full Baptista single-eigenvalue hypercharge normalization (g'/g = sqrt(3)*e^{-2tau}) gives sin^2 = 0.584 at the fold, which is never reached by SM RGE running. |
| 2026-03-12 | M_KK brackets | OPEN | **PINNED** | Convention A (metric ratio) gives M_KK ~ 10^9 GeV; Convention C (Connes/GUT trace) gives M_KK ~ 10^13 GeV. Both viable, normalization ambiguity unresolved. |
| 2026-03-12 | N_eff is a step function | OPEN | **LANDED** | Degeneracy count jumps 32→240 at infinitesimal tau (Jensen immediately breaks SU(3) isometry), then stays constant. Residual d_avg = 5.13 from representation-theoretic degeneracies that no smooth deformation can lift. |
| 2026-03-12 | Seeley-DeWitt coefficients are cutoff-insensitive | OPEN | **LANDED** | Min eigenvalue 0.819 >> all cutoffs. a_0 constant (mode count), a_2 drops 19%, a_4 drops 26% over [0, 0.5]. Ratios a_2/a_0 and a_4/a_2 provide concrete mapping tau → (G_N, alpha). |
| 2026-03-12 | Constants frozen post-BCS | OPEN | **LANDED** | Transit variation O(1-26%), but post-freeze tau_dot=0 → constants exactly static. Clock constraint automatically satisfied. |
| 2026-03-12 | 4D projection theorem | OPEN | **LANDED** | Internal crystal mode structure contributes only a multiplicative degeneracy factor N_internal to the 4D mode density rho_4D = (4*pi*omega^2/(2*pi*c)^3) x N_internal. Spectral SHAPE determined by 4D photon dispersion, not crystal band structure. |
| 2026-03-12 | FIRAS compatibility automatic | OPEN | **LANDED** | All 77,992 internal modes frozen out by exp(-10^{22}) Boltzmann suppression at T_CMB. Any non-thermal ring at z ~ 10^{23} is thermalized by standard double Compton + Bremsstrahlung processes. |
| 2026-03-12 | SU(3) under Jensen IS a phononic crystal | OPEN | **LANDED** | Formally satisfies all phononic crystal criteria: Bloch modes (Peter-Weyl = crystal momentum), 3-branch dispersion (B1 acoustic-like, B2 flat optical, B3 dispersive optical), hard spectral gap lambda_min = 0.820 M_KK, van Hove singularities. |
| 2026-03-12 | No Umklapp on SU(3) (structural) | OPEN | **LANDED** | Peter-Weyl representation lattice is infinite and non-periodic. No Brillouin zone boundary wrapping. Only Normal (momentum-conserving) scattering exists. Structural reason for GGE permanence independent of Richardson-Gaudin integrability. |
| 2026-03-12 | Debye temperature Theta_D ~ 10^{22-26} K | OPEN | **LANDED** | T/Theta_D ~ 10^{-22} at current epoch. Crystal is transcendently quantum — thermally dead (C_V = 0 exactly) at all temperatures below GUT scale. |
| 2026-03-12 | Q_B2 ~ 10 (struck drum) | OPEN | **LANDED** | From B2-DECAY-40: Im[Sigma_{Ϝ,B2}] = 0.043 M_KK. GGE permanence is permanent dent, not permanent oscillation. Corrects Tesla S40 "struck bell" label. |
| 2026-03-12 | Ϝ (digamma) notation adopted | OPEN | **REGISTERED** | Modes of D_K on the internal phononic crystal substrate. Complete symbol table: Ϝ_n, Ϝ_{(p,q)}, Ϝ_{B1/B2/B3}, |Ϝ_n⟩, n_Ϝ, omega_Ϝ, T_Ϝ, A_Ϝ(omega), G_Ϝ(omega), Sigma_Ϝ, kappa_Ϝ, Z_Ϝ. |
| 2026-03-12 | n/p = 1 from crystal symmetry (structural) | OPEN | **LANDED** | Jensen deformation preserves SU(2) exactly. PH-forced mu = 0 independently confirms particle-hole symmetry. Isospin breaking requires the Yukawa sector (finite geometry F), not D_K. This is algebraic and survives fabric coupling. |
| 2026-03-12 | The 59.8 Bogoliubov pairs are one collective mode | OPEN | **LANDED** | The GPV concentrates 85.5% of pair-addition strength. B1/total = 0.0045. The quench produces a strongly hierarchical output. |
| 2026-03-12 | Compound nucleus evaporation is the correct decay framework | OPEN | **LANDED** | Hauser-Feshbach branching ratios span 8 orders of magnitude from O(1) excitation. KK transmission coefficients are the decisive uncomputed quantity. |
| 2026-03-12 | Fabric reframing: single-crystal closures do not extend to the fabric for dynamical quantities | OPEN | **LANDED** | CUTOFF-SA-37 (structural monotonicity) is proved for Tr f(D²) at one crystal; gradient terms (∇τ)² are OUTSIDE its scope. TAU-DYN-36 and FRIEDMANN-BCS-38 are the strongest candidates for reopening with fabric spatial inertia. F.5 wrong sign STRENGTHENS with fabric coupling. Algebraic closures (10) are IMMUNE. |
| 2026-03-12 | 32-cell tessellation → giant structure boundaries | OPEN | **LANDED** | If N_eff = 32 modes tile early-epoch space, cell boundaries at ~1-3 Gpc match Giant Arc (1 Gpc), Big Ring (1.3 Gpc), HCBGW (2-3 Gpc). 240/32 subdivision gives linear ratio ~2. |
| 2026-03-12 | LCDM void model requires 21 assumptions + ignores 7 tensions | OPEN | **LANDED** | Lambda (50-120 OOM discrepant), CDM (never detected), inflation (unknown), plus 15 excursion set model choices. KBC void at 6.04σ, CMB Cold Spot ISW 3-5σ, excursion set overprediction. |
