...
**[x,p] = ihbar is TRIVIAL**: quantum-acoustics ceded computation priority on this computation. Klein (1926) already derived quantization conditions from 5D periodicity. The generalization to SU(3) is straightforward (Peter-Weyl theorem). This is a SOLVED problem, not an open computation.
...
├── [Phase 6d] Connect to Dirac spectrum computation (Level 1)
...
**computation** (DO FIRST -- decision gate for entire program):
1. Commutant computation: End_{U(2)}(Delta_8) = A_F? (2 weeks, pure rep theory)

**Priority 1** (DO AFTER computation if YES):
...
**Priority 1** (DO AFTER computation regardless):
