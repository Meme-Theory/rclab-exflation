"""
DIRAC SPECTRUM ON (SU(3), g_s) WITH JENSEN DEFORMATION
========================================================
...
def _build_irrep_no_cache(p, q, gens, f_abc):
    ...
    if (p, q) == (0, 0):
        ...
        # (p,q) with q>p inside a no-cache recursion: build (q,p) on conj(gens),
        # then the result IS the (p,q) representation on the current gens. This
        # mirrors the conjugation trick in get_irrep() and keeps recursion closed.
        conj_gens_inner = [-g.T for g in gens]
        rho, _ = _build_irrep_no_cache(q, p, conj_gens_inner, f_abc)
        return rho, dim_pq
    else:
...
def main():
    print("=" * 80)
    print("DIRAC SPECTRUM ON (SU(3), g_s)")
    print("=" * 80)
