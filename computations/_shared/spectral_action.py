"""
SPECTRAL ACTION ON (SU(3), g_s) WITH JENSEN DEFORMATION
=========================================================
...
using eigenvalue data from dirac_spectrum.py.
...
# Cap CPU threads to avoid contention with concurrent agents (eigvalsh path, small blocks)
...
# S81 T3 compliance: canonical constants import (tau_fold, Vol_SU3_Haar for cross-check).
# Note: legacy S14 script uses internal parameters s, Lambda, max_pq_sum (pinned in verdict).
from canonical_constants import tau_fold, Vol_SU3_Haar  # (local) used for T3 pins
...
def scalar_curvature_from_connection(s, f_abc):
    ...
    for d in range(n):
        for a in range(n):
            for b in range(n):
                for c in range(n):
                    val = 0.0  # (local) Riemann tensor accumulator
                    for e in range(n):
...
def gauge_boson_masses_baptista(sigma, s):
    ...
    m2_u2 = 0.0  # (local) u(2) bosons remain massless
    return m2_C2, m2_u2
...
def scalar_curvature_analytical(s):
    ...
    bracket_0 = 8.0  # (local) bracket value at s=0: 2-1+8-1=8
...
def su2_spectral_action_benchmark(verbose=True):
    ...
    k_max = 50  # (local) spin-rep truncation for SU(2) benchmark
...
def main():
    print("=" * 80)
    print("SPECTRAL ACTION ON (SU(3), g_s)")
    print("=" * 80)
    ...
    max_ratio_err = 0.0  # (local) tracker for Baptista cross-check
