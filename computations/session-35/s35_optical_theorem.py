...
def check_S_unitarity(T, rho, omega, label=""):
    """
    S = 1 + 2i*sqrt(rho)*T*sqrt(rho). Check S*S^dag = I.  # (local)
...
2. PHYSICAL S-MATRIX UNITARITY:
   S = 1 + 2i*pi*sqrt(rho)*T*sqrt(rho),  S*S^dag = I  # (local)
...
print(f"Results saved to: computations/session-35/s35_optical_theorem.npz")
print(f"\n{'='*70}")
