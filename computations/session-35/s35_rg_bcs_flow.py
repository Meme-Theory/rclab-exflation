...
# ============================================================
# Load stored data
vh_data = np.load('computations/session-35/s35a_vh_impedance_arbiter.npz', allow_pickle=True)
tv_data = np.load('computations/session-34/s34a_tesla_validation.npz', allow_pickle=True)
th_data = np.load('computations/session-34/s34a_dphys_thouless.npz', allow_pickle=True)
...
def integrate_rg_twoloop(g0, W, c_2loop, n_steps=100000):
    ...
    t_max = 50.0  # plenty of RG time (local)
    dt = t_max / n_steps
...
# Save
np.savez('computations/session-35/s35_rg_bcs_flow.npz',
    # Physical parameters
    V_bare=V_bare,
    V_gap=V_gap,
...
print("Data saved to computations/session-35/s35_rg_bcs_flow.npz")
...
plt.savefig('computations/session-35/s35_rg_bcs_flow.png', dpi=150, bbox_inches='tight')
plt.close()
print("Plot saved to computations/session-35/s35_rg_bcs_flow.png")
