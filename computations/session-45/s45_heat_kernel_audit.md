...
**Level 1 (exact on finite spectrum):**
...
**Level 2 (faithful approximation, converging as truncation -> infinity):**
...
**Level 3 (meaningless on finite spectrum without additional regularization):**
...
The computations in S44-S45 that produced large numbers (T ~ 10^{20301}, d_s reaching 15 at intermediate sigma) fall into **Level 3**. They are not wrong as arithmetic -- the numbers are correctly computed from the data. They are wrong as **spectral geometry**, because the quantities they purport to compute require continuum structure that the finite spectrum does not possess.
...
| CUTOFF-SA-37 (FAIL) | Spectral action monotonicity | NO -- fully valid | The structural monotonicity theorem operates on the spectral action (Level 1) and is unaffected by this audit. |
| HESS-40 (PASS) | Spectral action Hessian | NO -- fully valid | Hessian of spectral action is a Level 1 quantity. |
...
*Filed as Level 1 structural constraint. All future computations using heat kernel quantities on the truncated SU(3) spectrum must reference this audit to determine which tier their inputs belong to.*
