...
Input: dirac_spectrum.py (Dirac eigenvalue computation),
...
# DOS parameters
gamma_broadening = 0.005   # Gaussian width for DOS broadening  # (local)
n_E = 2000                # Number of energy grid points
E_min_global = 0.78  # (local)
E_max_global = 2.20  # (local)
...
ax.plot(tau_fine, T4, '^-', color='#FF9800', linewidth=1.5, markersize=3,
        alpha=0.7, label='T4: (1,1) min [B2]')  # (local)
...
for j, i_tau in enumerate(tau_indices):
    ...
    ax.fill_between(E_zoom, offset, offset + rho_norm * 0.7,
                     alpha=0.3, color=colors_tau[j])  # (local)
