...
where alpha is the specific heat exponent of the quantum liquid:
    alpha = 3 for phonon gas (d=3 Bose)  # (local)
    alpha = 2 for Fermi liquid  # (local)
    alpha = 1 for flat band  # (local)
...
sys.path.insert(0, str(Path(r"C:\sandbox\Ainulindale Exflation\computations")))
...
base = Path(r"C:\sandbox\Ainulindale Exflation\computations")
...
z_fold_cosmo = 3.65  # from phi_paasch (S12)  # (local)
...
w0_desi_err = 0.21
wa_desi = -1.52
wa_desi_up = 0.82  # (local)
wa_desi_dn = -0.73  # (local)
...
dG_over_G_at_fold = 0.0  # CONST-FREEZE-42: dG/dtau = 0 at fold  # (local)
...
for i, zt in enumerate(z_trans_values):
    ...
    ax1.plot(z_Fi[z_Fi<=3], w_eff_Fi[z_Fi<=3], '-', color=colors[i], linewidth=1.5,
             alpha=0.7, label=f'F: $z_{{trans}}={zt}$ ($w_0={w0_results[i]:.3f}$)')  # (local)
...
ax4.axhspan(w0_desi - w0_desi_err, w0_desi + w0_desi_err,
             alpha=0.15, color='blue', label=f'DESI $w_0$ 1$\\sigma$')  # (local)
