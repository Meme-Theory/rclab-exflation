...
d_s44f = np.load('computations/session-44/s44_friedmann_bcs_audit.npz', allow_pickle=True)
d_s44d = np.load('computations/session-44/s44_dimflow.npz', allow_pickle=True)
...
for ik, k_val in enumerate(k_values):
    ...
    norm = 1.0 / np.sqrt(2.0 * omega_init)  # (local)
...
np.savez('computations/session-45/s45_qnm_ns.npz', **save_dict)
print(f"Data saved to computations/session-45/s45_qnm_ns.npz")
...
plt.savefig('computations/session-45/s45_qnm_ns.png', dpi=150, bbox_inches='tight')
print(f"Plot saved to computations/session-45/s45_qnm_ns.png")
