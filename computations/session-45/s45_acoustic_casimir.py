...
from canonical_constants import E_cond  # canonical 8-mode value -0.137 (S36 ED-CONV-36); supersedes the S35 5-mode -0.115 originally used here
...
wall_data = np.load('computations/session-44/s44_coherent_wall.npz', allow_pickle=True)
dos_data  = np.load('computations/session-44/s44_dos_tau.npz', allow_pickle=True)
...
for bname in branch_names:
    ...
    if r > 0 and r < 1:
        ...
        ln_factor = -50.0  # Regularized perfect mirror limit  # (local)
    else:
        ln_factor = 0.0  # (local)
...
try:
    imp_data = np.load('computations/session-43/s43_impedance_mismatch.npz', allow_pickle=True)
    fab_data = np.load('computations/session-42/s42_fabric_dispersion.npz', allow_pickle=True)
...
if have_bdg_params:
    ...
    for bname in branch_names:
        ...
        for L in L_values:
            ...
            omega_hi = 3.0  # upper cutoff (beyond bandwidth)  # (local)
...
d_wall = 0.485  # from S44 coherent wall (0.485 M_KK^{-1})  # (local)
...
E_curvature = 1.0  # M_KK^2 (Ricci scalar of round SU(3) is O(1) in M_KK units)  # (local)
...
np.savez('computations/session-45/s45_acoustic_casimir.npz', **save_dict)
print("\n  Data saved to computations/session-45/s45_acoustic_casimir.npz")
...
plt.tight_layout()
plt.savefig('computations/session-45/s45_acoustic_casimir.png', dpi=150, bbox_inches='tight')
print("  Plot saved to computations/session-45/s45_acoustic_casimir.png")
