...
Input files:
  - computations/session-45/s45_unexpanded_sa.npz (forward moments, eigenvalues)
  - computations/session-44/s44_eih_grav.npz (singlet fractions)
  - computations/_shared/canonical_constants.py

Output: computations/session-45/s45_cc_hierarchy_cubed.npz
"""
...
hierarchy_EM_grav = 36.0  # log10(F_EM / F_grav) for proton  # (local)
...
cc_gap_conventional = 120.0      # The textbook "120 orders"  # (local)
...
h_EM = 36.0                                     # electromagnetic: alpha * m_p^2 / (G_N * m_p^2)  # (local)
...
for p in range(pmax + 1):
    for q in range(pmax + 1):
        ...
        if C2 == 0 and p == 0 and q == 0:
            ...
            C2_eff = 0.672  # ~ smallest eigenvalue^2 from data  # (local)
        else:
            C2_eff = C2
