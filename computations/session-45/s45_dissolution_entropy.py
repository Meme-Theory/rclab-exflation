...
def entanglement_entropy_bipartition(psi, d_A, dim_total):
    ...
    S_ent = -sum(p_i * ln(p_i)) where p_i = sigma_i^2.  # (local)
...
def measure_entropy_at_epsilon(H0, total_dim, H0_norm, epsilon, n_samples, d_A,
    ...
    for _ in range(n_samples):
        ...
        S = entanglement_entropy_bipartition(psi, d_A, total_dim)  # (local)
        S_values.append(S)
...
print("  Saved: computations/session-45/s45_dissolution_entropy.npz")
...
for name, res in fit_results.items():
    ...
    ax3.plot(N_plot, y, '-', color=fit_colors.get(name, 'purple'), linewidth=1.5,
             alpha=0.7, label=f'{res["label"]}, R2={res["R2"]:.3f}')  # (local)
...
print("  Saved: computations/session-45/s45_dissolution_entropy.png")
...
print(f"\n  Data: computations/session-45/s45_dissolution_entropy.npz")
print(f"  Plot: computations/session-45/s45_dissolution_entropy.png")
