...
Input:
    computations/session-27/s27_multisector_bcs.npz (eigenvalues at tau=0.0, 0.10, 0.15)
    computations/session-36/s36_sfull_tau_stabilization.npz (eigenvalues at tau=0.05, 0.19)
    computations/session-45/s45_dos_fine_scan.npz (fine scan near fold)
    computations/_shared/canonical_constants.py

Output:
    computations/session-45/s45_acoustic_ns.npz
    computations/session-45/s45_acoustic_ns.png
...
print(f"""
  ...
  At CMB pivot scale (k_pivot = 0.05 Mpc^-1):
    n_s = 1.000... (exact scale invariance from dispersion curvature)  # (local)
    Reason: k_pivot/k_max ~ 10^{{-57}}
...
Computation completed with 5 independent methods:
  ...
  Method 1 (Direct dispersion curvature at CMB scale):
    n_s = 1.000... (exact, to machine precision)  # (local)
