...
def analyze_critical_scaling(scan_result, data):
    ...
    m_star = 11.85  # M_KK, from S50 O-Z fit (tau-independent)  # (local)
...
def wilson_fisher_comparison(scan_result, data, scaling_result):
    ...
    This gives (2 - eta) / (1 + u) = 0.035, so:
        eta = 2 - 0.035 * (1 + u) = 2 - 0.035 * 57 = 2 - 1.995 = 0.005  # (local)
    ...
    # Known values
    m_star = 11.85      # M_KK, O-Z mass  # (local)
    m_L = omega_L1[idx_fold]  # Leggett mass
    K_pivot = 1.979     # from S50
    J_eff = 0.641       # effective Josephson stiffness, S50  # (local)
