...
data = np.load(r"C:\sandbox\Ainulindale Exflation\computations\s44_dos_tau.npz",
               allow_pickle=True)
...
for i_tau, (tau_val, t_str) in enumerate(zip(tau_values, tau_labels)):
    ...
    Lambda_test = 12.0  # Test plateau at the target Lambda (local)
...
threshold = 3 * mean_spacing  # (local)
...
outfile = r"C:\sandbox\Ainulindale Exflation\computations\s51_strutinsky.npz"
np.savez(outfile,
         # Input
...
plt.tight_layout()
plt.savefig(r"C:\sandbox\Ainulindale Exflation\computations\s51_strutinsky.png",
            dpi=150, bbox_inches='tight')
print(f"  Plot saved to: computations/session-51/s51_strutinsky.png")
