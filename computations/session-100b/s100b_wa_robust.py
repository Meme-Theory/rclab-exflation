...
def print_verdict_payload(payload):
    """Emit the emit_verdict payload block on stdout (race-safe path: the agent
    passes these exact values to the knowledge-MCP emit_verdict tool, the single
    lock-serialized verdict-file writer; no open-coded file append).

    POST-EMISSION ARTIFACT-CONTRACT REFACTOR (2026-06-07, stdout-identical):
    the as-emitted script (verdict-line content_sha256 077dc843babcc59c...)
    open-coded these three prints inline in main(); frozen snapshot at
    s100b_wa_robust.frozen-15c54621f59184cc.py per mechanical-closure-discipline.md
    s"Carry-forward script-bytes immutability". The verdict-line SHAs pin the
    as-emitted (frozen) state; this wrapper changes no output byte.
    """
