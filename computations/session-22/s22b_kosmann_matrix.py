...
Dependencies:
  - computations/session-22/s22b_eigenvectors.npz (from PA-1)
  - computations/_shared/dirac_spectrum.py (infrastructure)

Output: computations/session-22/s22b_kosmann_matrix.npz
...
# CPU thread cap — matrices are <=64x64, no GPU path warranted.
import os
...
# Add computations to path for canonical_constants + dirac_spectrum.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SHARED_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "computations"))
sys.path.insert(0, SHARED_DIR)
sys.path.insert(0, SCRIPT_DIR)

# Canonical constants — MANDATORY per computations/_shared/CLAUDE.md and
# computations/_shared/CLAUDE.md. Script uses no framework constants (M_KK,
# tau_fold, etc.), but the import must be present so the /weave audit marks
# it compliant. All numerics in this script are algebraic (Cliff8 dim=16,
# SU(3) coset indices) or scan parameters (tagged # (local)).
...
# =============================================================================
# SHA-256 pinning (S81+ closure protocol, per .claude/rules/gate-verdicts.md)
# =============================================================================

def _sha256_of_file(path):
    """Compute SHA-256 hex digest of a file, or return 'MISSING'."""
    try:
...
def log_input_pins(input_files):
    """
    Log SHA-256 pins of every input file in the first lines of stdout.
    Returns the closure SHA = SHA-256 of the ordered (name, sha) pin map.
    """
    ...
    for name, path in input_files:
        ...
        print(f"  {name}  {h}  {path}")
        serialization.append(f"{name}={h}")
    closure_payload = "|".join(serialization).encode('utf-8')
...
def build_multisector_kosmann_coupling(s, gens, f_abc, gammas, eigvec_data,
    ...
    irrep construction from dirac_spectrum.py.
    """
...
def main():
    ...
    # --- SHA-256 input pins (S81+ canonical protocol) ---
    eigvec_file = os.path.join(SCRIPT_DIR, "s22b_eigenvectors.npz")  # (local)
    script_self = os.path.abspath(__file__)                          # (local)
    helper_path = os.path.join(SHARED_DIR, "dirac_spectrum.py")  # (local)
    canon_path = os.path.join(SHARED_DIR, "canonical_constants.py")   # (local)

    closure_sha = log_input_pins([
        ("script",              script_self),
        ("dirac_spectrum", helper_path),
        ("canonical_constants",  canon_path),
        ("pa1_eigenvectors",    eigvec_file),
    ])
    ...
    # --- Schema adapter (T3 fix) -------------------------------------------
    # PA-1 stored eigvecs as `eigvec_{i_tau}_sector_{idx}` with `(p,q)` labels
    # in `sector_labels_{i_tau}`. Build a per-tau map that lets the rest of
    # this script's eigvec_dict[(p,q)] = (eigenvalues, eigenvectors) logic
    # work without schema-driven key misses.
    #
    # eigenvalues_{i_tau} is a flat array of length sum(sector_sizes_{i_tau}),
    # ordered by sector. We slice it per sector.
    pa1_adapter = {}  # (local)  dict[i_tau] -> dict[(p,q)] -> (evals, evecs)
    for _i_tau in range(n_tau):
        _labels = data[f'sector_labels_{_i_tau}']
        _sizes = data[f'sector_sizes_{_i_tau}']
        _flat_evals = data[f'eigenvalues_{_i_tau}']
        _sector_start = 0  # (local)
        _inner = {}  # (local)
        for _s_idx in range(len(_labels)):
            _p = int(_labels[_s_idx, 0])  # (local)
            _q = int(_labels[_s_idx, 1])  # (local)
            _sz = int(_sizes[_s_idx])     # (local)
            _key_vec = f'eigvec_{_i_tau}_sector_{_s_idx}'  # (local)
            if _key_vec in data.files:
                _evecs = data[_key_vec]
                _evals = _flat_evals[_sector_start:_sector_start + _sz]
                _inner[(_p, _q)] = (np.asarray(_evals), np.asarray(_evecs))
            _sector_start += _sz
        pa1_adapter[_i_tau] = _inner
    ...
    # Gate accumulators for T3-S22B-KOSMANN-MATRIX
    max_Lg_u2_norm = 0.0   # (local) H1: max over (a in U2_IDX, tau) ||Lg_a||  (Killing)
    max_K_LX_norm = 0.0    # (local) H0: max over (a in C2_IDX, tau) ||K_a_LX||  (should vanish)
    min_K_Ltilde_pos_tau = float('inf')  # (local) H2: min over (a in C2_IDX, tau>0) ||K_a_Ltilde||
    max_C_inter_norm = 0.0  # (local) H3: max over pair!=diagonal, tau  ||C_eig||_F (block-diag)
    gate_diag_samples = {}  # (local) pedagogical summary at tau=0.30 and tau=0.50
    ...
    for i_tau in range(n_tau):
        ...
        # Load eigenvectors for this tau (via T3 schema adapter)
        eigvec_dict = {pq: pa1_adapter[i_tau][pq]
                       for pq in sector_list if pq in pa1_adapter[i_tau]}

        if not eigvec_dict:
            print(f"  WARNING: No eigenvectors found for tau={tau} after schema adapt.")
        ...
        # --- Accumulate gate tests (T3-S22B-KOSMANN-MATRIX) ----------------
        # Rebuild tau-local metric/frame/phi once for per-tau tests.
        g_s_here = jensen_metric(compute_killing_form(f_abc), tau)  # (local)
        E_here = orthonormal_frame(g_s_here)                         # (local)
        g_hat_h, phi_h, phi_inv_h = compute_averaged_metric(f_abc, g_s_here)
        # H1: ||Lg_a|| for a in U2_IDX must be exactly zero (Killing).
        for a_u2 in U2_IDX:
            Lg_c = metric_lie_derivative_coordinate(f_abc, g_s_here, a_u2)
            Lg_o = metric_lie_derivative_ON(Lg_c, E_here)
            nrm_u2 = np.sqrt(np.sum(Lg_o ** 2))  # (local)
            if nrm_u2 > max_Lg_u2_norm:
                max_Lg_u2_norm = nrm_u2
        # H0: ||K_a_LX|| should vanish at machine precision (sym L_X g +
        # traceless -> Clifford contraction with {gamma_j, gamma_k} = 2 delta_jk
        # gives sum_j Lg_{jj} * I = Tr(Lg) = div(X) = 0 for volume-preserving
        # Jensen). H2 uses L_tilde (Paper 18 eq 1.4) which is the physically
        # meaningful corrected operator.
        for a_c2 in C2_IDX:
            Lg_c = metric_lie_derivative_coordinate(f_abc, g_s_here, a_c2)
            Lg_o = metric_lie_derivative_ON(Lg_c, E_here)
            K_lx = kosmann_spinor_operator_LX(Lg_o, gammas)
            Phi_c = compute_Ltilde_correction(Lg_o, phi_h, phi_inv_h,
                                              gammas, f_abc, g_s_here,
                                              E_here, a_c2)
            K_lt = K_lx + Phi_c
            nrm_KLX = np.sqrt(np.sum(np.abs(K_lx) ** 2))    # (local)
            nrm_KLt = np.sqrt(np.sum(np.abs(K_lt) ** 2))    # (local)
            if nrm_KLX > max_K_LX_norm:
                max_K_LX_norm = nrm_KLX
            if tau > 0.0 and nrm_KLt < min_K_Ltilde_pos_tau:
                min_K_Ltilde_pos_tau = nrm_KLt
        # H3: inter-sector ||C_eig||_F for distinct (p,q) pairs (both schemes).
        for source_dict in (coupling_LX, coupling_Lt):
            for pair_key, C_eig in source_dict.items():
                (p1, q1), (p2, q2) = pair_key
                if (p1, q1) == (p2, q2):
                    continue
                if C_eig.size == 0:
                    continue
                fr = np.sqrt(np.sum(np.abs(C_eig) ** 2))  # (local)
                if fr > max_C_inter_norm:
                    max_C_inter_norm = fr
        # Pedagogical snapshot at tau=0.30 and tau=0.50
        if abs(tau - 0.30) < 1e-9 or abs(tau - 0.50) < 1e-9:
            gate_diag_samples[float(tau)] = {
                'max_Lg_u2': max_Lg_u2_norm,
                'max_KLX':   max_K_LX_norm,
                'min_KLt':   min_K_Ltilde_pos_tau,
                'max_Cinter': max_C_inter_norm,
            }
    ...
    out_file = os.path.join(SCRIPT_DIR, "s22b_kosmann_matrix.npz")
...
""")
    ...
    # --- Gate verdict: T3-S22B-KOSMANN-MATRIX -----------------------------
    NORM_TOL = 1e-14     # (local) structural-zero tolerance
    COUPLING_TOL = 1e-12 # (local) structural-zero tolerance for ||C_eig||_F
    KLT_FLOOR = 1e-3     # (local) nontriviality floor for K_Ltilde (empirical:
                         #         |tau|=0.10 gives ||K_Lt||=3.8e-2, so
                         #         floor=1e-3 tests the Paper 18 correction
                         #         is operative across the full grid tau>=0.10)
    H1_pass = max_Lg_u2_norm < NORM_TOL                       # (local)
    H0_pass = max_K_LX_norm < NORM_TOL                        # (local) K_LX vanishes
    H2_pass = (min_K_Ltilde_pos_tau != float('inf') and
               min_K_Ltilde_pos_tau > KLT_FLOOR)               # (local)
    H3_pass = max_C_inter_norm < COUPLING_TOL                 # (local)
    all_pass = H0_pass and H1_pass and H2_pass and H3_pass    # (local)

    # Stdout 4-tuple tag (penultimate non-verdict line)
    print("\n=== GATE T3-S22B-KOSMANN-MATRIX: VALUE 4-TUPLE ===")
    print(f"  value = max||Lg||_u2={max_Lg_u2_norm:.6e}, "
          f"max||K||_LX={max_K_LX_norm:.6e}, "
          f"min||K||_Ltilde_posTau={min_K_Ltilde_pos_tau:.6e}, "
          f"max||C_eig||_interSector={max_C_inter_norm:.6e}")
    print(f"  scheme = Kosmann_spinor_LX_and_Ltilde_Baptista_P17-P18")
    print(f"  convention = KO-dim=6_Cliff8_ON-frame_PW-decomp")
    print(f"  L_max = L_max=max_pq_sum=3_(sectors_00_10_01_11_20_02)")
    print(f"  tol: NORM_TOL={NORM_TOL:.2e}, COUPLING_TOL={COUPLING_TOL:.2e}, "
          f"KLT_FLOOR={KLT_FLOOR:.2e}")
    print(f"  H0 (K_LX=0):     {'PASS' if H0_pass else 'FAIL'}   "
          f"max||K_LX||     = {max_K_LX_norm:.6e}  vs  NORM_TOL = {NORM_TOL:.0e}")
    print(f"  H1 (u2 Killing): {'PASS' if H1_pass else 'FAIL'}   "
          f"max||Lg_u2||    = {max_Lg_u2_norm:.6e}  vs  NORM_TOL = {NORM_TOL:.0e}")
    print(f"  H2 (Ltilde act): {'PASS' if H2_pass else 'FAIL'}   "
          f"min||K_Ltilde|| = {min_K_Ltilde_pos_tau:.6e}  vs  KLT_FLOOR = {KLT_FLOOR:.0e}")
    print(f"  H3 (block-diag): {'PASS' if H3_pass else 'FAIL'}   "
          f"max||C_inter||  = {max_C_inter_norm:.6e}  vs  COUPLING_TOL = {COUPLING_TOL:.0e}")
    print(f"  pedagogical samples: {gate_diag_samples}")
    print(f"\n  closure SHA-256 = {closure_sha}")

    verdict_str = "PASS" if all_pass else "FAIL"  # (local)
    value_str = (f"maxLg_u2={max_Lg_u2_norm:.3e},"
                 f"maxKLX={max_K_LX_norm:.3e},"
                 f"minKLt={min_K_Ltilde_pos_tau:.3e},"
                 f"maxCinter={max_C_inter_norm:.3e}")  # (local)
    verdict_line = (f"T3-S22B-KOSMANN-MATRIX: {verdict_str} -- "
                    f"value={value_str} "
                    f"scheme=Kosmann_spinor_LX_and_Ltilde_Baptista_P17-P18 "
                    f"convention=KO-dim=6_Cliff8_ON-frame_PW-decomp "
                    f"L_max=L_max=max_pq_sum=3_(sectors_00_10_01_11_20_02) "
                    f"sha256={closure_sha}")  # (local)
    print("\n" + verdict_line)

    # Also emit to a sidecar file (the canonical verdict file is written by
    # the Level-3 orchestrator; this is a redundant copy for human inspection).
    sidecar = os.path.join(SCRIPT_DIR, "s22b_gate_verdict.txt")  # (local)
    with open(sidecar, "w") as fh:
        fh.write(verdict_line + "\n")
    print(f"Sidecar verdict written: {sidecar}")
