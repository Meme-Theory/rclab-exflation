...
Input: computations/session-23/s23a_kosmann_singlet.npz, s32a_umklapp_vertex.npz
Output: computations/session-32/s32a_flat_b2_interaction.npz, .png
...
ax.fill_between(taus_computed, 0, np.real(V_eff_proper_vs_tau),
                ...
                alpha=0.2, color='blue', label='ATTRACTIVE')  # (local)
ax.fill_between(taus_computed, 0, np.real(V_eff_proper_vs_tau),
                where=(np.real(V_eff_proper_vs_tau) > 0),
                alpha=0.2, color='red', label='REPULSIVE')  # (local)
ax.set_xlabel(r'$\tau$')
