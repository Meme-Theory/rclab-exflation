...
riemann = np.load('computations/_shared/r20a_riemann_tensor.npz', allow_pickle=True)
fiber = np.load('computations/session-23/s23c_fiber_integrals.npz', allow_pickle=True)
kk = np.load('computations/session-25/s25_kk_workshop.npz', allow_pickle=True)
einstein = np.load('computations/session-25/s25_einstein_results.npz', allow_pickle=True)
eigdata = np.load('computations/session-23/s23a_eigenvectors_extended.npz', allow_pickle=True)
...
for i in range(21):
    total = 5 * R_scalar[i]**2 - 2 * Ric_sq_fiber[i] + 2 * K_kretschner[i]  # (local)
...
for idx in test_indices:
    ...
    unique_eigs = []
    tol = 1e-8 * (np.max(np.abs(eigvals)) + 1e-15)  # (local)
...
results = {
    ...
    '[SP]S-5': {
        ...
        'status': 'DEFERRED',
        'result': 'Level 3 theoretical. Not computable this session.',
        'impact': 'N/A'
    },
    '[SP]Q-1': {
...
# Save results
np.savez('computations/session-25/s25_sp_results.npz',
    tau_21=tau_21,
    tau_9=tau_9,
    Weyl_sq=Weyl_sq,
    Ric0_sq=Ric0_sq,
...
print("\nResults saved to computations/session-25/s25_sp_results.npz")
print("DONE.")
