...
from canonical_constants import *   # M_Pl_reduced, Lambda_obs_MP4, M_KK, rho_Lambda_obs, ...

import numpy as np

# =============================================================================
# LOAD ALL DATA (T3 re-run — inputs resolved to computations/_shared)
# =============================================================================

_ARCHIVE = Path(__file__).resolve().parent   # (local) path to this script's dir (computations/_shared)

kk = np.load(_ARCHIVE / 's25_kk_workshop.npz', allow_pickle=True)
fi = np.load(_ARCHIVE / 's23c_fiber_integrals.npz', allow_pickle=True)
ba = np.load(_ARCHIVE / 's25_baptista_results.npz', allow_pickle=True)
cn = np.load(_ARCHIVE / 's25_connes_results.npz', allow_pickle=True)
...
R_K_growth = R_K_9 / R_K_9[0]                     # (local)
omega3_growth = omega3_9 / omega3_9[0]            # (local)
a4_growth = a4_geom_9 / a4_geom_9[0]              # (local)
...
dim_spinor_P = 64  # (local) 12-dimensional total space; 2^{12/2}=64 is a representation-theoretic count, not a framework scale
...
gauge_to_curvature_ratio = (0.25 * omega3_9) / R_K_9   # (local)
...
R_Kerner_total_9 = R_K_9 + 0.25 * omega3_9    # (local)
R_Kerner_fiber_9 = R_K_9                       # (local)
...
tau = tau_21                   # (local)
R_K = fi['R_scalar']           # (local) ours normalization
omega3 = fi['omega_sq']        # (local)
a4_g = fi['a4_geom']           # (local)

# Fiber-only a_2 and a_4 (from Connes)
a2_fiber = cn['a2_values']     # (local) (20/3) * R_ours
a4_fiber = cn['a4_values']     # (local) Gilkey formula evaluated
...
rho_spec = a4_fiber[0] / a2_fiber[0]    # (local)
...
dR_dtau = np.gradient(R_K, tau)              # (local)
domega3_dtau = np.gradient(omega3, tau)       # (local)
da4_dtau = np.gradient(a4_g, tau)             # (local)
da2_dtau = np.gradient(a2_fiber, tau)         # (local)
...
rho_physical = 0.001   # (local) V_spec uses rho = f_0/(f_2*Lambda^2), typically very small
...
a2_norm = a2_fiber - a2_fiber[0]           # (local)
a4_norm = a4_fiber - a4_fiber[0]           # (local)
omega3_norm = omega3 - omega3[0]           # (local)

# Scan gamma from 0 to 5 and rho from 0 to 2
gamma_range = np.linspace(0, 5.0, 501)     # (local)
rho_range = np.linspace(0, 2.0, 201)       # (local)

# For each (gamma, rho), check if V_mixed has a minimum in tau in (0, 2.0]
min_results = []                                                              # (local)
has_min_map = np.zeros((len(gamma_range), len(rho_range)), dtype=bool)       # (local)
tau_min_map = np.full((len(gamma_range), len(rho_range)), np.nan)            # (local)
depth_map = np.full((len(gamma_range), len(rho_range)), np.nan)              # (local)
...
# Report
n_with_min = np.sum(has_min_map)       # (local)
total_pts = has_min_map.size            # (local)
...
RK_F2 = R_K_9 * omega3_9          # (local) R_K * |omega_3|^2
F4 = omega3_9**2                   # (local) |omega_3|^4
...
d_a4_fiber = np.diff(a4_geom_9)              # (local)
d_RK_F2 = np.diff(RK_F2)                      # (local)
d_F4 = np.diff(F4)                            # (local)
d_a2_total = np.diff(R_Kerner_total_9)        # (local) a_2^{total} ~ R_K + (1/4)|F|^2
...
a4_total_naive = np.zeros(len(tau_21))                            # (local)
for i in range(len(tau_21)):
    R_total = R_K[i] + 0.25 * omega3[i]                            # (local)
    a4_total_naive[i] = (64/360) * (5 * R_total**2 - 2 * Ric_sq_21[i] + 2 * K_21[i])

# The FIBER-ONLY a_4 (Connes):
a4_fiber_check = np.zeros(len(tau_21))                             # (local)
for i in range(len(tau_21)):
...
a2_total = (64/6) * (R_K + 0.25 * omega3)         # (local)
V_eff_mixed = a2_total + rho_spec * a4_total_naive  # (local)

# Normalize
V_eff_mixed_norm = V_eff_mixed - V_eff_mixed[0]    # (local)
...
dR_K_F2_21 = np.gradient(R_K * omega3, tau_21)    # (local)
da2_21 = np.gradient(a2_fiber, tau_21)             # (local)
da4_f_21 = np.gradient(a4_fiber, tau_21)           # (local)
...
a0_exact = 11424           # (local) total eigenvalues at max_pq=6 truncation (spectral count, not framework constant)
a2_exact = a2_connes       # (local)
a4_exact = a4_connes       # (local)
a4_over_a2_exact = cn['a4_over_a2']   # (local)

print(f"\na_0 (eigenvalue count) = {a0_exact} (constant)")
print(f"\na_0/a_2 ratio (measures cosmological-to-curvature scale):")
ratio_a0_a2 = a0_exact / a2_exact    # (local)
for i in range(0, len(tau_21), 2):
...
ratio_a2_a4 = a2_exact / a4_exact    # (local)
...
lambda_min_0 = 0.8333    # (local) at tau=0 (spectral truncation result, not a framework constant)
lambda_min_025 = 0.8186  # (local) at tau=0.25 (spectral truncation result)
F_0 = lambda_min_0**2            # (local)
F_min = lambda_min_025**2        # (local)
delta_F = F_0 - F_min            # (local)
depth_pct = 100 * delta_F / F_0  # (local)
...
gap_sq = 0.84**2 - 0.8333**2                          # (local) spectral gap squared in truncated D_K spectrum
beta_c = 1.0 / gap_sq if gap_sq > 0 else float('inf') # (local) BEC-analog condensation inverse-temperature
...
_OUTPUT_NPZ = _ARCHIVE / 's25_einstein_results.npz'   # (local) output location
np.savez(_OUTPUT_NPZ,
...
print(f"Results saved to {_OUTPUT_NPZ}")
