...
# S81 Level 3 canonical re-run ------------------------------------------------
# Gate: T3-S36-MULTISECTOR-ED
# Trigger: [VERIFY-THEOREM] (reproducibility of E_cond_ED_8mode under PRU+GPU)
# Classification: PHONONIC (BCS pairing in B2/B1/B3 positive-eigenvalue sector
#                 of the Kosmann kernel at tau=0.20, near fold_tau=0.190)
# Target: E_cond_ED_8mode = -0.13685055970476342 (MCP-registered, this script
#         is the canonical producer; re-run closes the self-referential loop
#         and emits a closure SHA for the S81+ verdict schema)
# Tolerance: THEOREM -> machine-epsilon (|reproduced - target|/|target| < 1e-12)
# Machinery pin (PRDR):
#   TAU_INDEX=3 (tau=0.20), ETA_REG=1e-3 (Thouless regulator; DO-NOT-PROMOTE),
#   N_POS_MAX=8 (positive sector: 1 B1 + 4 B2 + 3 B3), N_STATES=2**8=256
#   (Fock pair-occupation basis), N_EFF_B2=4 (bookkeeping for expansion param;
#   DO-NOT-PROMOTE), selection rules V(B1,B1)=0 and V(B1,B3)=0 inherited from
#   the Kosmann kernel (S23a, Trap 1) and carried forward here as structural.
# GPU path: N<=256 real-symmetric; numpy.linalg.eigh with OMP_NUM_THREADS=8 is
#   the chosen path (torch would add 4x device-hop with no wall-time payoff
#   below ~1000; this is the exception-clause in heavy-linear-algebra rule).
# ---------------------------------------------------------------------------
...
from canonical_constants import E_cond_ED_8mode, E_cond_ED_5mode, tau_fold

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
ARCHIVE_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), 'computations/_shared')  # (local) static .npz inputs live in computations/_shared
t0 = time.time()  # (local)

print("=" * 78)
print("Session 36: ED-CONV-36 -- Multi-Sector ED Convergence (S81 re-run)")
print("=" * 78)
...
DPHYS_PATH    = os.path.join(ARCHIVE_DIR, 's34a_dphys_kosmann.npz')         # (local)
VH_ARB_PATH   = os.path.join(ARCHIVE_DIR, 's35a_vh_impedance_arbiter.npz')  # (local)
S35_ED_PATH   = os.path.join(ARCHIVE_DIR, 's35_ed_corrected_dos.npz')       # (local)
CANON_PATH    = os.path.join(SCRIPT_DIR, 'canonical_constants.py')          # (local)


def _sha256_of_file(path):
    """Stream SHA-256 of a file; used for S81 closure-hash provenance."""
    h = hashlib.sha256()
...
_INPUT_PINS = {  # (local) S81 input-pin map used in the closure hash
    'script':              _sha256_of_file(os.path.abspath(__file__)),
    'canonical_constants': _sha256_of_file(CANON_PATH),
    's23a_kosmann':        _sha256_of_file(KOSMANN_PATH),
    's34a_dphys_kosmann':  _sha256_of_file(DPHYS_PATH),
    's35a_vh_arbiter':     _sha256_of_file(VH_ARB_PATH),
    's35_ed_corrected':    _sha256_of_file(S35_ED_PATH),
}
_CLOSURE_SHA = hashlib.sha256(                                               # (local)
    json.dumps(dict(sorted(_INPUT_PINS.items())), sort_keys=True).encode()
).hexdigest()

print("\nS81 INPUT PINS (SHA-256):")
for _k, _v in _INPUT_PINS.items():
    print(f"  {_k:<22s} : {_v}")
print(f"  closure_inputs (SHA-256 of sorted pin map): {_CLOSURE_SHA}")
print(f"  tau_fold (canonical) = {tau_fold}")
print(f"  E_cond_ED_8mode target  = {E_cond_ED_8mode}")
print(f"  E_cond_ED_5mode (legacy target) = {E_cond_ED_5mode}")

kosmann = np.load(KOSMANN_PATH, allow_pickle=True)
dphys_kosmann = np.load(DPHYS_PATH, allow_pickle=True)
vh_arbiter = np.load(VH_ARB_PATH, allow_pickle=True)
s35_ed = np.load(S35_ED_PATH, allow_pickle=True)

ETA_REG = 0.001      # (local) Thouless regulator; DO-NOT-PROMOTE (canonical_constants L634 exclusion list)
TAU_INDEX = 3        # (local) tau = 0.20, near fold_tau = 0.190
N_POS_MAX = 8        # (local) positive-eigenvalue sector: 1 B1 + 4 B2 + 3 B3
N_EFF_B2 = 4         # (local) flat-optical B2 count for expansion-parameter bookkeeping
ti = TAU_INDEX       # (local) back-compat alias retained for minimal diff
...
def exact_diag_bcs_general(V_matrix, E_vec, mu_val, rho_v, branch_labels,
    ...
    N_eff_B2 = 4  # (local) effective B2 modes for expansion parameter
    epsilon_mf = 1.0 - M_max_mf  # (local)
        ...
        for n in range(n_modes):
            for m in range(n_modes):
                ...
                if n == m:
                    ...
                    continue
                corr = 0.0  # (local) accumulator for <b_n^dag b_m>
                for state in range(n_states):
                    if (state & (1 << m)) and not (state & (1 << n)):
                        new_state = state ^ (1 << m) ^ (1 << n)
    ...
    if paired:
        for m in range(n_modes):
            b_m = 0.0  # (local) accumulator for anomalous <b_m>
            for state in range(n_states):
                if state & (1 << m):
                    partner = state ^ (1 << m)
...
mu = 0.0  # (local) particle-hole symmetric chemical potential
...
ax.plot(range(len(n_modes_seq)), M_maxs_seq, 'o-', color='steelblue',
        lw=2, ms=10, label='M_max (MF)')  # (local)
