...
d45 = np.load('computations/session-45/s45_qtheory_bcs.npz', allow_pickle=True)
d_pbcs = np.load('computations/session-46/s46_number_projected_bcs.npz', allow_pickle=True)
d_vb3 = np.load('computations/session-46/s46_v_b3b3.npz', allow_pickle=True)
...
ax.fill_between(B2_fine, mu_fine - 2*sig_fine, mu_fine + 2*sig_fine,
                alpha=0.15, color='b', label=r'$\pm 2\sigma_{GP}$')  # (local)
ax.fill_between(B2_fine, mu_fine - sig_fine, mu_fine + sig_fine,
                alpha=0.3, color='b', label=r'$\pm 1\sigma_{GP}$')  # (local)
...
ax.fill_between(lims, [l - loo_rmse for l in lims], [l + loo_rmse for l in lims],
                alpha=0.15, color='red', label=f'RMSE = {loo_rmse:.4f}')  # (local)
...
plt.savefig('computations/session-46/s46_bayesian_gp.png', dpi=150, bbox_inches='tight')
print("  Saved: computations/session-46/s46_bayesian_gp.png")
...
np.savez('computations/session-46/s46_bayesian_gp.npz',
         # Central result
...
print("  Saved: computations/session-46/s46_bayesian_gp.npz")
