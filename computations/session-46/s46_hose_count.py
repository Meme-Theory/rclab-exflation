...
z_bog = 2.024   # Bogoliubov dynamical critical exponent  # (local)
nu_3dxy = 0.6301  # 3D XY correlation length exponent  # (local)
    ...
    if r['dim'] > 1 and r['k'] > 0:
        ...
        if len(strengths) > 1:
            ax.bar(np.arange(len(strengths)) + 0.1 * r['idx'],
                   ...
                   width=0.08, label=f"({r['p']},{r['q']}), dim={r['dim']}")  # (local)
