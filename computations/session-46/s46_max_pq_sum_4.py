...
Input:
  - dirac_spectrum.py (spectrum computation infrastructure)
...
a0_old = 0.0  # max_pq_sum=3  # (local)
a2_old = 0.0  # (local)
a4_old = 0.0  # (local)
a0_new = 0.0  # max_pq_sum=4  # (local)
a2_new = 0.0  # (local)
a4_new = 0.0  # (local)
...
ax.bar(bin_centers, dos_new - dos_old, width=bin_width*0.9, bottom=dos_old,
       alpha=0.5, color='crimson', label=f'new at pq sum = 4 (+{total_new_evals})')  # (local)
