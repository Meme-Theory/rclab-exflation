...
def measure_singlet_entropy(H_singlet, H_norm, epsilon, n_samples, compute_r=True):
    ...
    for _ in range(n_samples):
        ...
        S = entanglement_entropy_4x4(psi)  # (local)
        S_values.append(S)
...
print("  Saved: computations/session-46/s46_dissolution_singlet.npz")
...
ax1.axhline(S_PAGE_SINGLET, color='blue', linestyle='--', linewidth=1,
            alpha=0.5, label=f'$S_{{\\rm Page}}$ = {S_PAGE_SINGLET:.3f}')  # (local)
ax1.axhline(S_MAX_SINGLET, color='blue', linestyle=':', linewidth=0.8,
            alpha=0.3, label=f'$S_{{\\rm max}}$ = {S_MAX_SINGLET:.3f}')  # (local)
...
print("  Saved: computations/session-46/s46_dissolution_singlet.png")
