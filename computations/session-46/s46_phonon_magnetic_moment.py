...
v_transit = 26.545  # M_KK units (from canonical_constants)  # (local)
...
ax5.fill_between(omega_centers, dos_unweighted / np.max(dos_unweighted + 1e-10),
                 alpha=0.3, color='steelblue', label='Unweighted DOS')  # (local)
if np.max(dos_berry) > 0:
    ax5.fill_between(omega_centers, dos_berry / np.max(dos_berry + 1e-10),
                     alpha=0.5, color='crimson', label='Berry-weighted DOS')  # (local)
...
Gate: PHONON-MAGNETIC-MOMENT-46 (DIAGNOSTIC/INFO)
Output: computations/session-46/s46_phonon_magnetic_moment.{{npz,png}}
""")
