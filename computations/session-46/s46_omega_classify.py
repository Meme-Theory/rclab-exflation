...
Date: 2026-03-15
Depends on: s45_weak_order_one.py (A_F infrastructure), dirac_spectrum.py
...
# Also add parent computations/_shared/ for canonical_constants + dirac_spectrum
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from canonical_constants import *
TAU_FOLD_CANON = tau_fold  # (local) alias
...
if __name__ == '__main__':
    ...
    print("=" * 78)
    print("OMEGA-CLASSIFY-46: Inner Fluctuation Module Classification (re-run)")
    print("=" * 78)
    print()

    # ---- S81 SHA-256 INPUT PINS (first 20 lines of stdout per gate-verdicts.md) ----
    import hashlib
    _input_pins = {
        "t3-intake/s46_omega_classify.py": "fdb95e8df0955ae067a0e631f4bed00e880bb975b4052e04fc0997d1230c5667",
    ...
    _concat = ""  # (local) ordered concat for closure
    for _k, _v in _input_pins.items():
        print(f"  {_v}  {_k}")
        _concat += f"{_k}={_v};"
    CLOSURE_SHA = hashlib.sha256(_concat.encode()).hexdigest()  # (local)
    print(f"CLOSURE_SHA: {CLOSURE_SHA}")
    print(f"TAU_FOLD_CANON: {TAU_FOLD_CANON}")
    print()
    ...
    tau_round = 0.00  # (local)
    tau_fold_val = TAU_FOLD_CANON  # (local) canonical fold from canonical_constants

    # Primary sector: (1,0) fundamental
    p_sector, q_sector = 1, 0  # (local)
    ...
    if omega_fold['dim_extra'] > 0:
        ...
        dim_ext_even_f = 0  # (local)
        dim_ext_odd_f = 0  # (local)
    ...
    if omega_round['dim_extra'] > 0:
        ...
        dim_ext_even_r = 0  # (local)
        dim_ext_odd_r = 0  # (local)
    ...
    if dim_comb_odd_r2 > 0:
        ...
        n_tachyonic_round = 0  # (local)
        n_massless_round = 0  # (local)
