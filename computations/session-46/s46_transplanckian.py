...
diss = np.load(os.path.join(SCRIPT_DIR, "s44_dissolution_scaling.npz"), allow_pickle=True)
...
k_lmax_Mpc = 0.25  # k at l ~ 2500  # (local)
...
Lambda_UV = 1.0  # M_KK (the natural cutoff)  # (local)
...
np.savez(os.path.join(SCRIPT_DIR, "s46_transplanckian.npz"), **save_dict)
print(f"\nData saved to: {os.path.join(SCRIPT_DIR, 's46_transplanckian.npz')}")
...
plt.tight_layout()
plt.savefig(os.path.join(SCRIPT_DIR, "s46_transplanckian.png"), dpi=150, bbox_inches='tight')
print(f"Plot saved to: {os.path.join(SCRIPT_DIR, 's46_transplanckian.png')}")
