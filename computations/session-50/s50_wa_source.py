...
# In M_KK units:
t_eq_s = 1.58e12  # matter-radiation equality  # (local)
t_Lambda_s = 3.1e17  # Lambda-matter equality  # (local)
...
Delta_tau_generous = 0.01  # 1% change in tau (unrealistic but illustrative)  # (local)
Delta_tau_physical = 0.0   # Frozen modulus  # (local)
...
wa_b_final = 0.0  # w_a = 0: oscillation averages to zero, thermalization wrong direction  # (local)
...
def compute_alpha_w0(eps_k_vals, lambda_k_vals):
    ...
    E = 2.0 * np.sum(n_k * eps_k_vals)  # (local)
...
wa_c_physical = 0.0  # Frozen modulus  # (local)
...
wa_e_physical = 0.0  # Per-fiber -> no dilution  # (local)
