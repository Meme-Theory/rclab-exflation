...
d37 = np.load('computations/session-37/s37_pair_susceptibility.npz', allow_pickle=True)
d38 = np.load('computations/session-38/s38_otoc_bcs.npz', allow_pickle=True)
d39 = np.load('computations/session-39/s39_richardson_gaudin.npz', allow_pickle=True)
...
np.savez('computations/session-39/s39_odd_blocking.npz', **save_dict)
print("  Saved: computations/session-39/s39_odd_blocking.npz")
...
ax.axhline(y=omega_plus_s37, color='blue', linewidth=1.5, linestyle='--',
           alpha=0.6, label=f'Pair add: {omega_plus_s37:.3f}')  # (local)
# Odd states
for k in range(N):
    dE = results[k]['delta_E']
...
plt.tight_layout()
plt.savefig('computations/session-39/s39_odd_blocking.png', dpi=150, bbox_inches='tight')
print("  Saved: computations/session-39/s39_odd_blocking.png")
