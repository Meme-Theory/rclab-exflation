...
wa_fw      = wa_free       # -0.009
sig_wa_fw  = 0.02          # Conservative: framework allows |w_a| < 0.02  # (local)
...
sig_w0_dr3 = 0.035  # Expected DR3 precision  # (local)
sig_wa_dr3 = 0.15   # Roughly proportional shrinkage  # (local)
...
w0_dr3_favorable = -0.6  # Near Keldysh edge  # (local)
...
ax.fill_between(w0_dr3_scan, 0, 1, where=B_dr3_map < 0.01,
                alpha=0.2, color='red', label='Decisive exclusion (B<1/100)')  # (local)
ax.fill_between(w0_dr3_scan, 0, 1, where=(B_dr3_map >= 0.01) & (B_dr3_map < 0.1),
                alpha=0.2, color='orange', label='Strong exclusion (B<1/10)')  # (local)
ax.fill_between(w0_dr3_scan, 0, 1, where=(B_dr3_map >= 0.1) & (B_dr3_map < 1),
                alpha=0.2, color='yellow', label='INFO (1/10<B<1)')  # (local)
ax.fill_between(w0_dr3_scan, 0, 1, where=B_dr3_map >= 1,
                alpha=0.2, color='green', label='Framework preferred (B>1)')  # (local)
