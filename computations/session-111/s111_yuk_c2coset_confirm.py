#!/usr/bin/env python3
"""
S111-CF-YUK-C2COSET-CONFIRM  Confirmation witness: the C^2-coset transverse
left-invariant modulus does NOT lift the d=2 generation degeneracy
================================================================================

Gate: S111-CF-YUK-C2COSET-CONFIRM ([SIGN]); track = session (session 111)
Classification: PARTICLE
Agent: baptista-spacetime-analyst (executor)
Plan: sessions/session-plan/session-111-plan-w3.md SS W3-3

PURPOSE (down-tiered confirmation, NOT a live discriminator)
------------------------------------------------------------
This gate is the CONFIRMATION-WHEN-CONVENIENT numerical witness down-tiered from
S110-CF1-YUK-C2COSET (which closed FAIL, |dY_12/d delta|_0 = 8.726920e-16, a
~13-OOM-below-eps_lift Schur-zero). It re-runs the SAME observable at the SAME
L_max=10 (bit-comparable) and CONFIRMS that the rank-1 Yukawa wall is off-ALL
left-invariant internal moduli -- i.e. it converts "the C^2-coset is the surviving
open seam" -> "closed like every other internal modulus." It folds into the same
Yukawa-sector compute window as YUK-FULLFLAVOR; no standalone dispatch overhead.
...
THE C^2-COSET TRANSVERSE DEFORMATION (the modulus this gate re-tests):
...
=> det g_K = 1 preserved EXACTLY, overall fiber volume held fixed per G6). It is
the 4-bond analog of the su(2) 3-bond split (2 delta - delta - delta = 0). It
breaks the C^2-coset's internal isotropy while keeping the metric LEFT-INVARIANT
and BLOCK-DIAGONAL by Peter-Weyl (BLOCK-DIAG-GENERAL-61/S61). The C^2-coset is the
DOMINANT directional stiffness (J_C2=0.933) -- the direction Baptista's
O'Neill/Riemannian-submersion analysis flags as carrying inter-generation
anisotropy.

THE OBSERVABLE (Bridge-1; identical to S110-CF1 / INV2-W1-1):
...
degenerate Dirac eigenvectors within the fundamental (1,0) Peter-Weyl sector
(|lambda|=0.8409). The Yukawa splitting block is Y(delta) = V_g^dag (1j D(delta))
V_g on the FIXED delta=0 degenerate-multiplet basis V_g (d x d Hermitian).
Y(0) = lambda I_d EXACTLY (Schur); its OFF-DIAGONAL and eigenvalue SPREAD are the
degeneracy-lift indicators.

THE TEST (rank-1-wall lift; [SIGN]; INVERTED gate-PASS direction; SAME thresholds
as S110-CF1 / INV2-W1-1):
  PASS  = Reading-A: |dY_12/d delta|_0 > eps_lift=1e-3 AND rank(Y_ij) 1 -> >=2 for
          ...
          STAGE-3-PERMANENT VII.BL Generation-Blindness theorem -> a FLAGGED
          Stage-2 CONTRADICTION re-audit (joint-theorem-promotion.md), NOT a quiet
          "Reading A wins". Dual prior 0.90 Track A on PASS.
  FAIL  = Reading-B: |dY_12/d delta|_0 <= eps_lift across the scan AND no rank
          increase -- the wall is genuine off the C^2-coset too; VII.BL CONFIRMED
          on ALL left-invariant internal moduli; hierarchy PINNED to external
          eps_LX. (Predicted; 0.95 Track B; S110-CF1 returned 8.727e-16.)
  INFO  = 0 < |dY_12/d delta|_0 <= eps_lift (sub-threshold non-zero lift) OR the
          regime sub-verdict is MARGINAL/BREAKDOWN; priors UNCHANGED; route to
          higher L_max.
...
Peter-Weyl sector (fund (1,0), t=1); an intra-sector generation-mixing operator O
connecting the two d=2 copies has t(a)=t(b)=1 => required t(O)=(t(a)-t(b)) mod 3
= 0 (triality-EVEN, admissible=True NECESSARY-only). BUT the C^2-coset split is a
LEFT-INVARIANT metric deformation: dD_K/d delta in Omega^1_{D_K}(A_K) acts on the
IRREP leg V_{(p,q)} and (x)1 on the multiplicity leg, so by VII.BL it is
multiplicity-SCALAR on each C^{m(p,q)} and its projection onto the inter-generation
(multiplicity-off-diagonal) channel is the commutant projection = 0 EXACTLY. The
admissibility check is SATISFIED but is NECESSARY-not-sufficient; the actual wall is
LEG-MEMBERSHIP, matching the S110-CF1 verdict-row note. The compute VERIFIES the
numerical zero directly via |dY_12/d delta|_0.

SUBSTITUTION CHAIN (math-scripts.md, [SIGN]): see compute() / the
SUBSTITUTION-CHAIN print block (numbers substituted at runtime).

BIT-COMPARABILITY (this gate's confirmation contract):
At L_max=10 the physics is IDENTICAL to S110-CF1. The compute loads the S110-CF1
baseline npz (s110_cf1_yuk_c2coset.npz) and cross-checks the recomputed
|dY_12/d delta|_0 and Schur-zero against it (the confirmation = bit-level match
to the down-tiered source). A bit-mismatch flags a machinery drift (NOT a physics
finding) and is recorded in the verdict.

DISCIPLINE
----------
- from canonical_constants import *  (tau_fold, M_KK, J_C2, J_su2 consumed)
- dirac_spectrum.py machinery reused (the EXACT S110-CF1 / INV2-W1-1 chain); the
  C^2-coset transverse helper deformed_c2coset_split_metric is authored HERE
  (identical to the S110-CF1 helper, the 4-bond analog of inv-2's
...
- dual-SHA (S84+): audit = sha256(script || canonical || pinmap_json);
  content = sha256(script). Per-gate identity keys embedded in the pinmap so
  audit_sha256 is gate-unique vs S110-CF1 (sig_5).
- verdict PRINTED as an emit_verdict payload (print_verdict_payload); the
  dispatching agent calls mcp__knowledge__emit_verdict(session=111, track="session").
  NO open("a") append.
...
# ---------------------------------------------------------------------------
# Section 3 -- Pre-registration (machinery pin map; plan SS W3-3)
# ---------------------------------------------------------------------------
SESSION = "111"                                                    # (local) session 111
GATE_ID = "S111-CF-YUK-C2COSET-CONFIRM"                            # (local)
SCHEME = "off-U(2)-C2coset-split-Yukawa-overlap"                   # (local)
CONVENTION = ("deformed-L3.I4-split-metric-C2coset-4bonds-"
              "JC2-0.9330-genmult-d2-Lmax10-S110CF1-bitcomparable")  # (local)
L_MAX = "10"                                                       # (local) S110-CF1 bit-comparable

# Pre-registered thresholds (plan SS W3-3 operator block; same as S110-CF1)
...
DELTA_HI = 0.20            # delta scan high (~tau_fold scale); plan (0,0.20] # (local)
N_DELTA = 41               # delta scan points (S110-CF1: 41-pt grid)    # (local)
MAX_PQ_SUM = 10            # L_max canonical Peter-Weyl truncation (S110-CF1 EXACT) # (local)
...
BITCOMPARE_TOL = 1.0e-6    # |dY_12/d delta|_0 bit-comparison vs S110-CF1 baseline # (local)

# Generation sector: the fundamental (1,0). The lowest distinct |lambda| same-sign
# degenerate multiplet (the generation copies on the U(2) surface) is the Yukawa
# block whose Schur-degeneracy this gate tests for an off-surface lift.
...
OUT_NPZ = SESSION_DIR / "s111_yuk_c2coset_confirm.npz"
OUT_PNG = SESSION_DIR / "s111_yuk_c2coset_confirm.png"

# S110-CF1 baseline (the down-tiered source; loaded for the bit-comparison)
S110_CF1_NPZ = COMPUTATIONS_DIR / "session-110" / "s110_cf1_yuk_c2coset.npz"
...
MACHINERY_PIN_MAP = {                                              # (local)
    "_gate_id": GATE_ID,
    "_wp_id": "session-111-w3-workingpaper.md#W3-3",
    "_scheme": SCHEME,
    ...
    "J_C2": str(J_C2),
    "J_su2": str(J_su2),
    "C2_IDX": str(list(C2_IDX)),       # the SPLIT block (this gate, = S110-CF1)
    "gen_sector": str(GEN_SECTOR),
    "fd_step": str(FD_STEP),
    "rank_tol": str(RANK_TOL),
    "down_tiered_from": "S110-CF1-YUK-C2COSET",
    "bitcompare_tol": str(BITCOMPARE_TOL),
}
...
def compute_dual_sha(script_path: Path, canonical_path: Path,
    ...
    audit_sha256 is gate-unique (sig_5; distinct from S110-CF1)."""
...
# ---------------------------------------------------------------------------
# Section 5 -- C^2-COSET block-split helper (IDENTICAL to S110-CF1)
# ---------------------------------------------------------------------------
...
def deformed_c2coset_split_metric(B_ab, L1, L2, L3, delta):
    ...
    This is the 4-bond analog of inv-2's deformed_su2_split_metric (3-bond su(2)
    block) and is BYTE-IDENTICAL to the S110-CF1 helper. The base metric is the
    U(2)-invariant Jensen metric u2_invariant_metric; this helper REPLACES the
    diagonal C^2 entries (C2_IDX = [3,4,5,6]) with the split values, leaving u(1)
    (L1) and su(2) (L2) untouched. The C^2-coset is the DOMINANT directional
    stiffness (J_C2=0.933, canonical_constants.py:734).
    ...
    SU(2)xU(1) stabilizer) while keeping the metric LEFT-INVARIANT and hence D_K
    BLOCK-DIAGONAL by Peter-Weyl (BLOCK-DIAG-GENERAL-61/S61).
...
def assemble_Dk_split(delta, tau, B_ab, f_abc, gens, gammas, rho):
    ...
    metric (chain identical to S110-CF1):
...
# ---------------------------------------------------------------------------
#
# CORRECTED OBSERVABLE (the decisive INV2-W1-1 / S110-CF1 method lesson):
...
# straddle DIFFERENT |lambda| levels. The Schur theorem Y = lambda I (the rank-1
# wall) is a statement about ONE GENUINELY DEGENERATE multiplet: a same-sign set
# of states at a common |lambda|. We project the FULL Dirac operator
...
def select_generation_multiplet(evals_H, evecs):
    ...
    Dirac eigenstates among the lowest |lambda| levels (procedure IDENTICAL to
    S110-CF1 / INV2-W1-1):
      ...
      3. pick the LOWEST cluster of size >= 2 (the generation multiplet). If no
         ...
         positive levels (so the block is at least 2x2 and the test non-vacuous).
...
def yukawa_splitting_block(delta, V_g, tau, B_ab, f_abc, gens, gammas, rho):
    ...
    the FIXED delta=0 multiplet basis V_g (d x d Hermitian). Y(0) = lambda I_d
    EXACTLY (Schur). For delta>0, Y(delta) carries the off-diagonal Y_ij and the
    eigenvalue spread that signal the degeneracy lift."""
...
def selection_rule_preflight():
    ...
    (the generation index is the MULTIPLICITY leg C^{m(p,q)}, NOT a distinct
    irrep). An intra-sector generation-mixing operator O connecting the two copies
    has t(a)=t(b)=1 => required t(O) = (t(a)-t(b)) mod 3 = 0 (triality-EVEN). The
    C^2-coset split is a LEFT-INVARIANT metric deformation: dD_K/d delta in
    Omega^1_{D_K}(A_K) acts on the IRREP leg and (x)1 on the multiplicity leg, so
    its center character relative to the generation index is t(O) = 0.
    ...
    NOT forbid the matrix element by triality. The OBSTRUCTION is the deeper
    LEG-MEMBERSHIP fact: the operator's image lies in (+) B(V_{(p,q)}) (x) 1
    (Skolem-Noether closure), whose projection onto the multiplicity-leg commutant
    (+) 1_V (x) M_m(C) is EXACTLY ZERO. The compute therefore CANNOT be shortcut by
    triality; the numerical zero must come from the leg-membership structure, which
    the scan measures directly via |dY_12/d delta|_0.
    ...
    # required t(O) for an intra-sector (t(a)=t(b)) mixing element:
    t_O_required = (t_gen - t_gen) % 3                            # (local) =0 (triality-EVEN)
    admissible = (t_O_required == t_O)                           # (local) -> True
    ...
    print(f"  intra-sector required t(O) = (t(a)-t(b)) mod 3 = {t_O_required} "
          f"(triality-EVEN)")
    print(f"  C^2-coset split operator center character t(O) = {t_O} "
          f"(left-invariant metric => (x)1 on multiplicity leg, does NOT move t)")
    print(f"  admissibility t(O)_required == t(O): {t_O_required} == {t_O} -> "
          f"{admissible} (NECESSARY-not-sufficient)")
    ...
    print("  => compute is NOT shortcut by triality; the numerical zero is "
          "measured directly.")
    return dict(t_gen=int(t_gen), t_O=int(t_O), t_O_required=int(t_O_required),
                admissible=bool(admissible))


# ---------------------------------------------------------------------------
# Section 7c -- S110-CF1 baseline load (the down-tiered source bit-compare)
# ---------------------------------------------------------------------------

def load_s110_cf1_baseline():
    """Load the S110-CF1-YUK-C2COSET baseline npz (the down-tiered source). The
    confirmation contract: at L_max=10 the recomputed observable matches it to the
    bit-comparison floor BITCOMPARE_TOL. A mismatch flags a machinery drift (NOT a
    physics finding) and is recorded in the verdict."""
    if not S110_CF1_NPZ.exists():
        print(f"  WARNING: S110-CF1 baseline npz absent at {S110_CF1_NPZ.name}; "
              f"bit-comparison SKIPPED.")
        return None
    d = np.load(S110_CF1_NPZ, allow_pickle=True)                  # (local)
    base = dict(                                                  # (local)
        abs_dY12_d0=float(d["abs_dY12_d0"]),
        schur_offdiag0=float(d["schur_offdiag0"]),
        distinct0=int(d["distinct0"]),
        lam0=float(d["lam0"]),
        n_gen=int(d["n_gen"]),
        verdict=str(d["verdict"]),
        intra_split_max=float(np.asarray(d["intra_split"])[-1]),
        d3_S=float(d["d3_S"]),
    )
    print(f"  S110-CF1 baseline loaded: |dY_12/d delta|_0 = "
          f"{base['abs_dY12_d0']:.6e}, schur_offdiag0 = {base['schur_offdiag0']:.3e}, "
          f"distinct0 = {base['distinct0']}, lam0 = {base['lam0']:.6f}, "
          f"n_gen = {base['n_gen']}, verdict = {base['verdict']}.")
    return base


# ---------------------------------------------------------------------------
# Section 8 -- Compute (the full delta-scan + variations + bit-compare)
# ---------------------------------------------------------------------------
...
def compute(pins: dict) -> dict:
    ...
    # ---- S110-CF1 baseline (the down-tiered source; for the bit-comparison) ----
    print("=== S110-CF1-YUK-C2COSET baseline (down-tiered source) ===")
    baseline = load_s110_cf1_baseline()                           # (local)
    print()
    ...
    # ---- (5) Leading lift dY_12/d delta|_0 (one-sided 4th-order on off-diag) ----
    h_scan = deltas[1] - deltas[0]                              # (local)
    lift_curve = offdiag_max if n_gen > 1 else np.zeros(N_DELTA)  # (local)
    # one-sided 4th-order forward stencil at delta=0 (matches S110-CF1):
    ...
    # ---- (10) Bit-comparison vs the S110-CF1 down-tiered baseline ----
    if baseline is not None:
        d_lift = abs(abs_dY12_d0 - baseline["abs_dY12_d0"])     # (local)
        d_schur = abs(schur_offdiag0 - baseline["schur_offdiag0"])  # (local)
        d_lam = abs(lam0 - baseline["lam0"])                    # (local)
        bit_match = bool((d_lift < BITCOMPARE_TOL)
                         and (d_schur < BITCOMPARE_TOL)
                         and (d_lam < BITCOMPARE_TOL)
                         and (distinct0 == baseline["distinct0"])
                         and (n_gen == baseline["n_gen"]))      # (local)
        print(f"\nBit-comparison vs S110-CF1 baseline (L_max=10 confirmation "
              f"contract):")
        print(f"  |dY_12/d delta|_0: this {abs_dY12_d0:.6e} vs S110-CF1 "
              f"{baseline['abs_dY12_d0']:.6e}; |Delta| = {d_lift:.3e} "
              f"(< {BITCOMPARE_TOL}: {d_lift < BITCOMPARE_TOL})")
        print(f"  schur_offdiag0:    this {schur_offdiag0:.3e} vs S110-CF1 "
              f"{baseline['schur_offdiag0']:.3e}; |Delta| = {d_schur:.3e}")
        print(f"  lam0:              this {lam0:.6f} vs S110-CF1 "
              f"{baseline['lam0']:.6f}; |Delta| = {d_lam:.3e}")
        print(f"  distinct0:         this {distinct0} vs S110-CF1 "
              f"{baseline['distinct0']}; n_gen: this {n_gen} vs "
              f"{baseline['n_gen']}")
        print(f"  => bit-match (confirmation reproduces the down-tiered source): "
              f"{bit_match}")
    else:
        d_lift = float("nan")                                   # (local)
        bit_match = None                                        # (local) baseline absent
        print("\nBit-comparison SKIPPED (S110-CF1 baseline npz absent).")
    ...
    res.update(dict(
        ...
        baseline=baseline, bit_match=bit_match, d_lift_vs_baseline=d_lift,
        preflight_t_gen=preflight["t_gen"], preflight_t_O=preflight["t_O"],
        preflight_t_O_required=preflight["t_O_required"],
...
def evaluate_gate(r: dict) -> tuple[str, str, str, str, dict]:
    """(composite, sign, magnitude, regime, crit) per plan SS W3-3 operator +
    gate-verdicts.md collapse rule.

    Operator (plan SS W3-3): the GATE-PASS direction is INVERTED relative to a
    naive 'larger is better'. PASS iff |dY_12/d delta|_0 > eps_lift=1e-3 AND rank
    1 -> >=2 (degeneracy LIFTS), which CONTRADICTS the §VII.BL permanent theorem;
    FAIL iff |dY_12/d delta|_0 <= eps_lift (degeneracy PERSISTS), which CONFIRMS
    §VII.BL. The 3-tuple semantics MATCH S110-CF1 EXACTLY (bit-comparable)."""
...
def make_plot(r: dict, verdict: str) -> None:
    ...
    if r["baseline"] is not None:
        ax1.axhline(r["baseline"]["abs_dY12_d0"], color="tab:green", ls="-.",
                    lw=0.9,
                    label=rf"S110-CF1 $|dY_{{12}}/d\delta|_0$="
                          rf"{r['baseline']['abs_dY12_d0']:.1e}")
    ...
    bit_txt = ("n/a" if r["bit_match"] is None
               else ("MATCH" if r["bit_match"] else "MISMATCH"))  # (local)
    fig.suptitle(f"{GATE_ID}: {verdict} -- C^2-coset off-U(2) Dirac Yukawa "
                 f"CONFIRM (J_C2={J_C2}, 4 bonds); rank-1-wall lift test\n"
                 f"(L_max={L_MAX}, tau_fold={tau_fold}, GEN_SECTOR={GEN_SECTOR}; "
                 f"S110-CF1 bit-compare: {bit_txt})",
                 fontsize=11)
...
def save_npz(r: dict, verdict: str, tup3: tuple, crit: dict,
    ...
    np.savez(
        ...
        preflight_t_O_required=r["preflight_t_O_required"],
        preflight_admissible=r["preflight_admissible"],
        # --- S110-CF1 baseline bit-comparison (the confirmation contract) ---
        baseline_abs_dY12_d0=(float("nan") if base is None
                              else base["abs_dY12_d0"]),
        baseline_schur_offdiag0=(float("nan") if base is None
                                 else base["schur_offdiag0"]),
        baseline_lam0=(float("nan") if base is None else base["lam0"]),
        baseline_distinct0=(-1 if base is None else base["distinct0"]),
        baseline_n_gen=(-1 if base is None else base["n_gen"]),
        baseline_verdict=np.array("ABSENT" if base is None else base["verdict"]),
        bit_match=(False if r["bit_match"] is None else r["bit_match"]),
        bit_compare_available=(r["bit_match"] is not None),
        d_lift_vs_baseline=r["d_lift_vs_baseline"],
        bitcompare_tol=BITCOMPARE_TOL,
        # --- criteria + pins ---
...
def main() -> int:
    ...
    print(f"  Pre-flight: triality t(gen)={r['preflight_t_gen']}, intra-sector "
          f"required t(O)={r['preflight_t_O_required']} (triality-EVEN), "
    ...
    print(f"  Step 1 generation multiplet in fund (1,0): |lambda|={r['lam0']:.6f}, "
          f"both copies share t=1 (multiplicity leg).")
    print(f"  Step 2 intra-sector mixing O: t(a)=t(b)=1 => required t(O)="
          f"{r['preflight_t_O_required']} (admissible={r['preflight_admissible']}, "
          f"NECESSARY-only).")
    print(f"  Step 3 C^2-coset deformation is LEFT-INVARIANT (g_K deformation "
          f"commuting with SU(3)) => multiplicity-SCALAR on each C^m(p,q) "
          f"(VII.BL) => inter-generation projection = commutant projection = 0.")
    print(f"  Step 4 dY_12/d delta|_0 = <psi_1|(dD_K/d delta)|_0|psi_2> with "
          f"(dD_K/d delta)|_0 multiplicity-scalar => off-diagonal vanishes by Schur.")
    ...
    print(f"  Direction: |dY_12/d delta|_0 "
          ...
          f"{'LIFTS (Reading-A, CONTRADICTS VII.BL)' if (r['abs_dY12_d0'] > EPS_LIFT and r['rank_increased']) else 'PERSISTS (Reading-B, CONFIRMS VII.BL)'}.")
    if base is not None:
        print(f"  Confirmation bit-compare: this |dY_12/d delta|_0="
              f"{r['abs_dY12_d0']:.6e} vs S110-CF1 {base['abs_dY12_d0']:.6e} "
              f"(|Delta|={r['d_lift_vs_baseline']:.3e} < {BITCOMPARE_TOL}: "
              f"bit_match={r['bit_match']}); reproduces the down-tiered source.")
    print(f"  Conclusion: composite = {verdict} "
          f"(sign={sign_v}, magnitude={mag_v}, regime={regime_v}). "
          f"{'CONFIRMS' if verdict == 'FAIL' else 'does NOT confirm'} VII.BL on "
          f"the C^2-coset; the rank-1 wall is "
          f"{'off-ALL left-invariant internal moduli (hierarchy PINNED to external eps_LX)' if verdict == 'FAIL' else 'POTENTIALLY lifted -> FLAGGED Stage-2 CONTRADICTION re-audit'}.")
    ...
    bit_txt = ("baseline_absent" if r["bit_match"] is None
               else f"bitmatch={r['bit_match']}_dLift{r['d_lift_vs_baseline']:.2e}")  # (local)
    ...
    base_dY = (float("nan") if base is None else base["abs_dY12_d0"])  # (local)
    print_verdict_payload(
        verdict, value, audit_sha, content_sha,
        sign_verdict=sign_v, magnitude_verdict=mag_v, regime_verdict=regime_v,
        companion_note=(
            f"CONFIRM witness (down-tiered from S110-CF1-YUK-C2COSET): off-U(2) "
            f"C^2-coset (J_C2={J_C2}, 4 bonds) Dirac Yukawa splitting on the "
            f"d={r['n_gen']} generation multiplet (|lambda|={r['lam0']:.4f}, fund "
            ...
            f"(intra-split@0.20={r['intra_split'][-1]:.2e}); S110-CF1 baseline "
            f"|dY_12/d delta|_0={base_dY:.3e}, bit_match={r['bit_match']}; "
            f"vol-preserving det-ratio={r['det_ratio']:.6f}"),
        extra_rows=[
            (f"# confirm-contract: down-tiered from S110-CF1-YUK-C2COSET "
             f"(FAIL, 8.726920e-16); this |dY_12/d delta|_0={r['abs_dY12_d0']:.6e}; "
             f"S110-CF1 baseline={base_dY:.6e}; |Delta|={r['d_lift_vs_baseline']:.3e} "
             f"vs tol {BITCOMPARE_TOL}; bit_match={r['bit_match']} "
             f"(L_max={L_MAX} bit-comparable) # {GATE_ID}"),
            ...
            (f"# selection-rule preflight: triality t(gen)={r['preflight_t_gen']} "
             f"intra-sector required t(O)={r['preflight_t_O_required']} (EVEN) "
            ...
            (f"# VII.BL-confirm: rank-1 Yukawa wall off-ALL left-invariant internal "
             f"moduli (su(2) INV2-W1-1 1.943e-15; C^2-coset S110-CF1 8.727e-16; "
             f"this confirm {r['abs_dY12_d0']:.3e}); hierarchy PINNED to external "
             f"non-LI eps_LX; dual prior re-allocates {'0.95 Track B (CONFIRMS)' if verdict == 'FAIL' else '0.90 Track A (CONTRADICTION -> Stage-2 re-audit)'} # {GATE_ID}"),
        ],
    )
