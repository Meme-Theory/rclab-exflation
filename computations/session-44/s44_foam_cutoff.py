...
For the S36 linear cutoff f(lambda^2) = |lambda|:
  ...
  For the peak to fall within the eigenvalue range:
    alpha=2: gamma in [0.12, 0.74]  -- requires gamma >> epsilon_crossover = 0.014  # (local)
    alpha=3: gamma in [0.027, 0.32] -- requires gamma > epsilon_crossover  # (local)
    alpha=4: gamma in [0.011, 0.22] -- minimum at gamma ~ 0.011 BARELY in range  # (local)
