...
for i, res in enumerate(results_per_tau):
    ax2.fill_between(bin_centers, offset_step, res['rho_smooth'] + offset_step,
                     alpha=0.4, color=tau_colors[i], label=f'$\\tau={res["tau"]:.2f}$')  # (local)
...
for gname in sector_groups:
    ...
    ax7.bar(bin_centers, h, width=bin_width * 0.9, bottom=bottom,
            alpha=0.7, color=group_colors[gname], label=gname)  # (local)
    bottom += h
...
print(f"  Data: computations/session-44/s44_dos_tau.npz")
print(f"  Plot: computations/session-44/s44_dos_tau.png")
print(f"  Script: computations/session-44/s44_dos_tau.py")
