...
Input:
  - computations/session-39/s39_richardson_gaudin.npz
  - computations/session-39/s39_kk_mass.npz

Output:
  - computations/session-40/s40_nohair_sensitivity.npz
  - computations/session-40/s40_nohair_sensitivity.png
"""
...
def find_T_gibbs(E_target, E_modes):
    ...
    Z = np.sum(boltz)
    p = boltz / Z
    S = -np.sum(p * np.log(p + 1e-300))  # (local)
