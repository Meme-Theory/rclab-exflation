...
For each max_pq_sum in {4, 5, 6, 7}:
  1. Compute Dirac spectrum at tau_fold = 0.19016 using dirac_spectrum
...
np.savez(os.path.join(SCRIPT_DIR, "s43_transplanckian.npz"), **save_dict)
print(f"\nData saved to: {os.path.join(SCRIPT_DIR, 's43_transplanckian.npz')}")
...
ax3.fill_between(mpq_arr, [0.014*0.9]*len(mpq_arr), [0.014*1.1]*len(mpq_arr),
                 alpha=0.2, color='green', label='10% band')  # (local)
...
plt.tight_layout()
plt.savefig(os.path.join(SCRIPT_DIR, "s43_transplanckian.png"), dpi=150, bbox_inches='tight')
print(f"Plot saved to: {os.path.join(SCRIPT_DIR, 's43_transplanckian.png')}")
