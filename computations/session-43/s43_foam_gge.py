...
pair = np.load('computations/session-37/s37_pair_susceptibility.npz', allow_pickle=True)
        ...
        for step in range(n_steps):
            ...
            for k in range(n_m):
                for l in range(k+1, n_m):
                    eta = epsilon * np.random.randn()  # (local)
...
np.savez('computations/session-43/s43_foam_gge.npz',
    # Physical parameters
    M_KK=M_KK_grav,
    M_Pl=M_Pl,
    epsilon_1=epsilon_1,
...
print(f"\nData saved to computations/session-43/s43_foam_gge.npz")
...
plt.savefig('computations/session-43/s43_foam_gge.png', dpi=150, bbox_inches='tight')
print("Plot saved to computations/session-43/s43_foam_gge.png")
plt.close()
