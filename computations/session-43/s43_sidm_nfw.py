...
def rho_sidm_isothermal(r_kpc):
    ...
    w = 0.5 * (1 + np.tanh((r_kpc - r_match) / (0.3 * r_core_sidm_kpc)))  # (local)
...
def rho_fdm_soliton(r_kpc):
    ...
    w = 0.5 * (1 + np.tanh((r_kpc - r_match) / (0.3 * r_sol_kpc)))  # (local)
...
# Save NPZ
np.savez('computations/session-43/s43_sidm_nfw.npz',
    # Gate
...
print(f"\nSaved: computations/session-43/s43_sidm_nfw.npz")
...
plt.savefig('computations/session-43/s43_sidm_nfw.png', dpi=150, bbox_inches='tight')
print("Saved: computations/session-43/s43_sidm_nfw.png")
plt.close()
