...
def compute_growth_factor(z_array, w=-1.0):
    ...
    a_init = 1e-3  # start deep in matter domination (local)
    a_final = 1.0  # (local)
...
# 11. Save results
# =============================================================================
np.savez("computations/session-43/s43_void_expansion.npz",
...
for label, w in w_models.items():
    ...
    ls = '-' if w == -1.0 else '--'
    lw = 2.5 if w == -1.0 else 1.5  # (local)
...
plt.tight_layout()
plt.savefig("computations/session-43/s43_void_expansion.png", dpi=150, bbox_inches='tight')
plt.close()

print(f"\nPlot saved: computations/session-43/s43_void_expansion.png")
print(f"Data saved: computations/session-43/s43_void_expansion.npz")
...
Results:
  ...
  7. Joint Euclid full survey FoM ~ 500: sufficient to distinguish
     w = -1 from w_a != 0 at > 5 sigma.  # (local)
