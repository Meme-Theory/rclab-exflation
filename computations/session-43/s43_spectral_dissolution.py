...
# Add computation paths (active + archive)
...
# S81 Level 3 retrofit: canonical constants import (per computations/_shared/CLAUDE.md).
# Provides: tau_fold, M_Pl_unreduced, M_Pl_reduced, l_Planck, etc.
...
# CONSTANTS
# ============================================================
R_POISSON = 2 * np.log(2) - 1   # (local) 0.38629
R_GOE = 0.5307                    # (local) Atas et al. 2013
R_GUE = 0.5996                    # (local) Atas et al. 2013
DEGENERACY_TOL = 1e-10            # (local) numerical tolerance
TAU_FOLD = tau_fold               # (local) alias to canonical tau_fold (Jensen fold)
N_SAMPLES = 20                    # (local) random perturbation realizations
EPSILON_VALUES = [0.001, 0.005, 0.01, 0.02, 0.03, 0.05, 0.10, 0.20, 0.50]  # (local) scan grid
MAX_PQ_SUM = 2                    # (local) include (0,0), (1,0), (0,1), (1,1), (2,0), (0,2)
...
np.savez('computations/session-43/s43_spectral_dissolution.npz', **save_data)
print("  Saved: computations/session-43/s43_spectral_dissolution.npz")
...
plt.savefig('computations/session-43/s43_spectral_dissolution.png', dpi=150, bbox_inches='tight')
print("  Saved: computations/session-43/s43_spectral_dissolution.png")
...
if epsilon_crossover is not None:
    ...
    print(f"   M_KK_threshold = {epsilon_crossover**2 * M_Pl_unreduced:.3e} GeV")
