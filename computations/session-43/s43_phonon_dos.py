...
for gname in sector_groups:
    ...
    ax4.bar(bin_centers, h, width=bin_width*0.9, bottom=bottom,
            alpha=0.7, color=colors_groups[gname], label=gname)  # (local)
    bottom += h
...
for b_label in ['B1', 'B2', 'B3']:
    ...
    if np.any(nonzero):
        ax6.bar(bin_centers[nonzero], h_branch[nonzero], width=bin_width*0.85,
                alpha=0.5, color=branch_colors[b_label],  # (local)
...
print(f"  Data: computations/session-43/s43_phonon_dos.npz")
print(f"  Plot: computations/session-43/s43_phonon_dos.png")
