...
print(f"Data saved to: computations/session-43/s43_polariton_bz.npz")
print()
...
for i in range(8):
    ...
    ax.plot(k_grid, omega_bare[i, :], '--', color=colors_bare[i],
            alpha=0.3, lw=1, label=label)  # (local)
# Dressed bands (solid)
for i in range(8):
    label = None
...
for i in range(8):
    ...
    ax.plot(k_grid, v_group[i, :], '-', color=color, lw=1.2,
            alpha=0.7, label=f'Band {i}' if i in [0, 4, 5] else None)  # (local)
...
print(f"Plot saved to: computations/session-43/s43_polariton_bz.png")
print()
