...
d_s40 = np.load('computations/session-40/s40_collective_inertia.npz', allow_pickle=True)
d_s42c = np.load('computations/session-42/s42_constants_snapshot.npz', allow_pickle=True)
...
np.savez('computations/session-43/s43_friedmann_bcs.npz',
...
print("\nData saved to computations/session-43/s43_friedmann_bcs.npz")
...
plt.tight_layout()
plt.savefig('computations/session-43/s43_friedmann_bcs.png', dpi=150, bbox_inches='tight')
print("Plot saved to computations/session-43/s43_friedmann_bcs.png")
...
AT THE FOLD (tau = 0.19, S = {float(cs_S(0.19)):.0f}):
  ...
  Scenario A (S39 transit, tau_dot = {tau_dot_S39:.0f}):
    ...
    n_s = 1 - 2*eps = {results['A_S39_velocity']['ns_approx']:.4f}  # (local)
