...
d_eff = 1 < d_uc = 4: mean-field is NOT exact for the BCS transition on moduli.  # (local)
...
def universality_class_analysis():
    ...
    - If the BCS transition happens simultaneously everywhere (homogeneous quench):
      d_eff = 0 for the transition, and KZ doesn't apply in the usual sense.  # (local)
    - If it happens at different times in different places (inhomogeneous quench):
      d_eff = 3 (spatial dimensions of the fabric), and we get 3D Ising.  # (local)
...
if __name__ == '__main__':
    ...
    # --- Load input data ---
    d36 = np.load('computations/session-36/s36_mmax_authoritative.npz', allow_pickle=True)
    d35 = np.load('computations/session-35/s35_ed_corrected_dos.npz', allow_pickle=True)
    d43 = np.load('computations/session-43/s43_lifshitz_class.npz', allow_pickle=True)
    ...
    np.savez('computations/session-43/s43_bcs_universality.npz', **save_dict)
    print("\nSaved: computations/session-43/s43_bcs_universality.npz")
    ...
    plt.tight_layout()
    plt.savefig('computations/session-43/s43_bcs_universality.png', dpi=150, bbox_inches='tight')
    print("Saved: computations/session-43/s43_bcs_universality.png")
    print("\nDONE.")
