...
# Canonical constants import (T3-S81 PRU migration)
sys.path.insert(0, os.path.abspath(os.path.join(SCRIPT_DIR, '..', 'computations')))
...
idx_fold = np.argmin(np.abs(tau_kap - tau_fold))  # (local) grid-index lookup, canonical tau_fold
omega_tau_fold = omega_tau_arr[idx_fold]  # (local) fold-value of modulus oscillation
...
F_pp_GL = 4.0 * abs(a_GL)  # (local) GL-quartic reference curvature
...
width = 0.2 * Delta_0_num  # (local) local-fit half-window (20% of Delta_0)
...
delta_inst = 1.0 / omega_barrier_BCS  # (local) instanton width (BCS numerical)
delta_inst_GL = 1.0 / omega_barrier_GL  # (local) instanton width (GL quartic)
...
n_inst_dilute = np.sqrt(S_inst / (2*np.pi)) * np.exp(-S_inst) / delta_inst  # (local) dilute-gas density
# Dense gas saturation
n_inst_dense = 1.0 / delta_inst  # (local) dense-gas saturation density
