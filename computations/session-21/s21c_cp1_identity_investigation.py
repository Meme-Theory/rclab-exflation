...
def investigate_delta_T_sectors(branching, data):
    ...
    for t_idx in range(n_tau):
        ...
        norm = 64 * np.pi**2 * np.exp(4 * tau)  # (local)
...
def make_plots(tau_values, S_b1, S_b2, S_b1_rel, S_b2_rel,
    ...
    plt.tight_layout()
    plt.savefig('computations/session-21/s21c_cp1_investigation.png', dpi=150)
    print(f"\nPlot saved: computations/session-21/s21c_cp1_investigation.png")
...
if __name__ == '__main__':
    ...
    # Load data
    data = np.load('computations/session-19/s19a_sweep_data.npz', allow_pickle=True)
    tau_values = data['tau_values']
    ...
    # Save results
    np.savez('computations/session-21/s21c_cp1_investigation.npz',
    ...
    print(f"\nData saved: computations/session-21/s21c_cp1_investigation.npz")
