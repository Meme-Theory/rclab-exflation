...
# Load Riemann data
d = np.load('C:/sandbox/Ainulindale Exflation/computations/_shared/r20a_riemann_tensor.npz',
            allow_pickle=True)
tau_vals = d['tau']
R_all = d['R_abcd']

print(f"Data file: computations/_shared/r20a_riemann_tensor.npz")
...
def compute_euler_S(R, eps8):
    ...
    S = eps_{a1...a8} eps_{b1...b8} R_{a1a2,b1b2} R_{a3a4,b3b4} R_{a5a6,b5b6} R_{a7a8,b7b8}  # (local)
...
for ti in range(len(tau_vals)):
    ...
    eta = elapsed / (ti + 1) * (len(tau_vals) - ti - 1)  # (local)
...
# Write output file
output_path = 'C:/sandbox/Ainulindale Exflation/computations/session-21/s21c_gauss_bonnet.txt'
with open(output_path, 'w') as fout:
    fout.write("Session 21c P0-5: Gauss-Bonnet Topological Check\n")
    fout.write("=" * 60 + "\n\n")
    fout.write(f"Date: 2026-02-19\n")
    fout.write(f"Data file: computations/_shared/r20a_riemann_tensor.npz\n")
