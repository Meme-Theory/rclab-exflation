...
# S81 Level 3 retrofit: canonical constants + fix the missing `d = np.load(...)`
# that made the script raise NameError at line 132 on every run.
# Sibling scripts (gb_debug, debug2, debug3, debug5, debug6) all load the same
# stored tensor; this file was the lone outlier. The NPZ lives in the archive,
# not computations (the sibling-script path hardcodes are stale; they work
# only because they're run from the project root where both dirs are sibling).
_this_dir = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, _this_dir)
_shared_comp = os.path.abspath(os.path.join(_this_dir, '..', 'computations'))
if os.path.isdir(_shared_comp):
    sys.path.insert(0, _shared_comp)

from canonical_constants import *  # noqa: F401,F403

# Load the stored Riemann tensor (fix for the original NameError at line 132).
# Check both computations/_shared and computations for the file (matches the
# sibling scripts' fallback pattern).
_rt_path_archive = os.path.join(_this_dir, 'r20a_riemann_tensor.npz')
_rt_path_comp = os.path.join(_shared_comp, 'r20a_riemann_tensor.npz')
_rt_path = _rt_path_archive if os.path.exists(_rt_path_archive) else _rt_path_comp
d = np.load(_rt_path, allow_pickle=True)  # provides d['R_abcd'][0] = tau=0 Riemann tensor
...
print(f"  |f_abc + f_bac| max: {err_antisym:.2e}")
