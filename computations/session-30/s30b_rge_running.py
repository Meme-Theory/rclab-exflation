...
# Canonical constants import (T3 rebuild — S81 discipline)
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))
from canonical_constants import *  # M_Z, sin2_thetaW_MSbar, b1_SM, b2_SM

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = Path(SCRIPT_DIR)

# ─── Physical Constants ──────────────────────────────────────────────────────
# M_Z, sin2_thetaW_MSbar, b1_SM, b2_SM imported from canonical_constants.

# PDG SM gauge couplings at M_Z — not yet promoted to canonical_constants.
# Tagged (local) with PDG 2024 provenance for audit.
ALPHA_1_MZ = 0.01699   # (local) PDG 2024 alpha_1(M_Z) = 5/3 * alpha_em / cos^2(theta_W)
ALPHA_2_MZ = 0.03376   # (local) PDG 2024 alpha_2(M_Z)
SIN2_TW_PDG = sin2_thetaW_MSbar   # (local) alias = 0.23122

# SM one-loop beta aliases (imported names: b1_SM, b2_SM)
B1 = b1_SM        # (local) +41/10
B2 = b2_SM        # (local) -19/6
B3 = -7.0         # (local) SU(3) one-loop — kept for reference, not used
...
GRID_NPZ_PATH = DATA_DIR / 's30b_grid_bcs.npz'
GRID_SHA256 = _sha256_of(GRID_NPZ_PATH)
print(f"INPUT_PIN: s30b_grid_bcs.npz sha256={GRID_SHA256}")
...
def main():
    ...
    target = SIN2_TW_PDG
    tol = 0.010  # (local) points within 0.01 of target
