...
# Canonical constants compliance (no framework constants consumed directly —
# Pfaffian is a topological invariant; tau scan range and N_max are machinery
# pins declared in the PRU block). Imported for project-wide hygiene.
...
def run_pfaffian_scan():
    ...
    N_TAU = 75  # (local) PRU pin: tau-scan resolution
    tau_scan = np.linspace(0.0, 2.5, N_TAU)  # (local) PRU pin: tau in [0, 2.5]

    # Storage
    pf_values = np.zeros(N_TAU, dtype=complex)  # (local)
    pf_signs = np.zeros(N_TAU)  # (local)
    min_gap_dtotal = np.zeros(N_TAU)  # (local)
    df_norms = np.zeros(N_TAU)  # (local)
    dk_norms = np.zeros(N_TAU)  # (local)
    asym_errs = np.zeros(N_TAU)  # (local)

    # Sector Pfaffians for diagnostics (6 sectors)
    n_sectors = 6  # (local) MAX_PQ_SUM=2 yields 6 sectors: (0,0),(0,1),(0,2),(1,0),(1,1),(2,0)
    sector_pf_log = np.zeros((N_TAU, n_sectors), dtype=complex)  # (local)
