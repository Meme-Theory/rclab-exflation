...
# hygiene (pre-S34 script, exempt from canonical import mandate): there
# are NO framework constants in this script. All hardcoded values are either
# topological (d_s=4, d_s=8 as dimensional reference), local scoring
# thresholds (h_ds, h_gap cutoffs — habitability heuristic, not canonical),
# or reference baselines (gap_ref = gaps[0], E_ref = vac[0]). Per pattern
# we still attempt the canonical import for hygiene and flag the absence.
sys.path.insert(0, os.path.join(SCRIPT_DIR, '..'))
try:
    from canonical_constants import tau_fold  # noqa: F401 (hygiene-only)
    _CANON_IMPORT_OK = True  # (local)
except Exception:
    _CANON_IMPORT_OK = False  # (local) pre-S34 exempt; fall through silently

from s19a_sweep_data import load_sweep_data


def _sha256_file(path):
    """closure: SHA-256 of a file for input pinning."""
...
def compute_spectral_dimension_surface(data, sigma_values):
    ...
    d_log_sigma = 0.01  # (local) finite-difference step for d_s derivative
...
def compute_habitability(data, sigma_star=1.0, beta_star=1.0):
    ...
    for i, tau in enumerate(tau_values):
        ...
        if Z > 0:
            ...
            S = 0  # (local) zero-partition-function fallback
        ...
        if 4 <= d_s <= 8:
            h_ds = 1.0  # (local) physical-regime plateau
        elif d_s < 4:
        ...
        S_ref = 8.0  # (local) approximate entropy at tau=0 (reference baseline)
...
def main():
    ...
    SWEEP_PY_PATH = os.path.join(SCRIPT_DIR, 's19a_sweep_data.py')  # (local)
    SCRIPT_PATH = os.path.abspath(__file__)  # (local)
    OUTPUT_DIR = SCRIPT_DIR

    # pin header: log SHA-256 of every input for reproducibility
    print("Session 19a -- False Vacuum Analysis (re-run)")
    print("=" * 60)
    print("PIN-BLOCK-BEGIN")
    input_pins = {}  # (local) ordered map of input path -> sha256
    for label, pth in [
        ("script", SCRIPT_PATH),
        ("sweep_data_py", SWEEP_PY_PATH),
        ("sweep_data_npz", DATA_PATH),
    ]:
        if os.path.exists(pth):
            h = _sha256_file(pth)  # (local)
            input_pins[label] = h
            print(f"  sha256[{label}] = {h}  ({os.path.basename(pth)})")
        else:
            input_pins[label] = "MISSING"  # (local)
            print(f"  sha256[{label}] = MISSING  ({pth})")
    print(f"  canonical_import_ok = {_CANON_IMPORT_OK}")
    # Closure SHA = SHA-256 of JSON-sorted input-pin map
    ...
    # 4-tuple output tag: reported AS-IS (no gate threshold for S19A —
    # this is diagnostic output feeding downstream closed mechanisms S19d).
    # We publish the two primary scalars: rate_vac (vacuum energy e-fold rate)
    # and d_s_min (physical-regime boundary). scheme = 'PW-weighted' indicates
    # the plane-wave multiplicity weighting used throughout.
    tau_at_ds_min = float(tau_values[idx_min_ds])  # (local)
    d_s_min_val = float(d_s_at_sigma1[idx_min_ds])  # (local)
    rate_vac_val = float(rate_vac) if rate_vac is not None else float('nan')  # (local)
    rate_gap_val = float(rate_gap) if rate_gap is not None else float('nan')  # (local)
    # L_max proxy: max_pq_sum=6 is fixed by sweep_data generator (see s19a_sweep_data.py)
    max_pq = 6  # (local) L_max proxy: fixed at max_pq_sum=6 per sweep generator

    print()
    print("4TUPLE-BEGIN")
    print(f"  value={{rate_vac:{rate_vac_val:.6f},rate_gap:{rate_gap_val:.6f},"
          f"d_s_min:{d_s_min_val:.4f},tau_at_ds_min:{tau_at_ds_min:.3f}}}")
    print(f"  scheme=PW-weighted-spectral-heat-kernel")
    print(f"  convention=d_log_sigma=0.01_centered_finite_difference")
    print(f"  L_max=max_pq={max_pq}")
    print("4TUPLE-END")
