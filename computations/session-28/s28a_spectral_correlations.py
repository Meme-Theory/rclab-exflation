...
Input:
  computations/session-27/s27_multisector_bcs.npz (D_K eigenvalues, 9 sectors, 9 tau)

Output:
  computations/session-28/s28a_spectral_correlations.npz
  computations/session-28/s28a_spectral_correlations.png
...
# Add computations to path for tier1 imports
...
def main():
    ...
    for col, tau_idx in enumerate(plot_tau_indices):
        ...
        if key in spacings_K_plot and len(spacings_K_plot[key]) > 5:
            ...
            ax.hist(sp, bins=min(20, max(8, len(sp) // 5)), density=True,
                    alpha=0.6, color='steelblue', label=f'D_K (n={len(sp)})')  # (local)
    ...
    for col, tau_idx in enumerate(plot_tau_indices):
        ...
        if key in spacings_can_plot and len(spacings_can_plot[key]) > 5:
            ...
            ax.hist(sp, bins=min(20, max(8, len(sp) // 5)), density=True,
                    alpha=0.6, color='darkorange', label=f'D_can (n={len(sp)})')  # (local)
    ...
    for ci, sec_idx in enumerate(usable_sectors):
        ...
        if np.any(valid_K):
            ...
            ax_qK.fill_between(tau_values[valid_K],
                               ...
                               q_K[sec_idx, valid_K] + q_K_err[sec_idx, valid_K],
                               alpha=0.1, color=colors[ci])  # (local)
    ...
    if key0 in numvar_can:
        ax_nv.plot(L_values, numvar_can[key0], color='darkorange', ls='-',
                   lw=1.5, label=f'D_can tau=0')  # (local)
    # Also at tau=0.30
    ...
    if key30 in numvar_can:
        ax_nv.plot(L_values, numvar_can[key30], color='darkorange', ls='--',
                   lw=1.2, label=f'D_can tau={tau_values[tau_idx_30]:.2f}')  # (local)
    # Reference lines
