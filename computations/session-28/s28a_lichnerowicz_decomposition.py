...
Input:  computations/session-27/s27_torsion_gap_gate.npz
Output: computations/session-28/s28a_lichnerowicz.npz
        computations/session-28/s28a_lichnerowicz.png
...
# Add computations to path for scalar curvature formula
...
def scalar_curvature_jensen(tau: float) -> float:
    ...
    computation in spectral_action.py gives R_K(0) = +2 exactly.
...
def main():
    ...
    for j in nontrivial:
        ax.fill_between(tau_values, 0, lich['gap_T_sq'][j, :],
                         alpha=0.3, color=colors[j], label=f'{sector_labels[j]} algebraic')  # (local)
        ax.fill_between(tau_values, lich['gap_T_sq'][j, :], lich['gap_K_sq'][j, :],
                         alpha=0.3, color=colors[j], hatch='//',  # (local)
