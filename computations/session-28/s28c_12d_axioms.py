...
Date: 2026-02-27
Depends on: dirac_spectrum.py, s27_torsion_gap_gate.py, s28b_order_one.py data
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'computations'))

from canonical_constants import tau_fold  # canonical provenance check

L_MAX_PIN = 5  # (local) pre-registered truncation — Peter-Weyl max_pq_sum
...
def main():
    ...
    tau_values = np.array([0.0, 0.15, 0.30])  # (local) diagnostic sweep — axioms are tau-independent; uses tau_fold context only

    # Run all 7 axiom checks
    results = {}

    t0 = time.time()
    results['axiom1'] = verify_axiom1(tau_values, gens, f_abc, gammas, max_pq_sum=L_MAX_PIN)
