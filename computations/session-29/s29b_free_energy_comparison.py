...
# Canonical constants (S81 T3 re-run)
sys.path.insert(0, os.path.abspath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), '..', 'computations')))
from canonical_constants import *  # noqa: F401,F403  (E_cond, Delta_BCS, tau_fold, etc.)

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# SHA-256 pins — inputs + canonical_constants (S81 T3-S29B)
SHA256_PINS = {
    's27_multisector_bcs.npz':              'ae110f792dfaec9e525f2457d530daf0466dbdd647b717b902a3d5e3e1781577',
    's28a_spectral_action_comparison.npz':  'c8c2cbf1254db9a3cba03b70bfa2c1fd3d4b56fa23b30733c6fc88337259bfa4',
    's28b_self_consistent_tau_T.npz':       '287e03bb8ae4ad3d9b2dd5680d4e971582103eb4a2929ae7c6dc32c130bf7074',
    's29a_derived_drive_rate.npz':          '215c7524fa7157b524da34761b3b9c6e62f157e35d44af3d0de513e3425b6d38',
    'canonical_constants.py':               '68b50cd325d2cc8c63b775da3b6b92f538da582bee44c5d906c81259f24dd12f',
}

def _verify_sha(fpath, expected):  # (local)
    h = hashlib.sha256(open(fpath, 'rb').read()).hexdigest()  # (local)
    assert h == expected, f'SHA mismatch for {fpath}: got {h}, expected {expected}'
    return h

def _verify_all_pins():  # (local)
    cc_path = os.path.abspath(os.path.join(SCRIPT_DIR, '..', 'computations', '_shared', 'canonical_constants.py'))  # (local)
    _verify_sha(cc_path, SHA256_PINS['canonical_constants.py'])
    for name in ['s27_multisector_bcs.npz', 's28a_spectral_action_comparison.npz',
                 's28b_self_consistent_tau_T.npz', 's29a_derived_drive_rate.npz']:
        _verify_sha(os.path.join(SCRIPT_DIR, name), SHA256_PINS[name])
    print('[S81 PIN] all 5 SHA-256 pins verified')
...
def compute_free_energy_comparison(data):
    ...
    tau_KC3 = 0.407  # (local) From s29a: n_gap crosses 20 at this tau
...
def main():
    ...
    # S81 T3 re-run: verify SHA-256 pins on inputs + canonical_constants
    _verify_all_pins()
    print()
