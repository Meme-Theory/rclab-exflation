...
Physics:
    ...
    - alpha: ratio of latent heat to radiation energy density
      alpha = epsilon / rho_rad, where rho_rad = (pi^2/30)*g_* T^4  # (local)
...
Inputs:
    computations/session-28/s28b_self_consistent_tau_T.npz
    computations/session-29/s29b_free_energy_comparison.npz
    computations/session-29/s29b_modulus_eom.npz
    computations/session-29/s29b_gaussian_correction.npz

Outputs:
    computations/session-29/s29c_gw_spectrum.npz
    computations/session-29/s29c_gw_spectrum.png
"""
...
for i in range(n_MKK):
    ...
    # alpha = epsilon / rho_rad
    alpha = epsilon_phys / rho_rad if rho_rad > 0 else 0.0  # (local)
