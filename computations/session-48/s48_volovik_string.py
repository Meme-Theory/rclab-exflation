...
def load_npz(name):
    """Load an npz file from the computations directory."""
...
Lambda_sp_over_MKK = 2.06  # from prior session  # (local)
...
gamma_strut = 0.05  # smoothing width in M_KK  # (local)
...
ax.fill_between(omega_bin_c, 0, rho_oscillating / max(domega, 1e-10),
                alpha=0.3, color='green', label=f'Shell (|E_sh/E_sm|={shell_fraction:.3f})')  # (local)
