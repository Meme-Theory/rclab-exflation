...
running = -4 * m_star**2 * x_sq / (x_sq + m_star**2)**2  # (local)
...
r_planck_limit = 0.06  # (local)

# Framework predictions at m*_angular
ns_framework_s48 = 0.965  # (local) S48-era value defining m*; distinct from canonical ns_framework=0.9595 promoted S81
alpha_s_framework = running
r_framework = 0.0  # (local)

print(f"  {'Observable':>15s} {'Planck':>15s} {'Framework':>15s} {'Sigma':>10s}")
print(f"  {'n_s':>15s} {ns_planck:>15.4f} {ns_framework_s48:>15.4f} {abs(ns_framework_s48-ns_planck)/ns_sigma:>10.1f}")
