...
# Load all input data
data_rg = np.load('computations/session-39/s39_richardson_gaudin.npz', allow_pickle=True)
data_dm = np.load('computations/session-44/s44_dm_de_ratio.npz', allow_pickle=True)
data_alpha = np.load('computations/session-45/s45_alpha_eff.npz', allow_pickle=True)
data_desi = np.load('computations/session-45/s45_two_fluid_desi.npz', allow_pickle=True)
data_pair = np.load('computations/session-37/s37_pair_susceptibility.npz', allow_pickle=True)
data_zub = np.load('computations/session-46/s46_zubarev_derivation.npz', allow_pickle=True)
...
w0_desi_dr1 = -0.55  # (local)
w0_desi_dr1_err = 0.21  # (local)
wa_desi_dr1 = -1.52  # (local)

# DESI DR2 (Y3, 2025, arXiv:2503.14738):
# w0waCDM with CMB+DESI BAO: w_0 = -0.752 +/- 0.058, w_a = -0.73 +/- 0.28
w0_desi_dr2 = -0.752  # (local)
w0_desi_dr2_err = 0.058  # (local)
wa_desi_dr2 = -0.73  # (local)
wa_desi_dr2_err = 0.28  # (local)
...
# Save all results
# ==============================================================================
np.savez('computations/session-48/s48_dmde_refine.npz',
    # Sub-1: Gibbs-Duhem
    E_GGE=E_GGE,
...
print(f"\n  Saved: computations/session-48/s48_dmde_refine.npz")
...
plt.tight_layout()
plt.savefig('computations/session-48/s48_dmde_refine.png', dpi=150, bbox_inches='tight')
print(f"  Saved: computations/session-48/s48_dmde_refine.png")
plt.close()
