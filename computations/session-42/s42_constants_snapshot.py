...
Data:
  ...
  computations/_shared/dirac_spectrum.py (jensen_metric)
...
# Level 3 canonical migration (S81): star import from canonical_constants.
from canonical_constants import *  # noqa: F401, F403
# Local aliases preserving original script variable names (introduced pre-S34; kept for minimal diff):
G_N_OBS = G_N                                   # (local) alias for G_N (m^3 kg^-1 s^-2)
ALPHA_EM_MZ_INV = alpha_em_MZ_inv               # (local) alias for 1/alpha_EM(m_Z)
LAMBDA_CC_OBS_GEV4 = rho_Lambda_obs             # (local) alias for observed CC in GeV^4
M_PL_REDUCED = M_Pl_reduced                     # (local) alias for reduced Planck mass
...
DATA_DIR = Path(__file__).parent              # (local)
PI2 = PI**2                                   # (local) pi^2 shorthand
# PI is now from canonical_constants (== np.pi; identical value, zero risk)

# Observed values — low-energy fine structure (distinct from alpha_em_MZ_inv in canonical)
ALPHA_EM_OBS = 1.0 / 137.036                   # (local) low-energy alpha_EM; historical reference only [FLAG: missing from canonical; candidate for promotion if reused]

# ======================================================================
#  SHA-256 input pin logging (S81 canonical form)
# ======================================================================

def _sha256(p):                                 # (local) helper for input pins
    h = hashlib.sha256()
    with open(p, 'rb') as f:
...
_script_path = Path(__file__).resolve()                                         # (local)
_canonical_constants_path = _script_path.parent / 'canonical_constants.py'      # (local)
_s41_path = _script_path.parent / 's41_constants_vs_tau.npz'                    # (local)
_s42_path = _script_path.parent / 's42_tau_dyn_reopening.npz'                   # (local)
_helper_path = _script_path.parent / 'dirac_spectrum.py'                   # (local)

_input_pins = {                                 # (local) ordered map of input SHA-256 pins
    'script':                   _sha256(_script_path),
    'canonical_constants':      _sha256(_canonical_constants_path),
    's41_constants_vs_tau.npz': _sha256(_s41_path),
    's42_tau_dyn_reopening.npz':_sha256(_s42_path),
    'dirac_spectrum.py':  _sha256(_helper_path),
}

print("SHA-256 INPUT PINS (S81):")
for _k in sorted(_input_pins):
    print(f"  {_k:30s} = {_input_pins[_k]}")

# Closure hash: SHA-256 of JSON-serialized sorted input-pin map
_closure_payload = json.dumps(_input_pins, sort_keys=True, separators=(',', ':'))  # (local)
closure_sha256 = hashlib.sha256(_closure_payload.encode('utf-8')).hexdigest()      # (local)
print(f"CLOSURE SHA-256: {closure_sha256}")
...
sin2_thetaW_0 = 0.5  # (local) at tau=0: g_1 = g_2 => sin^2 = 0.5
...
# e^{-4tau} = 0.2
# tau = ln(5)/4 = 0.4024
tau_weinberg_GUT = np.log(5.0) / 4.0           # (local) target tau for sin^2 = 3/8
...
alpha_GUT = 1.0 / 40.0                          # (local) approximate GUT unified coupling [FLAG: missing canonical]
...
alpha_2_GUT = 1.0 / 30.0                        # (local) alpha_2 at GUT scale [FLAG: missing canonical]
...
b_run = 2.06                                    # (local) empirical 1/alpha_EM running slope; diagnostic only
if M_KK_from_GN > M_Z:
    delta_alpha_inv = b_run * np.log(M_KK_from_GN / M_Z)  # (local)
else:
    delta_alpha_inv = 0                         # (local)
...
alpha2_mZ_inv = 29.587  # (local) PDG 1/alpha_2(m_Z) [FLAG: consider promoting to canonical as alpha2_mZ_inv_PDG]
...
# NOTE: This script uses the convention d(1/alpha_i)/d(ln mu) = b_i/(2pi).
# Canonical constants uses the opposite-sign convention (b1_SM = +41/10).
# We intentionally use a LOCAL name (b1_loc) so the star import does not shadow
# this value with the canonical one. Identical physics, opposite algebraic sign.
b1_loc = -41.0 / 10.0                           # (local) script-internal b1 [flag: canonical b1_SM uses opposite sign convention]
alpha1_mZ_inv = 59.0                            # (local) PDG (GUT normalization: alpha_1 = (5/3) alpha_Y)
alpha1_MKK_inv_from_running = alpha1_mZ_inv + (b1_loc / (2*PI)) * np.log(M_KK_kerner / M_Z)
...
# S81 canonical 4-tuple output tag (final non-verdict line)
# Primary gate quantity: |Delta log10(M_KK)| between Kerner (alpha_2) and spectral-zeta (G_N) routes
print(f"(value={OOM_diff:.6f}, scheme=Kerner-vs-SpectralZeta-OOM, convention=KK-SU3-Jensen-tau_fold, L_max=NA)")
