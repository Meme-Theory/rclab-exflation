...
# -*- coding: utf-8 -*-
r"""
CF-S102-B2-EPS2-WZ-HOLONOMY  --  FRAME-INVARIANT Wess-Zumino / Wilson-loop witness
================================================================================
Gate:   CF-S102-B2-EPS2-WZ-HOLONOMY  (trigger [VERIFY], classification GEOMETRIC)
...
Plan:   sessions/session-plan/session-102-plan-w7.md  ## SECTION W7-3
WP:     sessions/session-102/session-102-w7-workingpaper.md  ### SECTION W7-3
...
lambda_4..lambda_7) breaks the U(2) isotropy at second order.
...
lambda_6) coset plane,
    H(theta) = H_0 + eps * ( cos(theta) * dH_4 + sin(theta) * dH_6 ),
with dH_4, dH_6 the two off-block log-metric directions (||dH_a||_F = 1,
...
# ---------------------------------------------------------------------------
SESSION = "102"                                  # (local) emit_verdict session arg
GATE_ID = "CF-S102-B2-EPS2-WZ-HOLONOMY"          # (local)
...
# Deformation directions (coset generators; the loop rotates in the (4,6) plane)
COSET_A = 4                                       # (local) plan pin: first coset generator
COSET_B = 6                                       # (local) plan pin: second coset pair
...
NPZ_OUT = SESSION_DIR / "s102_w7_b2_eps2_wz_holonomy.npz"           # (local)
PNG_OUT = SESSION_DIR / "s102_w7_b2_eps2_wz_holonomy.png"           # (local)
...
def coset_H(theta, eps, H0, dH_a, dH_b):
    ...
    in the (lambda_4, lambda_6) plane. H(2pi) = H(0) exactly (closed loop)."""
...
def main():
    ...
    print(f"           closed coset loop in the (lambda_4, lambda_6) plane at eps={EPS_MAX}")
    ...
    #           [A_4, A_6] anisotropy on the U(2)-BROKEN loop. non_scalar_frac~1
    #           => genuine Wilczek-Zee (Track A); ~0 => Schur-scalar (Track B).
    # =====================================================================
    ...
    print(f"    ||M_ab=[A_4,A_6]||_F = {curv_nrm:.4e}; non_scalar_frac = {curv_nonscalar:.4e}")
    ...
    # next O(eps^2) coset-direction pair to test: W5-4's convention used array
    # indices a=4 (PRIMARY, "first coset generator") and a=6 (COMPANION). The
    # tested loop spans the (a=4, a=6) coset plane; the next pair probes the
    # remaining C^2 coset directions (array indices 3, 5) -- the orthogonal
    # off-block coset doublet, completing the C^2 coset span.
    next_pair = (3, 5)                                # (local) the orthogonal coset doublet (array indices 3,5)
    print(f"    next coset-direction pair to test = coset array-indices (3, 5) "
          f"[the orthogonal off-block C^2 coset doublet, completing the coset span]")
