...
`dirac_spectrum.py` (`build_cliff8()` function) which provides the
...
# Load: dirac_spectrum.py (build_cliff8 for gamma matrices)

R_data = np.load('computations/_shared/r20a_riemann_tensor.npz')
...
from dirac_spectrum import build_cliff8
...
| File | Arrays Used |
|------|-------------|
| `computations/_shared/r20a_riemann_tensor.npz` | `tau`, `R_abcd` (21,8,8,8,8), `Ric` (21,8,8), `R_scalar` (21,), `K` (21,) |
| `computations/session-23/s23c_fiber_integrals.npz` | `Ric_sq` (21,), `K_kretschner` (21,), `a4_geom` (21,) |
| `computations/session-24/s24a_vspec.npz` | `V_spec_rho_*` (21,) for comparison |
| `computations/_shared/dirac_spectrum.py` | `build_cliff8()`, `su3_generators()`, etc. |
| `computations/_shared/sd20a_seeley_dewitt_gate.py` | Analytic R(tau), Ric^2(tau), K(tau) formulas |

### 10.2 Output Files

| File | Contents |
|------|----------|
| `computations/session-26/s26_p3_a6.npz` | All computed invariants + V_spec^(6) + minima data |
| `computations/session-26/s26_p3_a6.png` | 4-panel diagnostic plot |

### 10.3 Script

| File | Purpose |
|------|---------|
| `computations/session-26/s26_p3_a6_computation.py` | Main computation script |

Python invocation: `"phonon-exflation-sim/.venv312/Scripts/python.exe" computations/session-26/s26_p3_a6_computation.py`
