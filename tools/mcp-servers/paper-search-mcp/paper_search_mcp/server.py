...
async def read_arxiv_paper(paper_id: str, save_path: str = "./downloads") -> str:
    """Read and extract text content from an arXiv paper.

    Tries arXiv's experimental LaTeXML HTML build first
    (`https://arxiv.org/html/{paper_id}`) -- equations are preserved verbatim
    as `$...$` / `$$...$$` from each <math> element's alttext, and section
    structure survives as ATX headings. Falls back to PDF download + PyPDF2
    extraction when HTML is unavailable (older papers, LaTeXML conversion
    failures). Output is prefixed with `[arxiv-paper-source: html url=...]`
    or `[arxiv-paper-source: pdf path=...]` so the caller knows which path
    was taken; HTML output is materially cleaner for math-heavy papers.

    Args:
        paper_id: arXiv paper ID (e.g., '2106.12345' or '2511.07517v3').
            Append a vN suffix to pin a specific version; omit for latest.
        save_path: Directory where the PDF would be saved when the HTML
            fallback fires (default: './downloads').
    Returns:
        str: The extracted text content of the paper, with a one-line
        source-tag header.
    """
    try:
