"""Apply batch 6 part 1: Parse _batch6_msg_part1.txt and apply LaTeX to knowledge-index.json."""
import json
import os
import re
...
with open(os.path.join(ROOT, "tools/_batch6_msg_part1.txt"), "r", encoding="utf-8") as f:
    content = f.read()
...
print(f"Parsed {len(batch)} equations from _batch6_msg_part1.txt")
...
print(f"Batch 6 part 1: {applied} applied, {len(missing)} missing")
if missing:
    print(f"  Missing: {missing}")
print(f"  audit_status already ok: {already_ok}, newly set to ok: {set_ok}")
