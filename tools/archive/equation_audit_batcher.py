...
Usage:
    ...
    python equation_audit_batcher.py --mode extract --source-type computation --batch-size 50
...
def stats_mode(idx):
    ...
    for eq in eqs:
        ...
        if sf.startswith('sessions'):
            mm_count += 1
        elif sf.startswith('computations') or sf.startswith('computation-archive'):
            t0_count += 1
        else:
            other_count += 1
    print(f"Sessions equations: {mm_count}")
    print(f"Computation-computation equations: {t0_count}")
...
def extract_mode(idx, source_type=None, source_file=None, batch_size=50):
    ...
    if source_file:
        ...
        filtered = [eq for eq in eqs if eq.get('source_file', '').startswith(('computations', 'computation-archive'))]
        label = 'computation'
    else:
        filtered = eqs
        label = 'all'
...
def main():
    ...
    parser.add_argument('--source-type', choices=['sessions', 'computation'], default=None)
