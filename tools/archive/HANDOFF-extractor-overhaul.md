...
- **Grade C**: 4 Giants sessions missing; 58% of computation scripts untracked in provenance; 22 probability trajectory entries with empty sessions

### 1d. Format Generations Cataloged (DONE)

Three exploration agents scanned all 80+ files in `sessions/` and `computations/`. Results written to `tools/format-generations.md` — a 450-line reference document cataloging every header pattern, content format, gate ID vocabulary, and verdict keyword across 8 formatting generations.
...
### Task 6: Expand collect_files() to scan computation .txt files

**File**: `tools/extract_entities.py` line 1441-1458

**Current**: Only grabs `*_gate_verdicts.txt` from computation:
```python
if COMPUTATIONS_DIR.exists():
    for f in COMPUTATIONS_DIR.iterdir():
        if f.name.endswith("_gate_verdicts.txt"):
            files.append(f)
```

**Change to**: Grab ALL `.txt` files from computation:
```python
if COMPUTATIONS_DIR.exists():
    for f in COMPUTATIONS_DIR.iterdir():
        if f.suffix == ".txt":
...
4. **Equations are special**: They come from ALL file types (sessions .md, computation .py, artifacts .md) and have their own dedup logic (`dedup_equations()`)
