#!/usr/bin/env python3
"""tools/_path_existence_audit.py — dead-path-reference audit.

Scans Python source files for `Path()` / `os.path.join()` expressions that
produce file paths under `computations/` and verifies each resolves to an
existing file on disk. Flags dead references — references to files that
don't exist anymore (typically: bare `computations/X.py` or `computations/sN_*`
patterns where the actual file lives at `computations/_shared/X.py` or
`computations/session-N/sN_*` per the abandoned `_phase3_path_string_migration`).

Why this audit exists:
  Per `_phase3_path_string_migration.py:8-13` the project moved file layouts:
  ...
  reference the pre-migration paths. This audit catches them so they can
  be fixed (and prevents new regressions from landing).

Companion to `extract_entities.py --audit-constants` (which flags hardcoded
canonical values). This audit flags hardcoded WRONG paths.

Usage:
  "phonon-exflation-sim/.venv312/Scripts/python.exe" tools/_path_existence_audit.py
  "phonon-exflation-sim/.venv312/Scripts/python.exe" tools/_path_existence_audit.py --json
  "phonon-exflation-sim/.venv312/Scripts/python.exe" tools/_path_existence_audit.py --strict   # exit 1 if any dead refs

Output: stdout report by default. --json emits machine-readable to
`tools/_path_existence_audit_report.json`.

Returns exit code 0 (all paths resolve) or 1 (--strict mode + dead refs found).
Without --strict, always exits 0 — WARN-only during initial rollout.
"""
