...
We then perform the Lorentzian CMPP classification on the twelve-dimensional product spacetime $M^{3,1}\times(\mathrm{SU}(3),g_\tau)$.  The static product structure guarantees exact CMPP Type~D at all~$\tau$, with a Weyl-aligned null direction (WAND) spanning a null 2-plane with components in both the flat external and curved internal factors.  The Riemannian Type~II result obtained from complexified null frames in definite signature is identified as a signature artifact.  A dense scan ($\tau\in[0,1.7]$, $171$ points) confirms that the classification is deformation-invariant: static Type~D holds at every grid point, with no algebraic-type transition in between.  When the modulus carries a nonzero deformation velocity, the extrinsic curvature of the constant-$\tau$ hypersurfaces dominates the internal Weyl tensor by a factor $\sim\!10^7$---the dynamic Weyl-squared norm is parameter-independent to $0.1\%$ at $|C|^2_{\mathrm{dyn}}\approx2.27\times10^7$, fixed by the deformation velocity rather than the internal geometry---shifting the classification to Type~G (algebraically general).  The static Type~D is restored once the modulus comes to rest.
...
The extension of algebraic classification to higher dimensions was accomplished by Coley, Milson, Pravda, and Pravdov\'a (CMPP)~\cite{CMPP2004,Milson2004}, who introduced a boost-weight decomposition of the Weyl tensor relative to a null frame.  The CMPP scheme classifies spacetimes by the vanishing of successively higher boost-weight components, defining types G (generic), I, II, D, III, N, and O.  A salient feature of the higher-dimensional theory, established in~\cite{Milson2004}, is that a generic Weyl tensor in $n>4$ dimensions possesses \emph{no} aligned null directions at all---algebraic speciality is exceptional, in sharp contrast to four dimensions where four principal null directions always exist.  This framework has been applied extensively to higher-dimensional black holes~\cite{MyersPerry1986,EmparanReall2008}, black strings~\cite{GregoryLaflamme1993}, and their perturbation theory~\cite{Ortaggio2013,Durkee2010}.  The Schwarzschild--Tangherlini solution in $D$ dimensions is CMPP Type~D, as are the Myers--Perry rotating black holes.

Despite this extensive body of work on \emph{spacetime} solutions, a notable gap exists in the literature: the algebraic classification of the Weyl tensor on \emph{compact internal manifolds} arising in Kaluza--Klein compactifications has not been systematically addressed.  In string theory and its low-energy supergravity limits, the internal geometry---typically a Calabi--Yau manifold, a coset space $G/H$, or a compact Lie group---determines the effective four-dimensional physics through dimensional reduction.  The curvature of the internal space enters the four-dimensional effective action through Seeley--DeWitt coefficients~\cite{Gilkey1975,VanSuijlekom2015}, governs the stability of the compactification~\cite{Koiso1980,WittenKKinstability1982}, and controls the spectrum of Kaluza--Klein excitations.  Yet the \emph{algebraic type} of this curvature---which encodes how the curvature operator decomposes into irreducible representations of the holonomy/isometry group---has received no systematic attention.\footnote{Throughout, the deformation parameter $\tau$ is an intrinsic modulus of the internal geometry $(\mathrm{SU}(3),g_\tau)$ itself, not a coordinate on an external configuration space; the Weyl operator we classify is an intrinsic invariant of that geometry. The Kaluza--Klein language (``internal space,'' ``external spacetime,'' ``tidal'') is used in its standard dimensional-reduction sense and carries no presumption that the internal factor sits inside a pre-existing container.}
...
We briefly review the CMPP framework~\cite{CMPP2004,Ortaggio2013}; see also the comprehensive treatment in~\cite{Durkee2010}.  Let $(M^n,g)$ be an $n$-dimensional (pseudo-)Riemannian manifold and let $C_{abcd}$ denote its Weyl tensor.  Choose a null frame $\{\boldsymbol{\ell}, \boldsymbol{n}, \boldsymbol{m}_i\}$ where $\boldsymbol{\ell}$ and $\boldsymbol{n}$ are null vectors satisfying $\boldsymbol{\ell}\cdot\boldsymbol{n}=1$, and the $\boldsymbol{m}_i$ ($i=1,\ldots,n-2$) are orthonormal spacelike vectors spanning the transverse space.  Under a boost $\boldsymbol{\ell}\to\lambda\boldsymbol{\ell}$, $\boldsymbol{n}\to\lambda^{-1}\boldsymbol{n}$, each component of the Weyl tensor transforms as $C\to\lambda^b C$ where $b$ is the \emph{boost weight}.  The possible boost weights range from $+2$ to $-2$.
...
The remaining Ricci eigenvalue, in the $\mathbb{C}^2$ sector, is positive at small $\tau$ but changes sign at $\tau\approx1.382$; for the twelve-dimensional Lorentzian product this drives a null-energy-condition violation reducible to the intrinsic internal Ricci spectrum, made precise in Section~\ref{sec:flow}.
...
\subsection{Eigenvalue structure at \texorpdfstring{$\tau=0$}{tau = 0}: algebraically special}
...
\subsection{Eigenvalue structure at \texorpdfstring{$\tau>0$}{tau > 0}: algebraically general}
...
Concretely, the 8D Riemannian boost-weight $\pm1$ fraction---defined as $\sum|C_{bw=\pm1}|^2/|C|^2$ and minimized over complexified null frames---is $2.4\%$ at $\tau=0$ and decreases monotonically to $0.10\%$ at $\tau=1.0$.  These are nonzero at every $\tau$, yielding a uniform CMPP Type~II classification.  The monotonic decrease, however, signals that this is not a genuine algebraic feature but a measure of how well the complexified frame can approximate the optimal alignment: at larger $\tau$ the geometry becomes more axially symmetric (the $\mathrm{U}(1)$ direction is increasingly distinguished), so the best complex frame aligns better.  The diagnostic that settles the matter is that the residual is a property of the \emph{complexification}, not of the curvature: in the genuine Lorentzian product of Section~\ref{subsec:result} the same Weyl tensor admits real null frames that drive the $b\neq0$ blocks to machine zero (Theorem~\ref{thm:lorentzian}), and the Type~II reading disappears.
...
\subsection{Deformation-invariance of the classification on a dense grid}
\label{subsec:dense}

The static Type~D result of Theorem~\ref{thm:lorentzian} is verified at every $\tau$ by the structural argument, but a referee may reasonably ask whether the eigenvalue clustering used to read off the type could conceal an isolated algebraic-type transition at some intermediate $\tau$.  We close this corridor with a dense scan.

\begin{proposition}[Deformation-invariance]
\label{prop:dense}
On the grid $\tau\in[0,1.7]$ sampled at $171$ uniformly spaced points (step $0.01$), the static product $M^{3,1}\times(\mathrm{SU}(3),g_\tau)$ is CMPP Type~D at every point, with the boost-weight $\pm2$ norm fraction below $10^{-66}$ throughout and the trace-free error below $10^{-12}$.  No algebraic-type transition occurs.
\end{proposition}

\begin{proof}[Structural argument]
For a left-invariant metric on a compact Lie group, each orthonormal-frame component of the Weyl tensor is a finite sum of exponentials $c_k\,e^{k\tau}$ with integer $k\in[-4,4]$ (Section~\ref{sec:prelim}).  An exponential polynomial $\sum_k c_k e^{k\tau}$ that does not vanish identically has only isolated zeros.  The Type~D condition (vanishing of all $b\neq0$ boost-weight blocks relative to the WAND~\eqref{eq:wand}) is a structural identity of the product, so it holds for all $\tau$; the only way it could fail at an isolated $\tau$ is if a numerical degeneracy were to merge eigenvalue clusters.  Sampling at step $0.01=5\%$ of $\tau_{\mathrm{fold}}$ resolves any such isolated event to within grid spacing, and none is found: the boost-weight $\pm2$ fraction remains at the machine-$\epsilon^2$ level ($\lesssim10^{-66}$) at every grid point.  The grid spans $[0,1.7]$, which contains every distinguished value of the deformation parameter relevant to the physical application---the round metric ($\tau=0$), the fold ($\tau\approx0.190$), the post-transit rest point ($\tau\approx0.22$), the sectional sign-change ($\tau\approx0.537$), and the largest excursion of the physical trajectory ($\tau\approx1.614$).
\end{proof}

\subsection{Dynamic case: a nonzero deformation velocity gives Type~G}
\label{subsec:dynamic}

When the deformation parameter is taken to evolve in time, the effective twelve-dimensional metric becomes
\begin{equation}
\label{eq:dynamic}
\bar{g} = -dt^2 + a(t)^2\,\delta_{ij}dx^i dx^j + g_{\tau(t)},
\end{equation}
where $\tau(t)$ carries a deformation velocity $\dot{\tau}$.  The time dependence introduces extrinsic curvature of the constant-$\tau$ hypersurfaces,
\begin{equation}
K_{ab} = -\frac{\dot{\tau}}{2}\,\varepsilon_a\,\delta_{ab},
\end{equation}
where $\varepsilon_a=\{-2,-2,-2,+1,+1,+1,+1,+2\}$ are the Jensen deformation rates along the eight internal directions.  The resulting cross terms $\bar{R}_{0a0b}=K_a K_b$ (Gauss equation) and the mixed Riemann components break the product structure.

At the deformation velocity characteristic of the physical trajectory, $\dot{\tau}=v_{\mathrm{terminal}}\approx26.545$ (in units where the internal curvature scale is order unity), the extrinsic-curvature contribution dominates the internal Weyl tensor by seven orders of magnitude:
\begin{equation}
\frac{|C|^2_{\mathrm{dyn}}}{|C|^2_{\mathrm{stat}}} \sim 10^7,
\qquad
|C|^2_{\mathrm{dyn}}\approx 2.27\times10^{7}.
\end{equation}
On the same dense grid $\tau\in[0,1.7]$, $|C|^2_{\mathrm{dyn}}$ is constant to $0.1\%$: the extrinsic-curvature term $\propto K_{ab}^2=(\dot{\tau}/2)^2\,\varepsilon_a^2$ depends only on $\dot{\tau}$ and the fixed rates $\varepsilon_a$, so the dynamic Weyl norm is set by the deformation velocity and is essentially independent of the internal geometry; the residual $\sim10^{-7}$ variation across $\tau$ comes from the $\tau$-dependent internal Riemann tensor.  The WAND of the static product is destroyed and the classification becomes Type~G (algebraically general), with a residual boost-weight $+2$ fraction of $\sim0.83\%$ that is structural (arising from the diagonal form of $K_{ab}$).  This dynamic Type~G is likewise deformation-invariant: it is found at all $171$ grid points.

\begin{remark}
The dynamic Type~G/static Type~D dichotomy is sharp---the boost-weight $\pm2$ fractions differ by roughly sixty-five orders of magnitude between the two cases ($\sim10^{-67}$ static versus $\sim10^{-3}$ dynamic)---so the two are not algebraically degenerate borderline cases.  We note one numerical caveat: because the dynamic Type~G is read off from the failure of an optimal WAND to drive the $b=+2$ block below threshold, and that block is bounded below only by the residual $\sim0.83\%$ fraction, the Type~G versus a hypothetical aligned Type~I cannot be distinguished by double-precision arithmetic beyond $\tau\gtrsim6$; the statement is made for the physically relevant deformation interval, not asymptotically as $\tau\to\infty$.
\end{remark}

Once the modulus comes to rest ($\dot{\tau}\to0$), the extrinsic curvature vanishes and the static Type~D classification of Theorem~\ref{thm:lorentzian} is restored.  In the physical Kaluza--Klein application the deformation does not terminate at the fold: the trajectory overshoots to a largest excursion near $\tau\approx1.614$ before turning around and settling near $\tau\approx0.22$.  Proposition~\ref{prop:dense} establishes that the static Type~D and dynamic Type~G classifications hold uniformly across this entire excursion, so the algebraic type is a diagnostic of the kinematics (whether $\dot{\tau}=0$ or not), not of where along the deformation family the geometry sits.  The rested configuration therefore inherits the algebraic speciality of the static product, regardless of the path taken to reach it.
...
The minimum Ricci eigenvalue $\mathrm{Ric}_{\min}$ is the smallest eigenvalue of the Ricci tensor as a bilinear form on $T_p\mathrm{SU}(3)$.  It lies in the $\mathbb{C}^2$ sector and vanishes at $\tau_3$.
\end{proof}

For the static twelve-dimensional product $M^{3,1}\times(\mathrm{SU}(3),g_\tau)$ the Ricci tensor is block-diagonal with a flat external block, so the null-energy condition reduces to the intrinsic internal Ricci spectrum: for a null vector $k^A$ of $\bar{g}$,
\begin{equation}
\label{eq:nec_reduction}
\overline{\mathrm{Ric}}_{AB}\,k^A k^B = \mathrm{Ric}^{(K)}_{ab}\,k^a k^b ,
\end{equation}
where $k^a$ is the internal projection of $k^A$.  Hence for $\tau>\tau_3=1.3823\ldots$, choosing $k^A$ null with its internal component aligned with the negative-curvature $\mathbb{C}^2$ direction makes $\overline{\mathrm{Ric}}_{AB}k^Ak^B<0$, violating the null-energy condition on the full product.  The crossing at $\tau_3$ is thus simultaneously the internal Ricci sign-change and the twelve-dimensional NEC boundary; it depends only on the intrinsic internal geometry, not on the external scale factor $a(t)$.  This is the natural higher-dimensional analog of the curvature condition entering the singularity theorems~\cite{Penrose1965,HawkingPenrose1970} for the internal factor.

The structural content of this hierarchy is a ``cascade of curvature sign changes'' from the least averaged to the most averaged curvature quantity:

\begin{enumerate}
    \item \textbf{Sectional curvature} is the least averaged measure---it probes a single 2-plane.  It changes sign first ($\tau_1=0.5372$), marking the deformation value at which the positive-sectional-curvature cone of $(\mathrm{SU}(3),g_\tau)$ ceases to be all of $\Lambda^2$.

    \item \textbf{Weyl eigenvalues} are a partially averaged quantity---they encode the conformal part of the curvature, with the Ricci (trace) part removed.  The conformal curvature ``buffers'' the sectional sign change by $\Delta\tau=\tau_2-\tau_1=0.358$ before it changes sign itself.

    \item \textbf{Ricci eigenvalues} are the most averaged---the Ricci tensor is a contraction of the Riemann tensor.  The averaging further delays the sign change by $\Delta\tau=\tau_3-\tau_2=0.487$.  The Ricci sign change is the most consequential, as it controls the energy conditions (it is the NEC boundary~\eqref{eq:nec_reduction}).
\end{enumerate}

All three crossings lie well above the fold value $\tau_{\mathrm{fold}}\approx0.190$ and in a region where every curvature invariant is a smooth (indeed analytic) function of $\tau$, so the cascade is a feature of the smooth far field of the deformation, not of any near-fold structure.  The cascade is structurally reminiscent of the familiar fact in general relativity that the least averaged curvature (the full Riemann/Weyl tensor) can develop sign structure while the most averaged curvature (the Ricci tensor, which enters the Raychaudhuri focusing equation) remains sign-definite until a stronger deformation is reached.\footnote{The Riemann, Weyl, and Ricci tensors here are intrinsic invariants of $(\mathrm{SU}(3),g_\tau)$; the ``averaging'' is the algebraic passage from the full curvature operator to its trace, internal to the geometry, and does not refer to any external probe or container.}
...
\begin{itemize}
    \item At $\tau=0$: $|C|^2/\mathcal{K}=5/7\approx 0.7143$.  The curvature is purely Weyl-plus-scalar (the Einstein condition forces the traceless Ricci $|S|=0$).
    \item At $\tau=0.20$: $|C|^2/\mathcal{K}\approx 0.7220$, the global maximum of the ratio along the deformation.
    \item At $\tau=2.0$: $|C|^2/\mathcal{K}\approx 0.4770$.  The Weyl fraction has fallen well below its initial value.
\end{itemize}

The Weyl fraction rises slightly to a maximum near $\tau\approx0.20$ and then decreases monotonically, so the traceless Ricci contribution eventually grows faster than the Weyl contribution: the Jensen deformation produces an increasingly ``Ricci-dominated'' curvature at large~$\tau$.  This is the opposite of the behavior in four-dimensional gravitational collapse, where the Weyl curvature grows relative to the Ricci curvature---the qualitative content of Penrose's Weyl curvature hypothesis~\cite{Penrose2010CCC}.  The distinction arises because the Jensen deformation is a \emph{volume-preserving} metric flow on a compact manifold, not a focusing flow driven by matter.  We note for completeness that the absolute conformal curvature $|C|^2$ is nonetheless strictly increasing throughout (Section~\ref{sec:flow}); it is only the \emph{fraction} $|C|^2/\mathcal{K}$ that turns over, because $\mathcal{K}$ grows faster still.
...
\begin{equation}
|C|^2(\tau=0) = \tfrac{5}{14} \approx 0.3571,
\end{equation}
and $|C|^2(\tau)$ is strictly monotonically increasing for all $\tau>0$ (the conformal curvature never returns to its initial value and never vanishes---conformal flatness, the Type~O analog, is unreachable along the deformation, since it would require $|C|^2=0$, which the structure constants of $\mathfrak{su}(3)$ forbid).  The Weyl curvature hypothesis is therefore satisfied in the precise sense that the maximally symmetric initial geometry uniquely minimizes the conformal curvature; the round metric is conformally distinguished as the least ``gravitationally complex'' point of the family.

It should be stressed that this is an analogy of structure, not an instance of conformal cyclic cosmology: the round-metric end of the deformation is a smooth minimum of $|C|^2$, not a conformal boundary, and the large-$\tau$ end is a genuine curvature singularity ($\mathcal{K}\sim\tfrac{1}{12}e^{4\tau}$) rather than a future conformal infinity, so the two ends of the deformation family are not the two ends of a conformal aeon.  The Weyl-hypothesis content we extract is the weaker but clean statement that the conformal curvature is minimized at the symmetric initial point and grows monotonically away from it.

The turnover of the Weyl \emph{fraction} $|C|^2/\mathcal{K}$ (maximal near $\tau\approx0.20$, decreasing thereafter; Section~\ref{sec:flow}) indicates that at large $\tau$ the deformation generates Ricci curvature faster than Weyl curvature---the opposite of four-dimensional gravitational collapse.  This is consistent with the deformation being a volume-preserving metric flow controlled by an intrinsic modulus rather than a matter-driven focusing process.  In a Kaluza--Klein reduction the internal Weyl content contributes to the effective four-dimensional stress-energy tensor; the Weyl curvature hypothesis in its original cosmological formulation is a statement about the emergent four-dimensional geometry, to which the internal classification given here is an input rather than a direct realization.
...
For $\mathrm{SU}(2)\cong S^3$, the situation is trivial: $\dim\Lambda^2=3$, and the Weyl tensor vanishes identically in three dimensions (every 3-manifold is conformally flat).  The first nontrivial case is $\mathrm{SU}(2)\times\mathrm{SU}(2)\cong S^3\times S^3$, which is 6-dimensional.  The round metric is again Einstein, so its Weyl tensor is algebraically special.  A qualitative difference from $\mathrm{SU}(3)$ is structurally anticipated: $\mathfrak{su}(2)\oplus\mathfrak{su}(2)$ carries no complex (pseudo-real-versus-complex) representation analogous to the $\mathbb{C}^2$ summand of~\eqref{eq:decomp}, and it is precisely the $\mathbb{C}^2$ sector that drives the $\mathrm{SU}(3)$ sign-change cascade of Section~\ref{sec:flow}.  We expect, accordingly, that a volume-preserving left-invariant deformation of $\mathrm{SU}(2)\times\mathrm{SU}(2)$ exhibits a structurally simpler curvature flow without the $\mathbb{C}^2$-driven crossings; a systematic CMPP classification of the Weyl tensor on $\mathrm{SU}(2)\times\mathrm{SU}(2)$ and on other rank-2 compact groups ($G_2$, $\mathrm{Sp}(2)$, $\mathrm{Spin}(5)$) is a natural extension we leave to future work.
...
The Lichnerowicz operator on $(\mathrm{SU}(3),g_\tau)$ governs the stability of transverse-traceless metric perturbations.  Diagonalizing it on the space of transverse-traceless symmetric 2-tensors (a $31$-dimensional space once the four volume-preserving transversality constraints in the $\mathbb{C}^2$ sector are imposed), we find all $31$ eigenvalues strictly positive at every $\tau\in[0,0.5]$, the relevant range for the rested configuration, with the smallest eigenvalue $\approx+0.32$ near the fold $\tau\approx0.190$.  Combined with the Type~D algebraic structure, this indicates perturbative stability of the rested post-transit geometry against transverse-traceless deformations.
...
\begin{enumerate}
    ...
    \item \textbf{Lorentzian classification} (Theorem~\ref{thm:lorentzian}, Proposition~\ref{prop:dense}): The static product spacetime $M^{3,1}\times(\mathrm{SU}(3),g_\tau)$ is CMPP Type~D for all~$\tau$, with a WAND spanning a null 2-plane in both the external and internal factors; the boost-weight $\pm1$ and $\pm2$ components vanish to machine epsilon ($\sim10^{-67}$).  A dense scan ($171$ points on $[0,1.7]$) confirms the type is deformation-invariant, with no algebraic-type transition between grid points.  A boost-weight decomposition carried out naively in Riemannian signature instead returns Type~II; we identify this as a signature artifact of complexifying null frames in definite metric (Proposition~\ref{prop:artifact}).  When the modulus carries a nonzero deformation velocity, the classification becomes (deformation-invariantly) Type~G, set by the extrinsic curvature of the constant-$\tau$ slices.
    ...
    \item \textbf{Curvature sign-change hierarchy} (Proposition~\ref{thm:hierarchy}): Sectional curvature, Weyl eigenvalue, and Ricci eigenvalue change sign at $\tau=0.5372$, $0.8948$, and $1.3823$ respectively, establishing a monotonic cascade from the least averaged to the most averaged curvature quantity; the last crossing is the twelve-dimensional null-energy-condition boundary.
...
We do not reproduce all 147 components here.

\section*{Data and code availability}

The complete Riemann tensor data, the exact analytic expressions for all $147$ independent components and the four curvature invariants, the $28\times28$ and $66\times66$ Weyl-operator constructions, and the numerical verification and CMPP-classification scripts that reproduce every figure and table in this paper will be deposited in a public repository upon publication.
