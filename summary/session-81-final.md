...
_Built from: session-81-results-workingpaper.md_
...
### session-81-results-workingpaper.md
...
`_batch_t3_migrate.py` pinned every computations/_shared + computation-archive
...
| `_consolidate_t3_prep.py` | Extend master prep doc with per-anchor blocks | ~70 |
| `_harvest_t3_edges.py` | Regex-based edge extraction from verdict prose | ~290 |
| `_batch_t3_migrate.py` | Mass script-pinning tool | ~260 |
